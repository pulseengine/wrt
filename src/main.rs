...
// Our own find_section implementation
fn find_section(binary: &[u8], section_id: u8) -> Result<Option<(usize, usize)>, String> {
    let mut reader = SectionReader::new(binary).map_err(|e| format!("Error creating section reader: {:?}", e))?;
    reader.find_section(section_id).map_err(|e| format!("Error finding section: {:?}", e))
}
    ...
    for payload_result in parser {
        match payload_result {
            Ok(Payload::ImportSection(data, size)) => {
                let reader = match Parser::create_import_section_reader(&Payload::ImportSection(data, size)) {
                    Ok(reader) => reader,
                    Err(err) => {
...
fn test_performance() -> Result<(), String> {
    ...
    for _ in 0..iterations {
        ...
        assert_eq!(result.len(), 1, "Expected 1 builtin");
    }
    ...
    println!("✅ Performance test passed: {} iterations in {}ms ({}µs per scan)", 
        iterations, milliseconds, avg_microseconds);
...
fn test_larger_module() -> Result<(), String> {
    ...
    for payload_result in parser {
        let payload = payload_result.map_err(|e| format!("Failed to parse payload: {:?}", e))?;
    ...
    if !import_section_found {
        return Err("Import section not found in larger module".into());
    }
    
    if section_count < 5 {
        return Err(format!("Expected at least 5 sections, found {}", section_count));
    }
    
    if custom_count != 1 {
        return Err(format!("Expected 1 custom section, found {}", custom_count));
    }
    ...
    if builtins.len() != 2 {
        return Err(format!("Expected 2 builtins, found: {}", builtins.len()));
    }
    
    println!("✅ Larger module parsing works correctly");
    Ok(())
}
...
fn main() {
    ...
    // Define test functions with explicit type
    type TestFn = fn() -> Result<(), String>;
    
    // Use function pointers for the tests
    let tests: [(&str, TestFn); 7] = [
        ("Module version detection", test_parser_finds_module_version as TestFn),
        ("Section finding", test_section_finding as TestFn),
        ("Builtin scanning", test_scanning_for_builtins as TestFn),
        ("Payload iteration", test_payloads as TestFn),
        ("Section reader", test_section_reader as TestFn),
        ("Larger module parsing", test_larger_module as TestFn),
        ("Performance", test_performance as TestFn),
    ];
