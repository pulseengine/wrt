...
//! the WRT foundation's unified type system, memory providers, and safety primitives.
//! These synchronization types are designed to work seamlessly with ASIL-aware
//! safety contexts and bounded collections.
//!
//! # Features
//!
//! - **Safety-Aware**: All primitives integrate with ASIL safety contexts
//! - **Memory-Bounded**: Uses unified memory providers for predictable allocation
//! - **Platform-Configurable**: Adapts behavior based on platform requirements
//! - **Lock-Free Options**: Provides lock-free alternatives for high-performance scenarios
//! - **Verification Support**: Built-in verification for safety-critical applications
//!
//! # Usage
//!
//! ```rust
//! use wrt_sync::unified_sync::{SafeMutex, BoundedChannel};
...
mod foundation_stubs {
    ...
    impl SafetyContext {
        ...
        pub fn record_violation(&self) -> u8 { 0 }
...
use foundation_stubs::{SafetyContext, AsilLevel, WrtResult, Error};
...
// Safety: SafeMutex can be shared across threads if T is Send (access is protected by the lock)
...
impl<T, const CAPACITY: usize> BoundedChannel<T, CAPACITY> {
    ...
    pub fn new(safety_context: SafetyContext) -> WrtResult<(BoundedSender<T, CAPACITY>, BoundedReceiver<T, CAPACITY>)> {
        ...
        Ok((
            BoundedSender { channel: channel_ptr },
            BoundedReceiver { channel: channel_ptr },
        ))
    }
