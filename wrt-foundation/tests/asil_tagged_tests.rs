...
mod framework_tests {
    ...
    fn test_asil_test_categorization() {
        ...
        assert!(
            asil_d_tests.len() >= 3,
            "Should have multiple ASIL-D tests, found: {}",
            asil_d_tests.len()
        );
        ...
        assert!(
            memory_tests.len() >= 2,
            "Should have multiple memory tests, found: {}",
            memory_tests.len()
        );
    ...
    fn test_statistics_accuracy() {
        ...
        assert!(
            stats.total_count >= 6,
            "Should have multiple ASIL tests, found: {}",
            stats.total_count
        );

        // Should have ASIL-D tests (highest safety level)
        assert!(
            stats.asil_d_count >= 2,
            "Should have ASIL-D tests, found: {}",
            stats.asil_d_count
        );

        // Should have memory safety tests
        assert!(
            stats.memory_count >= 2,
            "Should have memory tests, found: {}",
            stats.memory_count
        );

        // Should have resource safety tests
        assert!(
            stats.resource_count >= 2,
            "Should have resource tests, found: {}",
            stats.resource_count
        );

        // Verify totals make sense
        let level_total = stats.qm_count
            + stats.asil_a_count
            + stats.asil_b_count
            + stats.asil_c_count
            + stats.asil_d_count;
        assert_eq!(
            level_total, stats.total_count,
            "ASIL level counts should sum to total"
        );

        let category_total = stats.unit_count
            ...
            + stats.resource_count;
        assert_eq!(
            category_total, stats.total_count,
