...
use wrt_foundation::{
    ...
    safety_system::{SafetyLevel, AsilLevel},
};
...
mod resource_handle_tests {
    ...
    fn test_resource_lifetime_tracking() {
        ...
        
        {
            ...
            let during_stats = BudgetAwareProviderFactory::get_crate_stats(CrateId::Runtime).unwrap();
...
mod performance_tests {
    ...
    fn test_allocation_overhead() {
        ...
        let overhead_ratio = budget_aware_duration.as_nanos() as f64 / direct_duration.as_nanos() as f64;
    ...
    fn test_scalability_thousands_allocations() {
        ...
        assert!(duration < Duration::from_secs(1), "Allocation too slow for {} items", ALLOCATION_COUNT);
...
fn test_comprehensive_system_stress() {
    ...
    let workload_threads: Vec<_> = (0..4).map(|i| {
        thread::spawn(move || {
