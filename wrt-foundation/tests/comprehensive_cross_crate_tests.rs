...
mod error_propagation_tests {
    ...
    fn test_budget_exhaustion_recovery() {
        ...
        let _dropped: Vec<_> = providers.into_iter().take(half).collect();
        drop(_dropped);
...
mod performance_tests {
    ...
    fn test_allocation_overhead() {
        ...
        assert!(
            overhead_ratio < 5.0,
            "Budget tracking overhead too high: {:.2}x",
            overhead_ratio
        );
...
mod thread_safety_tests {
    ...
    fn test_atomic_budget_operations() {
        ...
        usage_before.store(
            BudgetAwareProviderFactory::get_crate_stats(CrateId::Runtime)
                .unwrap()
                .allocated_bytes,
            Ordering::SeqCst,
        );
...
mod diagnostic_integration_tests {
    ...
    fn test_memory_leak_detection() {
        ...
        let leaked_providers: Vec<BudgetProvider<512>> = (0..10)
            .filter_map(|_| BudgetProvider::<512>::new(CrateId::Logging).ok())
            .collect();
        ...
        println!(
            "Simulated memory leak scenario with {} allocations",
            leaked_providers.len()
        );
...
mod compile_time_enforcement_tests {
    ...
    fn test_type_safety_enforcement() {
        ...
        // let bad_provider: BudgetProvider<0> =
        // BudgetProvider::new(CrateId::Runtime).unwrap(); let negative_size:
        // BudgetProvider<-1> = BudgetProvider::new(CrateId::Runtime).unwrap();
...
fn test_comprehensive_system_stress() {
    ...
    for handle in workload_threads {
        ...
        println!(
            "Crate {:?}: allocated {}, retained {}",
            crate_id, allocated, retained
        );
