...
mod cross_crate_enforcement_tests {
    ...
    fn test_component_format_interaction() -> WrtResult<()> {
        ...
        let mut component_types = BoundedMap::<u32, BoundedString<64, _>, 100, _>::new(
            component_provider
        )?;
    ...
    fn test_debug_logging_interaction() -> WrtResult<()> {
        ...
        let mut trace_buffer = BoundedVec::<BoundedString<128, _>, 100, _>::new(
            debug_provider
        )?;
    ...
    fn test_enforcement_under_pressure() -> WrtResult<()> {
        ...
        assert!(allocations.len() < crates.len(), "All large allocations succeeded - budget not enforced");
