...
fn test_hierarchical_budget_system() {
    ...
    let mut budget = hierarchical_budgets::HierarchicalBudget::<4>::new(
        CrateId::Component, 
        10_000
    );
    
    // Add sub-budgets with different priorities
    budget.add_sub_budget("critical", 4000, hierarchical_budgets::MemoryPriority::Critical).unwrap();
    ...
    let (guard1, sub_idx1) = budget.allocate_prioritized::<1024>(
        512, 
        hierarchical_budgets::MemoryPriority::Critical
    ).unwrap();
    
    let (guard2, sub_idx2) = budget.allocate_prioritized::<1024>(
        256, 
        hierarchical_budgets::MemoryPriority::High
    ).unwrap();
...
fn test_auto_sizing_macros() {
    ...
    let _guard1 = auto_provider!(CrateId::Foundation, typical_usage: "bounded_collections").unwrap();
