...
use crate::{
    ...
    memory_coordinator::{GenericMemoryCoordinator, CrateIdentifier},
    wrt_memory_system::{WrtMemoryCoordinator, WrtProviderFactory, WRT_MEMORY_COORDINATOR},
...
mod proofs {
    ...
    fn verify_budget_never_exceeded() {
        ...
        let budgets = [
            (CrateId::Foundation, 1024),
            (CrateId::Component, 2048),
        ];
    ...
    fn verify_crate_budget_never_exceeded() {
        ...
        match coordinator.register_allocation(CrateId::Foundation, size) {
            ...
            Err(_) => {
                ...
                assert!(size > crate_budget || 
                       coordinator.get_crate_allocation(CrateId::Foundation) + size > crate_budget);
            }
        }
    }
