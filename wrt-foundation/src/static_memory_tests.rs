...
use crate::{
    ...
    memory_enforcement::{BoundedMemoryBlock, ComponentMemoryBlocks, pre_allocate_component_memory},
    memory_system::{ConfigurableProvider, SmallProvider, MediumProvider, UnifiedMemoryProvider},
    bounded_infrastructure::{BoundedSystemRegistry, BoundedEventSystem, BoundedMemoryPool, EventType},
...
fn test_event_system_bounded_queue() {
    ...
    for i in 0..100 {
        ...
        assert!(events.emit_event(
            EventType::ComponentInitialized,
            &source,
            payload.as_bytes()
        ).is_ok());
    }
...
fn test_no_dynamic_allocation_in_critical_path() {
    ...
    let string: BoundedString<128, TestProvider> = BoundedString::from_str("test", provider).unwrap();
