...
impl SafetyMonitor {
    ...
    fn update_error_rate(&mut self) {
        self.error_monitor.recent_error_count = self.error_monitor.recent_error_count.saturating_add(1;
...
/// # Safety
/// 
/// This function uses unsafe code for lock-free synchronization which is necessary
/// for runtime safety monitoring in no_std environments. The unsafe operations are:
/// 1. Volatile reads/writes to implement a spinlock without std::sync primitives
...
macro_rules! safety_assert {
    ($cond:expr, $violation:expr) => {
        if !$cond {
            $crate::safety_monitor::with_safety_monitor(|monitor| {
                match $violation {
                    $crate::safety_monitor::ViolationType::BudgetExceeded => {
                        monitor.record_budget_violation(
                            $crate::CrateId::Foundation, 0, 0
                        ;
                    }
