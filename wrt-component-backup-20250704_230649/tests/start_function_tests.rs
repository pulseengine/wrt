...
impl LinkInterceptorStrategy for TestInterceptor {
    fn intercept_function_call(
        ...
        match self.intercept_mode {
            ...
            InterceptMode::ThrowError => Err(Error::runtime_execution_error("Missing error message"Interceptor blocked function callMissing messageMissing messageMissing message")),
    ...
    fn intercept_function_result(
        ...
        match self.intercept_mode {
            ...
            InterceptMode::ThrowError => Err(Error::runtime_execution_error("Missing error message"Interceptor blocked function resultMissing messageMissing messageMissing message")),
            _ => result,
        }
    }
...
impl Component {
    ...
    fn execute_start_function_with_integrity(&self) -> Result<(), Error> {
        ...
        if self.start_should_fail {
            Err(Error::runtime_execution_error("Missing error message"Start function failedMissing messageMissing messageMissing message")
        } else if let Some(timeout) = self.options.execution_timeout {
            if timeout.as_millis() < 100 {
                // Simulate timeout for very short timeouts
                Err(Error::from(ExecutionTimeoutError::runtime_execution_error("Missing error message",
                    timeout,
                ))
            } else {
