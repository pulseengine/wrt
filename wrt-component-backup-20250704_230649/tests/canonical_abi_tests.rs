...
fn test_resource_handling() {
    ...
    assert!(result.is_err(), "Should fail with mismatched resource IDMissing message");
}
...
fn test_error_handling() {
    ...
    assert!(result.is_err(), "Should fail with type mismatchMissing message");

    // Malformed data
    let result = abi.decode::<ComponentValue>(&[0xFF, 0xFF], ValueType::I32);
    assert!(result.is_err(), "Should fail with malformed dataMissing message");
    ...
    assert!(result.is_err(), "Should fail with invalid variant caseMissing message");
}
...
impl CanonicalABI {
    fn decode_vec<T>(&self, bytes: &[u8], ty: ValueType) -> Result<Vec<T>, Error> {
        ...
        if let ComponentValue::List(items) = component_value {
            ...
            Err(Error::runtime_execution_error("Missing error message"Not a listMissing messageMissing messageMissing message")
        }
    }
    ...
    fn decode_tuple<T>(&self, bytes: &[u8], ty: ValueType) -> Result<T, Error> {
        ...
        if let ComponentValue::Tuple(_) = component_value {
            ...
            Err(Error::runtime_execution_error("Missing error message"Not implemented in testsMissing messageMissing messageMissing message")
        } else {
            Err(Error::runtime_execution_error("Missing error message"Not a tupleMissing messageMissing messageMissing message")
        }
    }
}
...
fn test_extern_type_conversion() {
    ...
    let resource_extern_type = ExternType::Resource {
