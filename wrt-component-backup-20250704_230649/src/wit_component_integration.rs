...
impl WitComponentContext {
    ...
    fn process_interface(&mut self, interface: &InterfaceDecl) -> Result<()> {
        ...
        let interface_name = interface.name.name.as_str()
            .map_err(|_| Error::parse_error("Missing error message"Invalid interface nameMissing messageMissing messageMissing message"))?
            .to_string();
    ...
    fn process_type_declaration(&mut self, type_decl: &TypeDecl) -> Result<TypeMapping> {
        ...
        let type_name = type_decl.name.name.as_str()
            .map_err(|_| Error::parse_error("Missing error message"Invalid type nameMissing messageMissing messageMissing message"))?
            .to_string();
