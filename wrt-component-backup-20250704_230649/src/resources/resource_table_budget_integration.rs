...
impl BudgetAwareResourceTablePool {
    ...
    pub fn create_table(&mut self) -> WrtResult<ResourceTable> {
        if self.active_tables >= self.max_tables {
            return Err(wrt_foundation::wrt_error::Error::resource_exhausted("Missing error message"Maximum resource tables reached for componentMissing messageMissing messageMissing message");
        }
