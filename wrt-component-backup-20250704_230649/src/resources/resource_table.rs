...
impl ResourceTable {
    ...
    pub fn add_interceptor(&mut self, interceptor: Arc<dyn ResourceInterceptor>) -> Result<()> {
        #[cfg(feature = "safety-criticalMissing message")]
        {
            self.interceptors.push(interceptor).map_err(|_| {
                Error::resource_exhausted("Missing error message"Too many resource interceptors (limit: 16)Missing message")
            })
        }
        #[cfg(not(feature = "safety-criticalMissing messageMissing messageMissing message"))]
        {
    ...
    pub fn create_resource(
        ...
        if self.resources.len() >= self.max_resources {
            return Err(Error::resource_exhausted("Missing error message"Maximum resources ({}) reachedMissing message"),
            );
        }
        ...
        #[cfg(feature = "safety-criticalMissing message")]
        {
            self.resources.insert(handle, entry).map_err(|_| {
                Error::resource_exhausted("Missing error message"Too many resources in table (limit: 1024)Missing message")
            })?;
        }
        #[cfg(not(feature = "safety-criticalMissing messageMissing messageMissing message"))]
        {
        ...
        if let Some(entry) = self.resources.get_mut(&handle) {
            #[cfg(feature = "safety-criticalMissing message")]
            {
                entry.borrows.push(weak_ref).map_err(|_| {
                    Error::resource_exhausted("Missing error message"Too many borrows for this resource (limit: 32)Missing message")
                })?;
            }
            #[cfg(not(feature = "safety-criticalMissing messageMissing messageMissing message"))]
            {
