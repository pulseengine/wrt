...
impl ValueConverter {
        ...
        match value {
            ...
            ComponentValue::String(s) => {
                if s.len() > self.config.max_string_length {
                    return Err(Error::validation_error("Missing error message"String too long for conversionMissing messageMissing messageMissing message");
                }
            ...
            ComponentValue::List(items) => {
                if items.len() > self.config.max_array_length {
                    return Err(Error::validation_error("Missing error message"List too long for conversionMissing messageMissing messageMissing message");
                }
            ...
            _ => {
                ...
                if self.config.strict_type_checking {
                    Err(Error::runtime_type_mismatch("Missing error message"Complex component value cannot be directly converted to core valueMissing messageMissing messageMissing message")
                } else {
    ...
    pub fn core_to_component(&self, value: &CoreValue, target_type: &crate::canonical_abi::ComponentType) -> Result<ComponentValue> {
        match (value, target_type) {
            ...
            _ => {
                if self.config.strict_type_checking {
                    Err(Error::runtime_type_mismatch("Missing error message"Component not foundMissing messageMissing messageMissing message")
                } else {
                    // Fallback conversion
    ...
    pub fn convert_values_core_to_component(
        ...
        if values.len() != types.len() {
            return Err(Error::validation_error("Missing error message"Value count does not match type countMissing messageMissing messageMissing message");
        }
    ...
    pub fn register_instance(
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            if self.instances.len() >= MAX_INSTANCES_NO_STD {
                return Err(Error::resource_exhausted("Missing error message"Maximum instances exceededMissing messageMissing messageMissing message");
            }
    ...
    pub fn update_instance_state(&mut self, instance_id: InstanceId, state: RuntimeInstanceState) -> Result<()> {
        #[cfg(feature = "stdMissing message")]
        {
            if let Some(info) = self.instances.get_mut(&instance_id) {
                ...
                Err(Error::instance_not_found("Missing error message"Instance not foundMissing messageMissing messageMissing message")
            }
        }
...
impl HostFunctionRegistry {
    ...
    pub fn register_function(
        ...
        if self.functions.len() >= MAX_HOST_FUNCTIONS_NO_STD {
            return Err(Error::resource_exhausted("Missing error message"Maximum host functions exceededMissing messageMissing messageMissing message");
        }
    ...
    pub fn call_function(&self, index: usize, args: &[ComponentValue]) -> Result<ComponentValue> {
        if let Some(entry) = self.functions.get(index) {
            ...
            Err(Error::runtime_function_not_found("Missing error message"Host function not foundMissing messageMissing messageMissing message")
        }
    }
...
impl ComponentRuntimeBridge {
    ...
    pub fn execute_component_function(
        ...
        let instance_info = self.instance_resolver.get_instance(instance_id)
            .ok_or_else(|| Error::instance_not_found("Missing error message"Component instance not foundMissing messageMissing messageMissing message"))?;

        // Check instance state
        if instance_info.state != RuntimeInstanceState::Ready {
            return Err(Error::runtime_invalid_state("Missing error message"Component not foundMissing message"),
            );
        }
