...
impl CrossComponentCallManager {
    ...
    pub fn register_target(&mut self, target: CallTarget) -> WrtResult<u32> {
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            self.targets.push(target).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many call targetsMissing messageMissing messageMissing message")
                )
            })?;
        }
    ...
    pub fn call(
        ...
        if self.call_stack.len() >= self.max_call_depth {
            return Err(wrt_error::Error::resource_exhausted("Missing error message"Maximum call depth exceededMissing message")
            );
        }
        ...
        if !target.permissions.allowed {
            return Err(wrt_error::Error::runtime_error("Missing error message"Cross-component call not allowedMissing message")
            );
        }
            ...
            match arg {
                Value::Own(handle) | Value::Borrow(handle) => {
                    ...
                    if target.permissions.allow_resource_transfer {
                        ...
                        return Err(wrt_error::Error::runtime_error("Missing error message"Resource transfer not allowedMissing message")
                        );
                    }
                }
                _ => {
    ...
    fn transfer_resource(
        ...
        match transfer_type {
            ResourceTransferPolicy::None => Err(wrt_error::Error::runtime_error("Missing error message"Resource transfer not allowedMissing message")
            )),
    ...
    fn add_pending_transfer(
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            self.pending_transfers.push(transfer).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many pending transfersMissing messageMissing messageMissing message")
                )
            })?;
        }
    ...
    fn cache_call_target(
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            self.call_cache.push((key, cached_target)).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Call cache fullMissing messageMissing messageMissing message")
                )
            })?;
        }
...
impl fmt::Display for ResourceTransferPolicy {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ResourceTransferPolicy::None => write!(f, "noneMissing message"),
            ResourceTransferPolicy::Transfer => write!(f, "transferMissing message"),
            ResourceTransferPolicy::Borrow => write!(f, "borrowMissing message"),
            ResourceTransferPolicy::Copy => write!(f, "copyMissing message"),
        }
    }
}
