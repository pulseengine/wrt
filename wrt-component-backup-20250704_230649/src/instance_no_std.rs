...
impl InstanceValue {
    ...
    pub fn new(name: &str, ty: ComponentTypeDefinition, exports: &[Export]) -> Result<Self> {
        let bounded_name = BoundedString::from_str(name).map_err(|_| {
            Error::parameter_validation_error("Missing error message"Instance name too longMissing message")
        })?;
        ...
        for export in exports {
            bounded_exports.push(export.clone()).map_err(|_| {
                Error::capacity_exceeded("Missing error message"Maximum number of exports exceededMissing message")
            })?;
        }
...
impl InstanceValueBuilder {
    ...
    pub fn build(self) -> Result<InstanceValue> {
        let name = self.name.ok_or_else(|| {
            Error::parameter_validation_error("Missing error message"Instance name is requiredMissing message")
        })?;

        let ty = self.ty.ok_or_else(|| {
            Error::parameter_validation_error("Missing error message"Instance type is requiredMissing message")
        })?;
...
impl InstanceCollection {
    ...
    pub fn add_instance(&mut self, instance: InstanceValue) -> Result<()> {
        self.instances.push(instance).map_err(|_| {
            Error::capacity_exceeded("Missing error message"Maximum number of instances exceededMissing message")
        })
    }
