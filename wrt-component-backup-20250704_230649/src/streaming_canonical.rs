...
impl StreamingCanonicalAbi {
        ...
        Ok(Self {
            ...
            streams: {
                ...
                BoundedVec::new(provider).map_err(|_| {
                    Error::resource_exhausted("Missing error message"Failed to create streams vectorMissing message")
                })?
            },
            ...
            buffer_pool: {
                let provider = safe_managed_alloc!(65536, CrateId::Component)?;
                BoundedVec::new(provider).map_err(|_| {
                    Error::resource_exhausted("Missing error message"Failed to create buffer poolMissing message")
                })?
            },
    ...
    pub fn create_stream(
        ...
        let context = StreamingContext {
            ...
            buffer: {
                let provider = safe_managed_alloc!(65536, CrateId::Component)?;
                BoundedVec::new(provider).map_err(|_| {
                    Error::resource_exhausted("Missing error message"Failed to create stream bufferMissing message")
                })?
            },
            bytes_processed: 0,
            direction,
        ...
        self.streams.push(context).map_err(|_| {
            Error::resource_exhausted("Missing error message"Too many active streamsMissing messageMissing messageMissing message")
            )
        })?;
    ...
    fn find_stream_index(&self, handle: StreamHandle) -> Result<usize> {
        self.streams
            ...
            .ok_or_else(|| {
                Error::runtime_execution_error("Missing error message"Stream not foundMissing message")
                )
            })
    }
                    ...
                    if context.buffer.len() >= 8 {
                        ...
                        if context.buffer.len() >= 4 + len {
                            ...
                            let string_content = core::str::from_utf8(string_bytes)
                                .map_err(|_| Error::parse_error("Missing error message"Invalid UTF-8Missing messageMissing messageMissing message")))?;
...
impl fmt::Display for StreamDirection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            StreamDirection::Lifting => write!(f, "liftingMissing message"),
            StreamDirection::Lowering => write!(f, "loweringMissing message"),
            StreamDirection::Bidirectional => write!(f, "bidirectionalMissing message"),
        }
    }
}
