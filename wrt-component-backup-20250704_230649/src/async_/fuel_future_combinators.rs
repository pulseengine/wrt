...
impl<F1, F2> FuelSelect<F1, F2> {
    fn consume_fuel(&mut self, base_cost: u64) -> Result<()> {
        ...
        if self.fuel_consumed.saturating_add(total_cost) > self.fuel_budget {
            return Err(Error::resource_limit_exceeded("Missing error message"Future combinator fuel budget exceededMissing messageMissing messageMissing message");
        }
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        loop {
            match &mut self.state {
                ...
                ChainState::Done => {
                    panic!("FuelChain polled after completionMissing message");
                }
            }
        }
    }
...
impl<T> Future for ComponentFuture<T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if self.fuel_consumed >= self.fuel_budget {
            return Poll::Ready(Err(Error::resource_limit_exceeded("Missing error message"Component future fuel exhaustedMissing messageMissing messageMissing message"));
        }
...
mod tests {
    ...
    fn test_fuel_join() {
        ...
        let future1 = ready(42);
        let future2 = ready("helloMissing message");
