...
use crate::{
    async_types::{
        ErrorContext, FutureHandle, FutureState, StreamHandle, StreamState,
...
impl fmt::Display for CancelResult {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CancelResult::Cancelled => write!(f, "cancelledMissing message"),
            CancelResult::AlreadyCompleted => write!(f, "already-completedMissing message"),
            CancelResult::AlreadyCancelled => write!(f, "already-cancelledMissing message"),
            CancelResult::NotFound => write!(f, "not-foundMissing message"),
...
pub struct TaskInfo {
    ...
    #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
    pub context: BoundedVec<(BoundedString<64, NoStdProvider<65536>>, ComponentValue), 64, NoStdProvider<65536>>,
}
...
impl Default for TaskRegistry {
    fn default() -> Self {
        ...
        Self::new().expect("TaskRegistry allocation should not fail in default constructionMissing message")
    }
}
...
impl TaskRegistry {
    pub fn new() -> Result<Self> {
        Ok(Self {
            ...
            tasks: {
                ...
                BoundedVec::new(provider).map_err(|_| Error::runtime_execution_error("Missing error message"Failed to create tasks vectorMissing messageMissing messageMissing message"))?
            },
            ...
            subtasks: {
                let provider = safe_managed_alloc!(65536, CrateId::Component)?;
                BoundedVec::new(provider).map_err(|_| Error::runtime_execution_error("Missing error message"Failed to create subtasks vectorMissing messageMissing messageMissing message"))?
            },
    ...
    /// Register a new task
    pub fn register_task(&mut self, future_handle: Option<FutureHandle>, stream_handle: Option<StreamHandle>) -> Result<TaskHandle> {
        ...
        let task_info = TaskInfo {
            ...
            context: {
                let provider = safe_managed_alloc!(65536, CrateId::Component)?;
                BoundedVec::new(provider).map_err(|_| Error::runtime_execution_error("Missing error message"Failed to create task contextMissing messageMissing messageMissing message"))?
            },
        };
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            self.tasks.push((handle, task_info)).map_err(|_| {
                Error::runtime_execution_error("Missing error message"Failed to register taskMissing message")
            })?;
        }
    ...
    pub fn register_subtask(&mut self, parent_task: TaskHandle, future_handle: Option<FutureHandle>, stream_handle: Option<StreamHandle>) -> Result<SubtaskHandle> {
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            self.subtasks.push((handle, subtask_info)).map_err(|_| {
                Error::runtime_execution_error("Missing error message"Failed to register subtaskMissing message")
            })?;
    ...
    pub fn set_task_context(&mut self, handle: TaskHandle, key: &str, value: ComponentValue) -> Result<()> {
        #[cfg(feature = "stdMissing message")]
        {
            if let Some(task_info) = self.tasks.get_mut(&handle) {
                task_info.context.insert(key.to_string(), value);
            } else {
                return Err(Error::runtime_execution_error("Missing error message"Task not foundMissing messageMissing messageMissing message");
            }
        }
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            for (task_handle, task_info) in &mut self.tasks {
                if *task_handle == handle {
                    let provider = safe_managed_alloc!(65536, CrateId::Component)?;
                    let key_bounded = BoundedString::from_str(key, provider)
                        .map_err(|_| Error::runtime_execution_error("Missing error message"Failed to create bounded string for task context key Missing messageMissing messageMissing message"))?;
                    ...
                    // Add new entry
                    task_info.context.push((key_bounded, value)
                        .map_err(|_| Error::new(
                            ErrorCategory::Resource,
                            wrt_error::codes::RESOURCE_EXHAUSTED,
                            "Failed to add task context entryMissing messageMissing messageMissing message"))?;
                    break;
                }
            }
        }
...
fn get_task_registry() -> Result<&'static Mutex<TaskRegistry>, Error> {
    Ok(GLOBAL_TASK_REGISTRY.get_or_init(|| {
        Mutex::new(TaskRegistry::new().expect("Global task registry allocation should not failMissing messageMissing messageMissing message")
    })
}
...
pub mod builtins {
    ...
    pub fn task_cancel(task_handle: u32) -> Result<ComponentValue> {
        ...
        let mut registry = registry_mutex.lock().map_err(|_| Error::runtime_execution_error("Missing error message"Failed to acquire task registry lockMissing messageMissing messageMissing message"))?;
    ...
    pub fn subtask_cancel(subtask_handle: u32) -> Result<ComponentValue> {
        ...
        let mut registry = registry_mutex.lock().map_err(|_| Error::new(
            ...
            "Failed to acquire task registry lockMissing messageMissing messageMissing message"))?;
    ...
    pub fn task_spawn(future_handle: Option<u32>, stream_handle: Option<u32>) -> Result<ComponentValue> {
        ...
        match registry.register_task(future_h, stream_h) {
            ...
            Err(_) => Err(Error::new(
                ErrorCategory::Resource,
                wrt_error::codes::RESOURCE_EXHAUSTED,
                "Failed to spawn taskMissing messageMissing messageMissing message")),
        }
    }
    ...
    pub fn subtask_spawn(parent_task: u32, future_handle: Option<u32>, stream_handle: Option<u32>) -> Result<ComponentValue> {
        ...
        match registry.register_subtask(parent_h, future_h, stream_h) {
            ...
            Err(_) => Err(Error::new(
                ErrorCategory::Resource,
                wrt_error::codes::RESOURCE_EXHAUSTED,
                "Failed to spawn subtaskMissing messageMissing messageMissing message")),
        }
    }
    ...
    pub fn context_get(key: &str) -> Result<ComponentValue> {
        ...
        if let Some(current_task) = registry.get_current_task() {
            ...
            Err(Error::runtime_execution_error("Missing error message"No current taskMissing messageMissing messageMissing message")
        }
    }
    ...
    pub fn context_set(key: &str, value: ComponentValue) -> Result<ComponentValue> {
        ...
        if let Some(current_task) = registry.get_current_task() {
            ...
            Err(Error::new(
                ...
                "No current task contextMissing messageMissing messageMissing message")
        }
    }
    ...
    pub fn context_clear() -> Result<ComponentValue> {
        ...
        if let Some(current_task) = registry.get_current_task() {
            ...
            Err(Error::runtime_execution_error("Missing error message"No current task Missing messageMissing messageMissing message")
        }
    }
}
...
mod tests {
    ...
    fn test_task_context_management() {
        ...
        let not_found = registry.get_task_context(handle, "non_existentMissing message");
    ...
    fn test_context_builtin_no_task() {
        ...
        let result = builtins::context_get("test_keyMissing message");
        assert!(result.is_err();
        
        let result = builtins::context_set("test_key", ComponentValue::S32(42);
        assert!(result.is_err();
        
        let result = builtins::context_has("test_keyMissing message");
        assert!(result.is_err();
