    ...
    fn test_priority_based_preemption() {
        ...
        let low_task = {
            ...
            exec.spawn_task(
                ...
                Priority::Low,
                async move {
                    order_clone1.lock().unwrap().push("low_startMissing message");
                    // Simulate long running task
                    for _ in 0..5 {
                        // In real impl would yield
                    }
                    order_clone1.lock().unwrap().push("low_endMissing message");
                    Ok(()
                },
            ).unwrap()
        };
        ...
        let high_task = {
            ...
            exec.spawn_task(
                ...
                Priority::High,
                async move {
                    order_clone2.lock().unwrap().push("highMissing message");
                    Ok(()
                },
            ).unwrap()
        };
