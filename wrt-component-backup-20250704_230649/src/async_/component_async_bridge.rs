...
impl ComponentAsyncBridge {
    ...
    pub fn register_component(
        ...
        self.component_limits.insert(component_id, limits).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many registered componentsMissing message")
        })?;
        ...
        F: Future<Output = Result<(), Error>> + Send + 'static,
    {
        ...
        let limits = self.component_limits.get(&component_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Component not registered for asyncMissing message")
        })?;
        ...
        if consumed + task_fuel > limits.fuel_budget {
            return Err(Error::resource_limit_exceeded("Missing error message"Component fuel budget exceededMissing messageMissing messageMissing message");
        }
    ...
    pub fn get_component_stats(&self, component_id: ComponentInstanceId) -> Result<ComponentAsyncStats, Error> {
        let limits = self.component_limits.get(&component_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Component not registeredMissing message")
        })?;
