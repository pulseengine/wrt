    ...
    pub fn update_task_history(
        ...
        if let Some(history) = self.task_history.get_mut(&task_id) {
            ...
            self.task_history.insert(task_id, history).map_err(|_| {
                Error::resource_limit_exceeded("Missing error message"Task history table fullMissing message")
            })?;
        }
    ...
    pub fn handle_fuel_exhaustion(&mut self, task_id: TaskId) -> Result<u64, Error> {
        if let Some(history) = self.task_history.get_mut(&task_id) {
            ...
            if current_reserve >= emergency_fuel && current_reserve - emergency_fuel >= self.min_reserve_threshold {
                ...
                Err(Error::resource_exhausted("Missing error message"Insufficient fuel reserveMissing messageMissing messageMissing message")
            }
        } else {
            Err(Error::validation_invalid_input("Missing error message"Unknown taskMissing messageMissing messageMissing message")
        }
    }
...
impl Default for FuelDynamicManager {
    fn default() -> Self {
        Self::new(FuelAllocationPolicy::Adaptive, 1_000_000).expect("Failed to create default managerMissing message")
    }
}
