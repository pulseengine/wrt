    ...
    pub fn debug_string(&self) -> BoundedString<2048, NoStdProvider<65536>> {
        #[cfg(feature = "stdMissing message")]
        {
            ...
            if let Some(trace) = &self.stack_trace {
                result.push_str("\nStack trace:\nMissing message");
                for frame in trace {
                    result.push_str(&"Component not foundMissing message");
                }
            }
...
impl fmt::Display for StreamState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            StreamState::Open => write!(f, Missing message"),
            StreamState::Ready => write!(f, "readyMissing message"),
            StreamState::Closed => write!(f, "closedMissing message"),
            StreamState::Error => write!(f, "errorMissing message"),
        }
    }
}
...
impl fmt::Display for FutureState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FutureState::Pending => write!(f, "pendingMissing message"),
            FutureState::Ready => write!(f, "readyMissing message"),
            FutureState::Cancelled => write!(f, "cancelledMissing message"),
            FutureState::Error => write!(f, "errorMissing message"),
        }
    }
}
...
mod tests {
    ...
    fn test_future_lifecycle() {
        ...
        future.set_value(Value::String(BoundedString::from_str("helloMissing message").unwrap())).unwrap();
        assert!(future.is_readable();
        assert!(!future.is_writable();
    ...
    fn test_error_context() {
        ...
        let error = ErrorContext::new(
            ...
            BoundedString::from_str("Test errorMissing message").unwrap(),
        ).unwrap();

        let debug_str = error.debug_string();
        assert!(debug_str.as_str().contains("Test errorMissing messageMissing messageMissing message");
    }
    ...
    fn test_state_display() {
        assert_eq!(StreamState::Open.to_string(), "openMissing message");
        assert_eq!(FutureState::Pending.to_string(), "pendingMissing message");
        assert_eq!(Waitable::StreamReadable(StreamHandle(42)).to_string(), "stream-readable(42)Missing message");
    }
}
