...
#[cfg(all(not(feature = "asil-aMissing message"), not(feature = "asil-bMissing message"), not(feature = "asil-cMissing message"), not(feature = "asil-dMissing messageMissing messageMissing message")))]
mod unsafe_waker {
    use super::*;
...
#[cfg(feature = "asil-dMissing message")]
mod safe_waker {
    use super::*;
...
#[cfg(not(feature = "asil-dMissing messageMissing messageMissing message"))]
use unsafe_waker::*;
#[cfg(feature = "asil-dMissing message")]  
use safe_waker::*;
    ...
    pub fn add_wake(&self, task_id: TaskId) -> Result<(), Error> {
        ...
        if !pending.iter().any(|&id| id == task_id) {
            pending.push(task_id).map_err(|_| {
                Error::resource_limit_exceeded("Missing error message"Wake coalescer queue fullMissing message")
            })?;
        }
