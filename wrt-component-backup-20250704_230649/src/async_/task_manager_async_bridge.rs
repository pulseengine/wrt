...
impl TaskManagerAsyncBridge {
    ...
    pub fn initialize_component_async(
        ...
        self.async_contexts.insert(component_id, context).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many component async contextsMissing message")
        })?;
        ...
        F: CoreFuture<Output = Result<Vec<ComponentValue>, Error>> + Send + 'static,
    {
        ...
        let context = self.async_contexts.get_mut(&component_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Component not initialized for asyncMissing message")
        })?;

        if context.async_state != ComponentAsyncState::Active {
            return Err(Error::validation_invalid_state("Missing error message"Component async operations not activeMissing messageMissing messageMissing message");
        }

        // Check resource limits
        if context.active_tasks.len() >= context.resource_limits.max_concurrent_tasks {
            return Err(Error::resource_limit_exceeded("Missing error message"Component async task limit exceededMissing messageMissing messageMissing message");
        }
        ...
        self.async_tasks.insert(component_task_id, async_task).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many async tasksMissing message")
        })?;

        self.task_mapping.insert(component_task_id, executor_task_id).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Task mapping table fullMissing message")
        })?;

        // Update component context
        context.active_tasks.push(component_task_id).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Component task list fullMissing message")
        })?;
    ...
    pub fn create_future_handle(
        ...
        if context.futures.len() >= context.resource_limits.max_futures {
            return Err(Error::resource_limit_exceeded("Missing error message"Component future limit exceededMissing messageMissing messageMissing message");
        }
        ...
        context.futures.insert(handle, task_id).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Future handle table fullMissing message")
        })?;
    ...
    pub fn create_stream_handle(
        ...
        if context.streams.len() >= context.resource_limits.max_streams {
            return Err(Error::resource_limit_exceeded("Missing error message"Component stream limit exceededMissing messageMissing messageMissing message");
        }
        ...
        context.streams.insert(handle, task_id).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Stream handle table fullMissing message")
        })?;
    ...
    pub fn task_wait(&mut self, waitables: WaitableSet) -> Result<u32, Error> {
        let current_task = {
            ...
            tm.current_task_id().ok_or_else(|| {
                Error::validation_invalid_state("Missing error message"No current taskMissing message")
            })?
        };
