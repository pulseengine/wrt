...
impl FuelWcetAnalyzer {
    ...
    pub fn analyze_task_wcet(
        ...
        self.analysis_results.insert(task_id, result.clone()).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many WCET analysis resultsMissing message")
        })?;
    ...
    pub fn collect_execution_sample(
        ...
        if !self.execution_samples.contains_key(&task_id) {
            ...
            self.execution_samples.insert(task_id, samples).map_err(|_| {
                Error::resource_limit_exceeded("Missing error message"Too many task sample collectionsMissing message")
            })?;
        }
        ...
        if let Some(samples) = self.execution_samples.get_mut(&task_id) {
            ...
            samples.push(sample).map_err(|_| {
                Error::resource_limit_exceeded("Missing error message"Sample collection is fullMissing message")
            })?;
        }
    ...
    pub fn register_control_flow_path(
        ...
        for &bb in basic_blocks {
            bb_vec.push(bb).map_err(|_| {
                Error::resource_limit_exceeded("Missing error message"Too many basic blocks in pathMissing message")
            })?;
        }
        ...
        if !self.task_paths.contains_key(&task_id) {
            ...
            self.task_paths.insert(task_id, paths).map_err(|_| {
                Error::resource_limit_exceeded("Missing error message"Too many task path collectionsMissing message")
            })?;
        }

        if let Some(paths) = self.task_paths.get_mut(&task_id) {
            paths.push(path).map_err(|_| {
                Error::resource_limit_exceeded("Missing error message"Too many paths for taskMissing message")
            })?;
        }
    ...
    pub fn validate_wcet_estimate(
        ...
        if let Some(result) = self.analysis_results.get(&task_id) {
            ...
            Err(Error::resource_not_found("Missing error message"No WCET analysis result found for taskMissing messageMissing messageMissing message")
        }
    }
    ...
    fn perform_measurement_analysis(&mut self, task_id: TaskId) -> Result<WcetAnalysisResult, Error> {
        let samples = self.execution_samples.get(&task_id)
            .ok_or_else(|| Error::resource_not_found("Missing error message"No execution samples available for measurement-based analysisMissing messageMissing messageMissing message"))?;
    ...
    fn perform_probabilistic_analysis(&mut self, task_id: TaskId) -> Result<WcetAnalysisResult, Error> {
        ...
        let samples = self.execution_samples.get(&task_id)
            .ok_or_else(|| Error::resource_not_found("Missing error messageMissing messageMissing messageMissing message"))?;
            ...
            for path in paths.iter_mut() {
                if path.path_id == path_id {
                    ...
                    if path.measured_samples.len() < MAX_EXECUTION_SAMPLES {
                        ...
                        path.measured_samples.push(fuel_consumed).map_err(|_| {
                            Error::resource_limit_exceeded("Missing error message"Failed to add path sampleMissing message")
                        })?;
                    }
                    break;
                }
    ...
    fn refine_wcet_estimate(&mut self, task_id: TaskId) -> Result<(), Error> {
        ...
        self.analysis_results.insert(task_id, refined_result).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Failed to update WCET analysis resultMissing message")
        })?;
