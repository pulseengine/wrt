    ...
    pub fn stream_new(&mut self, element_type: &ValType) -> WrtResult<StreamHandle> {
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            ...
            self.streams.push((handle, stream_enum)).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many streamsMissing message")
            })?;
        }
    ...
    pub fn stream_read(&mut self, stream_handle: StreamHandle) -> WrtResult<AsyncReadResult> {
        #[cfg(feature = "stdMissing message")]
        {
            if let Some(stream) = self.streams.get_mut(&stream_handle) {
                ...
                Err(wrt_error::Error::runtime_execution_error("Missing error message"Invalid stream handleMissing messageMissing messageMissing message")
            }
        }
        #[cfg(not(any(feature = "std", )))]
        {
                        ...
                        StreamValueEnum::Values(ref mut s) => {
                            if s.buffer.is_empty() {
                                ...
                                #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
                                {
                                    ...
                                    values.push(value).map_err(|_| wrt_error::Error::runtime_execution_error("Missing error message"Failed to push valueMissing messageMissing messageMissing message"))?;
    ...
    pub fn stream_write(&mut self, stream_handle: StreamHandle, values: &[Value]) -> WrtResult<()> {
        #[cfg(feature = "stdMissing message")]
        {
            if let Some(stream) = self.streams.get_mut(&stream_handle) {
                ...
                Err(wrt_error::Error::runtime_execution_error("Missing error message"Invalid handleMissing messageMissing messageMissing message")
            }
        }
        #[cfg(not(any(feature = "std", )))]
        {
            for (handle, stream) in &mut self.streams {
                if *handle == stream_handle {
                    return match stream {
                        StreamValueEnum::Values(ref mut s) => {
                            if s.writable_closed {
                                return Err(wrt_error::Error::runtime_execution_error("Missing error message"Stream is closedMissing messageMissing messageMissing message");
                            }
                            for value in values {
                                s.buffer.push(value.clone()).map_err(|_| {
                                    wrt_error::Error::resource_exhausted("Missing error message"Buffer fullMissing message")
                                })?;
                            }
    ...
    pub fn future_new(&mut self, value_type: &ValType) -> WrtResult<FutureHandle> {
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            ...
            self.futures.push((handle, future_enum)).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many futuresMissing message")
            })?;
        }
    ...
    pub fn error_context_new(&mut self, message: &str) -> WrtResult<ErrorContextHandle> {
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            self.error_contexts.push((handle, error_context)).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many error contextsMissing message")
            })?;
        }
    ...
    pub fn async_lift(
        ...
        let operation = AsyncOperation {
            ...
            context: BoundedVec::from_slice(values).map_err(|_| {
                Error::runtime_execution_error("Missing error message"Context too largeMissing message")
            })?,
            task_handle: None,
        };
    ...
    fn write(&mut self, values: &[Value]) -> WrtResult<()> {
        ...
        for value in values {
            if let Ok(typed_value) = T::try_from(value.clone()) {
                ...
                return Err(wrt_error::Error::type_mismatch_error("Missing error message"Value type mismatchMissing messageMissing messageMissing message");
            }
        }
    ...
    T: TryFrom<Value>,
{
    ...
    fn write(&mut self, value: &Value) -> WrtResult<()> {
        if let Ok(typed_value) = T::try_from(value.clone()) {
            ...
            Err(wrt_error::Error::type_mismatch_error("Missing error message"Value type mismatch for futureMissing messageMissing messageMissing message")
        }
    }
...
impl Default for AsyncCanonicalAbi {
    fn default() -> Self {
        Self::new().expect("Failed to create AsyncCanonicalAbiMissing message")
    }
}
...
impl fmt::Display for AsyncOperationType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            AsyncOperationType::AsyncCall => write!(f, "async-callMissing message"),
            AsyncOperationType::StreamRead => write!(f, "stream-readMissing message"),
            AsyncOperationType::StreamWrite => write!(f, "stream-writeMissing message"),
            AsyncOperationType::FutureGet => write!(f, "future-getMissing message"),
            AsyncOperationType::FutureSet => write!(f, "future-setMissing message"),
            AsyncOperationType::WaitablePoll => write!(f, "waitable-pollMissing message"),
        }
    }
}
...
impl fmt::Display for AsyncOperationState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            AsyncOperationState::Starting => write!(f, "startingMissing message"),
            AsyncOperationState::InProgress => write!(f, "in-progressMissing message"),
            AsyncOperationState::Waiting => write!(f, "waitingMissing message"),
            AsyncOperationState::Completed => write!(f, "completedMissing message"),
            AsyncOperationState::Cancelled => write!(f, "cancelledMissing message"),
            AsyncOperationState::Failed => write!(f, "failedMissing message"),
        }
    }
}
...
mod tests {
    ...
    fn test_stream_lifecycle() {
        ...
        match result {
            ...
            _ => panic!("Expected valuesMissing message"),
        }
    ...
    fn test_future_lifecycle() {
        ...
        // Write value
        let value = Value::String(BoundedString::from_str("helloMissing message").unwrap();
    ...
    fn test_error_context() {
        ...
        let handle = abi.error_context_new("Test errorMissing message").unwrap();
        let debug_string = abi.error_context_debug_string(handle).unwrap();
        assert!(debug_string.as_str().contains("Test errorMissing messageMissing messageMissing message");
    ...
    fn test_async_lift_immediate() {
        ...
        match abi.async_lift(&values, &types, &context).unwrap() {
            ...
            _ => panic!("Expected immediate resultMissing message"),
        }
    }
    ...
    fn test_async_lift_stream() {
        ...
        match abi.async_lift(&values, &types, &context).unwrap() {
            ...
            _ => panic!("Expected stream resultMissing message"),
        }
    }
    ...
    fn test_operation_state_display() {
        assert_eq!(AsyncOperationState::Starting.to_string(), "startingMissing message");
        assert_eq!(AsyncOperationType::AsyncCall.to_string(), "async-callMissing message");
        assert_eq!(AsyncOperationState::Completed.to_string(), "completedMissing message");
        assert_eq!(AsyncOperationType::StreamRead.to_string(), "stream-readMissing message");
    }
}
