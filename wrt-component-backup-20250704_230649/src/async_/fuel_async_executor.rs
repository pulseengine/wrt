...
#[cfg(not(feature = "component-model-threadingMissing messageMissing messageMissing message"))]
            ...
            ASILExecutionMode::D { .. } => {
                ...
                if self.limits.max_fuel_per_step.is_none() ||
                   ...
                   self.limits.max_execution_slice_ms.is_none() {
                    return Err(Error::configuration_error("Missing error message"ASIL-D requires all execution limits to be specifiedMissing messageMissing messageMissing message");
                }
            },
            ASILExecutionMode::C { .. } => {
                ...
                if self.limits.max_memory_usage.is_none() ||
                   self.limits.max_call_depth.is_none() {
                    return Err(Error::configuration_error("Missing error message"ASIL-C requires memory and call depth limits to be specifiedMissing messageMissing messageMissing message");
                }
            },
            _ => {
...
impl ExecutionLimitsConfig {
    ...
    pub fn from_binary_metadata(binary_hash: [u8; 32], custom_section_data: &[u8]) -> Result<Self, Error> {
        ...
        let resource_limits = ResourceLimitsSection::decode(custom_section_data)
            .map_err(|e| Error::runtime_execution_error("Missing error message", e),
            ))?;
...
impl FuelAsyncExecutor {
    ...
    pub fn unregister_component(&mut self, component_id: ComponentInstanceId) -> Result<(), Error> {
        if self.component_registry.remove(&component_id).is_none() {
            return Err(Error::runtime_execution_error("Missing error message", component_id),
            );
        }
        Ok(()
    }
        ...
        F: Future<Output = Result<(), Error>> + Send + 'static,
    {
        ...
        if self.fuel_enforcement.load(Ordering::Acquire) {
            ...
            if global_consumed + allocated_fuel > global_limit {
                return Err(Error::resource_limit_exceeded("Missing error message"Global fuel limit would be exceededMissing messageMissing messageMissing message");
            }
        }
        ...
        self.tasks.insert(task_id, task).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many concurrent async tasksMissing message")
        })?;

        // Add to ready queue
        self.ready_queue.lock()?.push(task_id).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Ready queue is fullMissing message")
        })?;
    ...
    fn execute_task_with_context(
        ...
        let task = self.tasks.get_mut(&task_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Task not found for executionMissing message")
        })?;
    ...
    fn execute_wasm_function(
        ...
        if task.execution_context.stack_depth >= task.execution_context.max_stack_depth {
            task.execution_context.stack_depth -= 1;
            return Err(Error::resource_limit_exceeded("Missing error message"Stack depth limit exceededMissing messageMissing messageMissing message");
        }
    ...
    fn execute_basic_step(
        ...
        if task.execution_context.stack_depth > 100 {
            return Err(Error::runtime_execution_error("Missing error message"Stack depth warningMissing messageMissing messageMissing message");
        }
    ...
    pub fn handle_component_async_operation(
        ...
        let task = self.tasks.get_mut(&task_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Task not foundMissing message")
        })?;
    ...
    fn restore_execution_state_from_yield_point(
        ...
        if let ASILExecutionMode::D { deterministic_execution: true, .. } = task.execution_context.asil_config.mode {
            ...
            if current_timestamp < yield_point.yield_timestamp {
                return Err(Error::runtime_execution_error("Missing error message"Deterministic execution violation during resumeMissing messageMissing messageMissing message");
            }
        }
    ...
    fn execute_fresh_function(
        ...
        match engine.execute_function_step(
            ...
            Err(e) => {
                // Execution error
                Err(Error::runtime_execution_error("Missing error messageMissing messageMissing messageMissing message")),
                )
            }
        }
    }
    ...
    fn resume_from_yield_point(
        ...
        match engine.continue_execution(max_instructions) {
            ...
            Err(e) => {
                // Execution error
                Err(Error::runtime_execution_error("Missing error message"WebAssembly execution resume failed: {}Missing messageMissing messageMissing message")),
                )
            }
        }
    }
    ...
    pub fn incur_fuel_debt(&mut self, task_id: TaskId, amount: u64) -> Result<(), Error> {
        let system = self.debt_credit_system.as_mut().ok_or_else(|| {
            Error::validation_invalid_state("Missing error message"Debt/credit system not enabledMissing message")
        })?;
...
impl Default for FuelAsyncExecutor {
    fn default() -> Self {
        Self::new().expect("Failed to create default FuelAsyncExecutorMissing message")
    }
}
...
impl FuelDebtCreditSystem {
    ...
    pub fn incur_debt(
        ...
        debts.insert(task_id, debt).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many task debtsMissing message")
        })?;
    ...
    pub fn grant_credit(
        ...
        credits.insert(component_id, credit).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many component creditsMissing message")
        })?;
...
impl ExecutorServices for FuelAsyncExecutor {
    ...
    fn register_task_waitables(&mut self, task_id: TaskId, waitables: Vec<WaitableHandle>) -> Result<(), Error> {
        ...
        if let Some(task) = self.tasks.get_mut(&task_id) {
            ...
            Err(Error::runtime_execution_error("Missing error message"Task {} not foundMissing message"),
            )
        }
    }
