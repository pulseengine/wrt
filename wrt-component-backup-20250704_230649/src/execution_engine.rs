...
impl CallFrame {
        ...
        Ok(Self {
            ...
            locals: {
                ...
                BoundedVec::new(provider).map_err(|_| {
                    wrt_error::Error::resource_exhausted("Missing error message"Failed to allocate locals vectorMissing message")
                })?
            },
            return_address: None,
        })
    ...
    pub fn push_local(&mut self, value: Value) -> WrtResult<()> {
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            self.locals.push(value).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many local variablesMissing message")
                )
            })
        }
    }
    ...
    pub fn set_local(&mut self, index: usize, value: Value) -> WrtResult<()> {
        if index < self.locals.len() {
            ...
            Err(wrt_error::Error::validation_invalid_input("Missing error message"Invalid inputMissing message")
            )
        }
    }
}
...
impl fmt::Display for ExecutionState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecutionState::Ready => write!(f, "ReadyMissing message"),
            ExecutionState::Running => write!(f, "RunningMissing message"),
            ExecutionState::Completed => write!(f, "CompletedMissing message"),
            ExecutionState::Failed => write!(f, "FailedMissing message"),
            ExecutionState::Suspended => write!(f, "SuspendedMissing message"),
        }
    }
}
...
impl ComponentExecutionEngine {
    ...
    pub fn new() -> WrtResult<Self> {
        Ok(Self {
            ...
            call_stack: {
                let provider = safe_managed_alloc!(65536, CrateId::Component)?;
                BoundedVec::new(provider).map_err(|_| {
                    wrt_error::Error::resource_exhausted("Missing error message"Failed to allocate call stackMissing message")
                })?
            },
            ...
            host_functions: {
                let provider = safe_managed_alloc!(65536, CrateId::Component)?;
                BoundedVec::new(provider).map_err(|_| {
                    wrt_error::Error::resource_exhausted("Missing error message"Failed to allocate host functionsMissing message")
                })?
            },
    ...
    pub fn call_function(
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            self.call_stack.push(frame).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Call stack overflowMissing messageMissing messageMissing message")
                )
            })?;
        }
    ...
    fn execute_function_internal(
        ...
        let instance_id = self.current_instance.ok_or_else(|| {
            wrt_error::Error::runtime_error("Missing error message"No current instance setMissing messageMissing messageMissing message")
            )
        })?;
        ...
        let result = self.runtime_bridge
            ...
            .map_err(|_| wrt_error::Error::runtime_error("Missing error message"Failed to execute component functionMissing messageMissing messageMissing message"))?;
    ...
    fn convert_values_to_component(&self, values: &[Value]) -> WrtResult<Vec<crate::canonical_abi::ComponentValue>> {
        ...
        let mut component_values = BoundedVec::new(provider).map_err(|_| {
            wrt_error::Error::resource_exhausted("Missing error message"Failed to allocate component values vectorMissing message")
        })?;
        for value in values {
            let component_value = self.convert_value_to_component(value)?;
            component_values.push(component_value).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many component valuesMissing message")
            })?;
        }
        Ok(component_values)
    }
    ...
    fn convert_value_to_component(&self, value: &Value) -> WrtResult<crate::canonical_abi::ComponentValue> {
        ...
        match value {
            ...
            _ => Err(wrt_error::Error::validation_invalid_input("Missing error message"Invalid inputMissing message")
            )),
        }
    }
    ...
    pub fn register_component_instance(
        ...
        self.runtime_bridge
            ...
            .map_err(|_| wrt_error::Error::runtime_error("Missing error message"Conversion errorMissing message")
            )
    }
    ...
    pub fn register_runtime_host_function(
        ...
        let signature = crate::component_instantiation::FunctionSignature {
            ...
            params: wrt_foundation::bounded::BoundedVec::from_slice(&[ComponentType::S32]).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many parametersMissing message")
                )
            })?,
            returns: wrt_foundation::bounded::BoundedVec::from_slice(&[ComponentType::S32]).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many return valuesMissing message")
                )
            })?,
        };
