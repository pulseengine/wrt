...
impl ResourceLimits {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.max_resource_types == 0 {
            return Err(Error::invalid_input("Missing error message"max_resource_types cannot be zeroMissing messageMissing messageMissing message");
        }
        if self.max_resources_per_instance == 0 {
            return Err(Error::invalid_input("Missing error message"max_resources_per_instance cannot be zeroMissing messageMissing messageMissing message");
        }
        if self.max_global_resources == 0 {
            return Err(Error::invalid_input("Missing error message"max_global_resources cannot be zeroMissing messageMissing messageMissing message");
        }
        Ok(()
    }
}
...
impl BoundedResourceManager {
    ...
    pub fn register_resource_type(
        ...
        if safety_level as u8 > self.safety_context.effective_asil() as u8 {
            return Err(Error::invalid_input("Missing error message"Resource safety level exceeds runtime safety levelMissing messageMissing messageMissing message");
        }
    ...
    pub fn create_resource(
        ...
        let resource_type = self.resource_types.iter()
            ...
            .ok_or(Error::invalid_input("Missing error message"Resource type not foundMissing messageMissing messageMissing message"))?;
    ...
    pub fn transfer_ownership(
        ...
        if resource.ownership != ResourceOwnership::Owned {
            return Err(Error::invalid_input("Missing error message"Cannot transfer non-owned resourceMissing messageMissing messageMissing message");
        }
    ...
    pub fn borrow_resource(
        ...
        if resource.state != ResourceState::Active {
            return Err(Error::invalid_input("Missing error message"Cannot borrow inactive resourceMissing messageMissing messageMissing message");
        }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        for resource in &self.global_resources {
            if !self.resource_types.iter().any(|rt| rt.id == resource.type_id) {
                return Err(Error::invalid_input("Missing error message"Resource type not foundMissing messageMissing messageMissing message");
            }
        }
