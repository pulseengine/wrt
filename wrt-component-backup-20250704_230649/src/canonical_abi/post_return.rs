...
impl fmt::Display for ResourceType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ResourceType::File => write!(f, "fileMissing message"),
            ResourceType::Network => write!(f, "networkMissing message"),
            ResourceType::Component => write!(f, "componentMissing message"),
            ResourceType::Stream => write!(f, "streamMissing message"),
            ResourceType::Future => write!(f, "futureMissing message"),
            ResourceType::Generic => write!(f, "genericMissing message"),
        }
    }
}
...
impl PostReturnContext {
    ...
    pub fn add_cleanup(&mut self, entry: PostReturnEntry) -> Result<()> {
        if self.is_executing {
            return Err(Error::runtime_invalid_state("Missing error message"Cannot add cleanup during post-return executionMissing message")
            );
        }
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            self.entries.push(entry).map_err(|_| {
                Error::resource_exhausted("Missing error message"Post-return cleanup queue fullMissing messageMissing messageMissing message")
                )
            })?;
        }
    ...
    pub fn execute_post_return(&mut self, instance: &mut Instance, memory: &mut Memory, options: &CanonicalOptions) -> Result<()> {
        if self.is_executing {
            return Err(Error::runtime_invalid_state("Missing error message"Post-return already executingMissing message")
            );
        }
    ...
    fn component_value_to_raw(&self, value: &ComponentValue) -> Result<Value> {
        match value {
            ...
            _ => Err(Error::runtime_execution_error("Missing error message"Unsupported ComponentValue type for post-return conversionMissing messageMissing messageMissing message")
        }
    }
