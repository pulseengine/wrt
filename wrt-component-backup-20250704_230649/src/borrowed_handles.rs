...
impl HandleLifetimeTracker {
    ...
    pub fn create_owned_handle<T>(
        ...
        self.owned_handles.push(entry).map_err(|_| {
            Error::resource_exhausted("Missing error message"Too many owned handlesMissing messageMissing messageMissing message")
            )
        })?;
    ...
    pub fn borrow_handle<T>(
        ...
        if source_entry.generation != source.generation {
            return Err(Error::runtime_execution_error("Missing error message"Stale handle generationMissing message")
            );
        }
        
        if source_entry.dropped {
            return Err(Error::runtime_execution_error("Missing error message"Cannot borrow dropped handleMissing message")
            );
        }
        
        // Validate scope
        if !self.is_scope_active(scope) {
            return Err(Error::runtime_execution_error("Missing error message"Lifetime scope is not activeMissing message")
            );
        }
        ...
        self.borrowed_handles.push(entry).map_err(|_| {
            Error::resource_exhausted("Missing error message"Too many borrowed handlesMissing messageMissing messageMissing message")
            )
        })?;
    ...
    pub fn create_scope(&mut self, component: ComponentId, task: TaskId) -> Result<LifetimeScope> {
        ...
        self.scope_stack.push(entry).map_err(|_| {
            Error::resource_exhausted("Missing error message"Scope stack overflowMissing messageMissing messageMissing message")
            )
        })?;
    ...
    pub fn end_scope(&mut self, scope: LifetimeScope) -> Result<()> {
        ...
        let scope_index = self.scope_stack
            ...
            .ok_or_else(|| {
                Error::runtime_execution_error("Missing error message"Scope not foundMissing messageMissing messageMissing message")
                )
            })?;
    ...
    fn find_owned_handle(&self, handle: u32) -> Result<&OwnedHandleEntry> {
        self.owned_handles
            ...
            .ok_or_else(|| {
                Error::runtime_execution_error("Missing error message"Owned handle not foundMissing message")
                )
            })
    }
    ...
    fn find_borrowed_handle(&self, borrow_id: BorrowId) -> Result<&BorrowedHandleEntry> {
        self.borrowed_handles
            ...
            .ok_or_else(|| {
                Error::runtime_execution_error("Missing error message"Borrowed handle not foundMissing message")
                )
            })
    }
    ...
    fn add_borrow_to_scope(&mut self, scope: LifetimeScope, borrow_id: BorrowId) -> Result<()> {
        let scope_entry = self.scope_stack
            ...
            .ok_or_else(|| {
                Error::runtime_execution_error("Missing error message"Scope not found or inactiveMissing messageMissing messageMissing message")
                )
            })?;
        
        scope_entry.borrows.push(borrow_id).map_err(|_| {
            Error::resource_exhausted("Missing error message"Too many borrows in scopeMissing messageMissing messageMissing message")
            )
        })?;
...
impl fmt::Display for BorrowValidation {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            BorrowValidation::Valid => write!(f, "validMissing message"),
            BorrowValidation::SourceNotFound => write!(f, "source not foundMissing message"),
            BorrowValidation::SourceDropped => write!(f, "source droppedMissing message"),
            BorrowValidation::GenerationMismatch => write!(f, "generation mismatchMissing message"),
            BorrowValidation::ScopeEnded => write!(f, "scope endedMissing message"),
            BorrowValidation::PermissionDenied => write!(f, "permission deniedMissing message"),
        }
    }
}
