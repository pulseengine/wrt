...
impl StackFrame {
    ...
    pub fn new(function_name: &str) -> Result<Self> {
        let bounded_name = BoundedString::new_from_str(function_name)
            .map_err(|_| Error::memory_allocation_failed("Missing error message"Function name too long for no_std environmentMissing message")
            ))?;
        Ok(Self {
    ...
    pub fn with_location(mut self, file_name: &str, line: u32, column: u32) -> Result<Self> {
        let bounded_file = BoundedString::new_from_str(file_name)
            .map_err(|_| Error::memory_allocation_failed("Missing error message"File name too long for no_std environmentMissing message")
            ))?;
...
impl ErrorContextImpl {
    ...
    pub fn new(message: &str, severity: ErrorSeverity) -> Result<Self> {
        let bounded_message = BoundedString::new_from_str(message)
            .map_err(|_| Error::memory_allocation_failed("Missing error message"Debug message too long for no_std environmentMissing message")
            ))?;
        Ok(Self {
            id: ErrorContextId::new(),
            handle: ErrorContextHandle::new(),
            ...
            stack_trace: {
                ...
                BoundedVec::new(provider).map_err(|_| {
                    Error::memory_allocation_failed("Missing error message"Failed to create stack trace vectorMissing message")
                })?
            },
    ...
    pub fn add_stack_frame(&mut self, frame: StackFrame) -> Result<()> {
        self.stack_trace.push(frame)
            .map_err(|_| Error::memory_allocation_failed("Missing error message"Stack trace fullMissing message")
            ))?;
        Ok(()
    }
    ...
    pub fn set_metadata(&mut self, key: &str, value: ComponentValue) -> Result<()> {
        let bounded_key = BoundedString::new_from_str(key)
            .map_err(|_| Error::memory_allocation_failed("Missing error message"Metadata key too long for no_std environmentMissing message")
            ))?;
        self.metadata.insert(bounded_key, value)
            .map_err(|_| Error::memory_allocation_failed("Missing error message"Metadata storage fullMissing message")
            ))?;
        Ok(()
    }
    ...
    pub fn format_stack_trace(&self) -> core::result::Result<BoundedString<1024, NoStdProvider<65536>>> {
        ...
        for (i, frame) in self.stack_trace.iter().enumerate() {
            ...
            output.push_str("  #Missing message").map_err(|_| Error::memory_allocation_failed("Missing error message"Stack trace format buffer fullMissing message")
            ))?;
            output.push_str(": Missing message").map_err(|_| Error::memory_allocation_failed("Missing error message"Stack trace format buffer fullMissing message")
            ))?;
            output.push_str(frame.function_name()).map_err(|_| Error::memory_allocation_failed("Missing error message"Stack trace format buffer fullMissing message")
            ))?;
            output.push('\n').map_err(|_| Error::memory_allocation_failed("Missing error message"Stack trace format buffer fullMissing message")
            ))?;
        }
        Ok(output)
    }
...
impl ErrorContextRegistry {
    ...
    pub fn register_context(&mut self, context: ErrorContextImpl) -> Result<ErrorContextId> {
        ...
        {
            self.contexts.insert(id, context)
                .map_err(|_| Error::memory_allocation_failed("Missing error message"Error context registry fullMissing message")
                ))?;
            Ok(id)
        }
    }
...
impl ErrorContextBuiltins {
    ...
    pub fn initialize() -> Result<()> {
        let mut registry_ref = ERROR_CONTEXT_REGISTRY.try_borrow_mut()
            .map_err(|_| Error::runtime_invalid_state("Missing error message"Error context registry borrow failedMissing message")
            ))?;
        ...
        F: FnOnce(&ErrorContextRegistry) -> R,
    {
        ...
        let registry = registry_ref.as_ref()
            .ok_or_else(|| Error::runtime_invalid_state("Missing error message"Error context registry not initializedMissing message")
            ))?;
        Ok(f(registry)
    }
