...
fn safe_read_lock<T>(lock: &RwLock<T>) -> Result<std::sync::RwLockReadGuard<T>> {
    lock.read().map_err(|_| wrt_error::Error::threading_error("Missing error message"Lock poisonedMissing messageMissing messageMissing message")
}
...
fn safe_write_lock<T>(lock: &RwLock<T>) -> Result<std::sync::RwLockWriteGuard<T>> {
    lock.write().map_err(|_| wrt_error::Error::threading_error("Missing error message"Lock poisonedMissing messageMissing messageMissing message")
}
...
fn safe_mutex_lock<T>(mutex: &Mutex<T>) -> Result<std::sync::MutexGuard<T>> {
    mutex.lock().map_err(|_| wrt_error::Error::threading_error("Missing error message"Mutex poisonedMissing messageMissing messageMissing message")
}
...
impl ThreadManager {
    ...
    pub fn join(&self, thread_id: ThreadId) -> Result<Vec<ComponentValue>> {
        ...
        let thread = threads.get_mut(&thread_id).ok_or_else(|| {
            Error::component_not_found("Missing error messageMissing messageMissing messageMissing message")
        })?;
        ...
        if thread.handle.is_none() {
            ...
            match state {
                ThreadState::Completed => {
                    ...
                    let result =
                        thread.result.read().unwrap().clone().ok_or_else(|| {
                            Error::threading_error("Missing error message"Thread result unavailableMissing message")
                        })?;
                    Ok(result)
                }
                ThreadState::Error => {
                ...
                ThreadState::Running => {
                    ...
                    Err(Error::threading_error("Missing error message"Thread is still running but handle is missingMissing messageMissing messageMissing message")
                }
            }
        } else {
            ...
            let handle = thread
                .handle
                .take()
                .ok_or_else(|| Error::threading_error("Missing error message"Thread handle unavailableMissing messageMissing messageMissing message"))?;
            ...
            match handle.join() {
                ...
                Err(_) => {
                    ...
                    *thread.error.write().unwrap() = Some("Thread panicked".to_string();
                    Err(Error::threading_error("Missing error message"Thread panicked during executionMissing messageMissing messageMissing message")
                }
            }
        }
    }
    ...
    pub fn is_thread_completed(&self, thread_id: ThreadId) -> Result<bool> {
        ...
        let thread = threads.get(&thread_id).ok_or_else(|| {
            Error::component_not_found("Missing error message"Component not foundMissing messageMissing messageMissing message")
        })?;
    ...
    pub fn lock_mutex(
        ...
        match primitives.get(&sync_id) {
            ...
            Some(_) => {
                Err(Error::component_not_found("Missing error message"Component not foundMissing messageMissing messageMissing message"))
            }
            None => Err(Error::component_not_found("Missing error message"Component not foundMissing messageMissing messageMissing message"))),
        }
    }
    ...
    pub fn wait_condvar(
        ...
        match primitives.get(&sync_id) {
            ...
            Some(_) => Err(Error::runtime_execution_error("Missing error message",
                sync_id
            )))),
            None => Err(Error::component_not_found("Missing error messageMissing messageMissing messageMissing message"))),
        }
    }
...
impl BuiltinHandler for ThreadingSpawnHandler {
    ...
    fn execute(&self, args: &[ComponentValue]) -> Result<Vec<ComponentValue>> {
        // Validate arguments
        if args.len() < 1 {
            return Err(Error::threading_error("Missing error message"threading.spawn requires at least 1 argumentMissing messageMissing messageMissing message");
        }
        ...
        let function_id = match args[0] {
            ...
            _ => {
                return Err(Error::threading_error("Missing error message"threading.spawn first argument must be a function IDMissing messageMissing messageMissing message");
            }
        };
        ...
        let thread_id =
            self.thread_manager
                .spawn(function_id, function_args, move |id, args| executor(id, args))?;
...
impl BuiltinHandler for ThreadingJoinHandler {
    ...
    fn execute(&self, args: &[ComponentValue]) -> Result<Vec<ComponentValue>> {
        // Validate arguments
        if args.len() != 1 {
            return Err(Error::threading_error("Missing error message"threading.join requires exactly 1 argumentMissing messageMissing messageMissing message");
        }
        ...
        let thread_id = match args[0] {
            ...
            _ => {
                return Err(Error::threading_error("Missing error message"threading.join argument must be a thread IDMissing messageMissing messageMissing message")
            }
        };
...
impl ThreadingSyncHandler {
    ...
    /// * `thread_manager` - Thread manager to use
    pub fn new(thread_manager: Arc<ThreadManager>) -> Self {
        Self { thread_manager, no_std_mode: AtomicBool::new(false) }
    }
}
...
impl BuiltinHandler for ThreadingSyncHandler {
    ...
    fn execute(&self, args: &[ComponentValue]) -> Result<Vec<ComponentValue>> {
        ...
        if self.no_std_mode.load(Ordering::Relaxed) {
            return Err(Error::threading_error("Missing error message"Threading is not supported in no_std modeMissing messageMissing messageMissing message");
        }

        // Validate arguments
        if args.len() < 1 {
            return Err(Error::threading_error("Missing error message"threading.sync requires at least 1 argumentMissing messageMissing messageMissing message");
        }
        ...
        let op_type = match &args[0] {
            ...
            _ => {
                return Err(Error::threading_error("Missing error message"threading.sync first argument must be a stringMissing messageMissing messageMissing message")
            }
        };
        ...
        match op_type {
            ...
            "lock-mutex" => {
                // Lock a mutex
                if args.len() < 2 {
                    return Err(Error::threading_error("Missing error message"lock-mutex requires a mutex IDMissing messageMissing messageMissing message");
                }

                let mutex_id = match args[1] {
                    ComponentValue::U64(id) => id,
                    _ => {
                        return Err(Error::threading_error("Missing error message"lock-mutex requires a mutex ID as second argumentMissing messageMissing messageMissing message");
                    }
                };
            ...
            "wait-condvar" => {
                // Wait on a condition variable
                if args.len() < 2 {
                    return Err(Error::threading_error("Missing error message"wait-condvar requires a condvar IDMissing messageMissing messageMissing message");
                }

                let condvar_id = match args[1] {
                    ComponentValue::U64(id) => id,
                    _ => {
                        return Err(Error::threading_error("Missing error message"wait-condvar requires a condvar ID as second argumentMissing messageMissing messageMissing message");
                    }
                };
            ...
            "signal-condvar" => {
                // Signal a condition variable
                if args.len() < 2 {
                    return Err(Error::threading_error("Missing error message"signal-condvar requires a condvar IDMissing messageMissing messageMissing message");
                }

                let condvar_id = match args[1] {
                    ComponentValue::U64(id) => id,
                    _ => {
                        return Err(Error::threading_error("Missing error message"signal-condvar requires a condvar ID as second argumentMissing messageMissing messageMissing message");
                    }
                };
            ...
            "read-rwlock" => {
                ...
                if args.len() < 2 {
                    return Err(Error::threading_error("Missing error message"read-rwlock requires a rwlock IDMissing messageMissing messageMissing message");
                }

                let rwlock_id = match args[1] {
                    ComponentValue::U64(id) => id,
                    _ => {
                        return Err(Error::threading_error("Missing error message"read-rwlock requires a rwlock ID as second argumentMissing messageMissing messageMissing message");
                    }
                };
            ...
            "write-rwlock" => {
                ...
                if args.len() < 2 {
                    return Err(Error::threading_error("Missing error message"write-rwlock requires a rwlock IDMissing messageMissing messageMissing message");
                }

                let rwlock_id = match args[1] {
                    ComponentValue::U64(id) => id,
                    _ => {
                        return Err(Error::threading_error("Missing error message"write-rwlock requires a rwlock ID as second argumentMissing messageMissing messageMissing message");
                    }
                };
...
mod tests {
    ...
    fn test_executor(function_id: u32, args: Vec<ComponentValue>) -> Result<Vec<ComponentValue>> {
        match function_id {
            ...
            3 => Err(Error::runtime_execution_error("Missing error message"Test errorMissing messageMissing messageMissing message")),

            // Unknown function
            _ => Err(Error::component_not_found("Missing error message"Component not foundMissing messageMissing messageMissing message"))),
        }
    }
    ...
    fn test_thread_manager_error() {
        ...
        assert_eq!(result.unwrap_err().to_string(), "Test errorMissing message");
    }
