...
#[cfg(all(feature = "component-model-async", feature = "stdMissing messageMissing messageMissing message"))]
use std::{
    boxed::Box,
    collections::HashMap,
    sync::{Arc, Mutex},
...
#[cfg(all(feature = "component-model-async", not(feature = "stdMissing messageMissing messageMissing message")))]
use alloc::{boxed::Box, vec::Vec};

#[cfg(all(feature = "component-model-async", not(feature = "stdMissing messageMissing messageMissing message")))]
use wrt_foundation::{bounded::BoundedVec, safe_memory::NoStdProvider};

#[cfg(feature = "component-model-asyncMissing message")]
            ...
            Some(async_value) => {
                if async_value.status == AsyncStatus::Ready {
                    async_value.result.clone().ok_or_else(|| {
                        Error::async_error("Missing error message"Async result not availableMissing messageMissing messageMissing message")
                    })
                    ...
                    Err(Error::async_error("Missing error message"Async operation still pendingMissing messageMissing messageMissing message"))
                }
            }
            None => Err(Error::component_not_found("Missing error message"Component not foundMissing messageMissing messageMissing message"))),
        }
