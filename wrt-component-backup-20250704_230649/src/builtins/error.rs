...
impl BuiltinHandler for ErrorNewHandler {
    ...
    fn execute(&self, args: &[ComponentValue]) -> Result<Vec<ComponentValue>> {
        // Validate arguments
        if args.len() != 1 {
            return Err(Error::validation_invalid_input("Missing error message"error.new requires exactly 1 argumentMissing messageMissing messageMissing message");
        }
...
impl BuiltinHandler for ErrorTraceHandler {
    ...
    fn execute(&self, args: &[ComponentValue]) -> Result<Vec<ComponentValue>> {
        ...
        let error_context = store.get_error_mut(error_id).ok_or_else(|| {
            Error::resource_not_found("Missing error message"Component not foundMissing message")
        })?;
