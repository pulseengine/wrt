...
impl ComponentMemoryManager {
    ...
    pub fn new() -> WrtResult<Self> {
        Ok(Self {
            ...
            memories: {
                ...
                BoundedVec::new(provider).map_err(|_| wrt_error::Error::resource_exhausted("Missing error message"Failed to create memory managerMissing messageMissing messageMissing message"))?
            },
            #[cfg(feature = "stdMissing message")]
            ...
            sharing_policies: {
                let provider = safe_managed_alloc!(65536, CrateId::Component)?;
                BoundedVec::new(provider).map_err(|_| wrt_error::Error::resource_exhausted("Missing error message"Failed to create sharing policiesMissing messageMissing messageMissing message"))?
            },
            total_allocated: 0,
    ...
    pub fn create_memory(
        ...
        if self.total_allocated + initial_size > self.max_memory {
            return Err(wrt_error::Error::resource_exhausted("Missing error message"Memory limit exceededMissing message")
            );
        }
        ...
        let mut data = BoundedVec::new(provider).map_err(|_| {
            wrt_error::Error::resource_exhausted("Missing error message"Failed to allocate memory dataMissing message")
        })?;
        #[cfg(not(any(feature = "std", )))]
        {
            for _ in 0..initial_size {
                data.push(0u8).map_err(|_| {
                    wrt_error::Error::resource_exhausted("Missing error message"Memory allocation failedMissing messageMissing messageMissing message")
                    )
                })?;
            }
        }
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            self.memories.push(memory).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many memoriesMissing messageMissing messageMissing message")
                )
            })?;
        }
    ...
    pub fn read_memory(
        ...
        let memory = self
            ...
            .ok_or_else(|| wrt_error::Error::validation_invalid_input("Missing error message"Invalid inputMissing message")
            ))?;

        // Check permissions
        if !self.check_read_permission(memory_id, instance_id)? {
            return Err(wrt_error::Error::runtime_error("Missing error message"Read permission deniedMissing message")
            );
        }
        ...
        if end_offset > memory.data.len() {
            return Err(wrt_error::Error::validation_invalid_input("Missing error message"Invalid inputMissing message")
            );
        }
    ...
    pub fn write_memory(
        ...
        if !self.check_write_permission(memory_id, instance_id)? {
            return Ok(MemoryAccess {
                success: false,
                bytes_accessed: 0,
                error: Some(BoundedString::from_str("Write permission deniedMissing message").unwrap_or_default()),
            });
        }
        ...
        if end_offset > memory.data.len() {
            return Ok(MemoryAccess {
                ...
                bytes_accessed: 0,
                error: Some(
                    BoundedString::from_str("Memory access out of boundsMissing message").unwrap_or_default(),
                ),
            });
        }
    ...
    pub fn grow_memory(
        ...
        // Check permissions
        if !self.check_write_permission(memory_id, instance_id)? {
            return Err(wrt_error::Error::runtime_error("Missing error message"Write permission deniedMissing message")
            );
        }
    ...
    pub fn set_sharing_policy(&mut self, policy: MemorySharingPolicy) -> WrtResult<()> {
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            self.sharing_policies.push(policy).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many sharing policiesMissing message")
                )
            })
        }
    }
...
impl ComponentTableManager {
    ...
    pub fn new() -> WrtResult<Self> {
        Ok(Self {
            ...
            tables: {
                let provider = safe_managed_alloc!(65536, CrateId::Component)?;
                BoundedVec::new(provider).map_err(|_| wrt_error::Error::resource_exhausted("Missing error message"Failed to create table managerMissing messageMissing messageMissing message"))?
            },
            #[cfg(feature = "stdMissing message")]
            sharing_policies: Vec::new(),
            #[cfg(not(any(feature = "std", )))]
    ...
    pub fn create_table(
        ...
        let mut elements = BoundedVec::new(provider).map_err(|_| {
            wrt_error::Error::resource_exhausted("Missing error message"Failed to allocate table elementsMissing message")
        })?;
        #[cfg(not(any(feature = "std", )))]
        {
            for _ in 0..limits.min {
                elements.push(TableElement::Null).map_err(|_| {
                    wrt_error::Error::resource_exhausted("Missing error message"Table allocation failedMissing messageMissing messageMissing message")
                    )
                })?;
            }
        }
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            self.tables.push(table).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many tablesMissing messageMissing messageMissing message")
                )
            })?;
        }
    ...
    pub fn get_element(&self, table_id: u32, index: u32) -> WrtResult<&TableElement> {
        ...
        table.elements.get(index as usize).ok_or_else(|| {
            wrt_error::Error::validation_invalid_input("Missing error message"Invalid inputMissing message")
            )
        })
    }
...
impl Default for ComponentMemoryManager {
    fn default() -> Self {
        ...
        Self::new().expect("Failed to create default ComponentMemoryManagerMissing message")
    }
}
...
impl Default for ComponentTableManager {
    fn default() -> Self {
        ...
        Self::new().expect("Failed to create default ComponentTableManagerMissing message")
    }
}
...
impl fmt::Display for SharingMode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            SharingMode::Private => write!(f, "privateMissing message"),
            SharingMode::ReadOnly => write!(f, "readonlyMissing message"),
            SharingMode::ReadWrite => write!(f, "readwriteMissing message"),
            SharingMode::CopyOnWrite => write!(f, "copyonwriteMissing message"),
        }
    }
}
...
mod tests {
    ...
    fn test_table_access() {
        ...
        match (retrieved, &element) {
            ...
            _ => panic!("Element mismatchMissing message"),
        }
    }

    #[test]
    fn test_sharing_mode_display() {
        assert_eq!(SharingMode::Private.to_string(), "privateMissing message");
        assert_eq!(SharingMode::ReadOnly.to_string(), "readonlyMissing message");
        assert_eq!(SharingMode::ReadWrite.to_string(), "readwriteMissing message");
        assert_eq!(SharingMode::CopyOnWrite.to_string(), "copyonwriteMissing message");
    }
