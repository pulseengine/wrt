    ...
    fn execute_synchronous_call(&mut self, frame: UnifiedCallFrame, args: &[Value]) -> WrtResult<Value> {
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            self.core_state.call_stack.push(frame).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Call stack overflowMissing message")
            })?;
        }
        ...
        let result = self.core_state.runtime_bridge
            ...
            .map_err(|_| wrt_error::Error::runtime_error("Missing error message"Component operation resultMissing messageMissing messageMissing message"))?;
    ...
    fn execute_async_call(&mut self, frame: UnifiedCallFrame, args: &[Value]) -> WrtResult<Value> {
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            self.async_state.executions.push(async_execution).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many async executionsMissing message")
            })?;
        }
...
impl fmt::Display for UnifiedExecutionState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            UnifiedExecutionState::Ready => write!(f, "ReadyMissing message"),
            UnifiedExecutionState::Running => write!(f, "RunningMissing message"),
            UnifiedExecutionState::Waiting => write!(f, "WaitingMissing message"),
            UnifiedExecutionState::Suspended => write!(f, "SuspendedMissing message"),
            UnifiedExecutionState::Completed => write!(f, "CompletedMissing message"),
            UnifiedExecutionState::Failed => write!(f, "FailedMissing message"),
            UnifiedExecutionState::Cancelled => write!(f, "CancelledMissing message"),
        }
    }
}
