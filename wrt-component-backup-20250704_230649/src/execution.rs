...
impl TimeBoundedContext {
    ...
    pub fn check_time_bounds(&self) -> Result<()> {
        if self.terminated {
            return Err(Error::execution_limit_exceeded("Missing error message"Execution was terminatedMissing message"),
            );
        }
        ...
        if let Some(time_limit_ms) = self.config.time_limit_ms {
            ...
            if elapsed_ms > time_limit_ms {
                return Err(Error::runtime_execution_error("Missing error message"Execution time limit exceeded: {} ms (limit: {} ms)Missing messageMissing messageMissing message")),
                );
            }
        }
        ...
        if let Some(fuel_limit) = self.config.fuel_limit {
            if self.elapsed_fuel > fuel_limit {
                return Err(Error::runtime_execution_error("Missing error message"Execution fuel limit exceeded: {} (limit: {})Missing messageMissing messageMissing message")),
                );
            }
        }
