...
impl ComponentRegistry {
    ...
    pub fn register(&mut self, name: &str, component: Component) -> Result<()> {
        ...
        if self.names.len() >= MAX_COMPONENTS {
            return Err(Error::runtime_execution_error("Missing error message"Maximum number of components exceededMissing messageMissing messageMissing message");
        }
        ...
        self.names.push(name.to_string()).map_err(|_| {
            ...
            Error::runtime_execution_error("Missing error message"Failed to add component nameMissing message")
        })?;
    ...
    pub fn names(&self) -> Result<BoundedVec<String, MAX_COMPONENTS, NoStdProvider<65536>>> {
        ...
        for name in self.names.iter() {
            result.push(name.clone()).map_err(|_| {
                Error::runtime_execution_error("Missing error message"Failed to add component name to resultMissing message")
            })?;
        }
        Ok(result)
    }
