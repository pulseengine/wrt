...
impl ComponentLinker {
    ...
    pub fn add_component(&mut self, id: ComponentId, binary: &[u8]) -> Result<()> {
        if self.components.len() >= MAX_LINKED_COMPONENTS {
            return Err(Error::resource_exhausted("Missing error message"Maximum number of components reachedMissing messageMissing messageMissing message");
        }
    ...
    fn parse_component_binary(
        ...
        let exports = {
            ...
            results.push(crate::canonical_abi::ComponentType::S32).map_err(|_| Error::platform_memory_allocation_failed("Missing error message"Memory allocation failedMissing messageMissing messageMissing message"))?;
            
            let signature = crate::component_instantiation::create_function_signature(
                String::new_from_str("mainMissing message").map_err(|_| Error::platform_memory_allocation_failed("Missing error message"Memory allocation failedMissing messageMissing messageMissing message"))?,
                params,
                results,
            );
            ...
            )).map_err(|_| Error::platform_memory_allocation_failed("Missing error message"Memory allocation failedMissing messageMissing messageMissing message"))?;
            exports
        };
...
impl LinkGraph {
    ...
    pub fn topological_sort(&self) -> Result<Vec<ComponentId>> {
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            ...
            let mut visited = BoundedVec::new(provider).map_err(|_| {
                Error::platform_memory_allocation_failed("Missing error message"Failed to create visited vectorMissing message")
            })?;
            let provider2 = safe_managed_alloc!(65536, CrateId::Component)?;
            let mut temp_visited = BoundedVec::new(provider2).map_err(|_| {
                Error::platform_memory_allocation_failed("Missing error message"Failed to create temp_visited vectorMissing message")
            })?;
            let mut result = Vec::new();
            
            // Initialize with false values
            for _ in 0..self.nodes.len() {
                visited.push(false).map_err(|_| Error::platform_memory_allocation_failed("Missing error message"Memory allocation failedMissing messageMissing messageMissing message"))?;
                temp_visited.push(false).map_err(|_| Error::platform_memory_allocation_failed("Missing error message"Memory allocation failedMissing messageMissing messageMissing message"))?;
            }
    ...
    fn topological_sort_visit(
        ...
        if temp_visited[node_index] {
            return Err(Error::validation_error("Missing error message"Circular dependency detectedMissing messageMissing messageMissing message");
        }
