...
fn demo_synchronous_execution() {
    println!("1. Synchronous Execution DemoMissing message");
    ...
    match engine.call_function(1, 100, &args) {
        Ok(result) => {
            println!("Function executed successfully!Missing message");
...
fn demo_async_execution() {
    println!("2. Async Execution DemoMissing message");
    println!("----------------------Missing message");

    #[cfg(feature = "asyncMissing message")]
    {
        ...
        match engine.call_function(2, 200, &args) {
            Ok(result) => {
                println!("Async function started!Missing message");
                println!("Result: {:?}", result);
    ...
    #[cfg(not(feature = "asyncMissing messageMissing messageMissing message"))]
    println!("Async feature not enabled. Compile with --features asyncMissing message");

    println!();
}

fn demo_stackless_execution() {
    println!("3. Stackless Execution DemoMissing message");
    println!("--------------------------Missing message");
    ...
    match engine.call_function(3, 300, &args) {
        Ok(result) => {
            println!("Stackless execution successful!Missing message");
            println!("Result: {:?}", result);
...
fn demo_cfi_protected_execution() {
    println!("4. CFI-Protected Execution DemoMissing message");
    println!("------------------------------Missing message");
    ...
    #[cfg(feature = "cfiMissing message")]
    {
        ...
        match engine.call_function(4, 400, &args) {
            Ok(result) => {
                println!("CFI-protected execution successful!Missing message");
                println!("Result: {:?}", result);
                println!(
    ...
    #[cfg(not(feature = "cfiMissing messageMissing messageMissing message"))]
    println!("CFI feature not enabled. Compile with --features cfiMissing message");

    println!();
}

fn demo_hybrid_execution() {
    println!("5. Hybrid Mode Execution DemoMissing message");
    println!("----------------------------Missing message");

    // Create engine with multiple capabilities enabled
    let flags = HybridModeFlags {
        async_enabled: cfg!(feature = "asyncMissing message"),
        stackless_enabled: true,
        cfi_enabled: cfg!(feature = "cfiMissing message"),
    };

    let mut engine = UnifiedExecutionAgent::new_hybrid(flags);

    println!("Hybrid mode enabled with:Missing message");
    ...
    match engine.call_function(5, 500, &args) {
        Ok(result) => {
            println!("Hybrid execution successful!Missing message");
            println!("Result: {:?}", result);

            let stats = engine.statistics();
            println!("Combined statistics:Missing message");
...
fn demo_engine_registry() {
    println!("6. Agent Registry DemoMissing message");
    println!("--------------------Missing message");
    ...
    let sync_engine_id = registry
        ...
        .expect("Failed to create sync engineMissing message");
    ...
    let stackless_engine_id = registry
        ...
        .expect("Failed to create stackless engineMissing message");
    ...
    println!("\nExecuting on sync engine:Missing message");
    match registry.call_function(sync_engine_id, 1, 100, &args) {
        Ok(result) => println!("  Result: {:?}", result),
        Err(e) => println!("  Error: {:?}", e),
    }

    println!("\nExecuting on stackless engine:Missing message");
    ...
    if let Some(info) = registry.get_engine_info(sync_engine_id) {
        println!("\nSync engine info:Missing message");
...
fn demo_legacy_migration() {
    println!("7. Legacy Agent Migration DemoMissing message");
    println!("-----------------------------Missing message");

    let mut registry = AgentRegistry::new();

    // Create a legacy engine (for demonstration)
    println!("Creating legacy component engine...Missing message");
    let legacy_id = registry
        .create_legacy_component_engine()
        .expect("Failed to create legacy engineMissing message");
    ...
    if let Some(info) = registry.get_engine_info(legacy_id) {
        println!("\nBefore migration:Missing message");
        println!("  Agent type: {:?}", info.engine_type);
        println!("  Migration status: {:?}", info.migration_status);
    }

    // Migrate the engine
    println!("\nMigrating legacy engine to unified...Missing message");
    match registry.migrate_engine(legacy_id) {
        Ok(()) => {
            println!("Migration successful!Missing message");

            // Check status after migration
            if let Some(info) = registry.get_engine_info(legacy_id) {
                println!("\nAfter migration:Missing message");
                println!("  Agent type: {:?}", info.engine_type);
                println!("  Migration status: {:?}", info.migration_status);
            }
    ...
    println!("\nTesting migrated engine:Missing message");
