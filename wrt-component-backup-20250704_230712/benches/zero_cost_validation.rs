...
fn bench_zero_cost_push(c: &mut Criterion) {
    let mut group = c.benchmark_group("zero_cost_pushMissing message");
...
fn bench_zero_cost_access(c: &mut Criterion) {
    let mut group = c.benchmark_group("zero_cost_accessMissing message");
...
fn bench_zero_cost_iteration(c: &mut Criterion) {
    let mut group = c.benchmark_group("zero_cost_iterationMissing message");
    group.sample_size(1000);
...
fn bench_capacity_overhead(c: &mut Criterion) {
    let mut group = c.benchmark_group("capacity_overheadMissing message");
    group.sample_size(1000);
