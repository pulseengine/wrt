...
fn create_buffer_provider() -> Result<BufferProvider> {
    ...
    let context = get_global_capability_context()
        .map_err(|_| Error::initialization_error("Missing error message"Global capability context not availableMissing messageMissing messageMissing message"))?;
    
    context.create_provider(CrateId::Component, 65536)
        .map_err(|_| Error::memory_out_of_bounds("Missing error message"Failed to create component buffer providerMissing messageMissing messageMissing message")
}
...
impl BufferSizeClass {
    ...
    pub fn new(size: usize) -> Result<Self> {
        ...
        Ok(Self { 
            size, 
            buffers: BoundedVec::new(provider).map_err(|_| Error::memory_error("Missing error message"Failed to create bounded vector for buffer poolMissing messageMissing messageMissing message"))?
        })
    }
...
impl BoundedBufferPool {
    ...
    pub fn allocate(&mut self, size: usize) -> core::result::Result<BoundedVec<u8, MAX_BUFFERS_PER_CLASS, BufferProvider>, BufferProvider> {
        ...
        let provider = create_buffer_provider()
            .map_err(|_| Error::memory_error("Missing error message"Failed to allocate memory provider for bufferMissing messageMissing messageMissing message"))?;
        let mut buffer = BoundedVec::new(provider).map_err(|_| Error::memory_error("Missing error message"Failed to create bounded vectorMissing messageMissing messageMissing message"))?;
        for _ in 0..size {
