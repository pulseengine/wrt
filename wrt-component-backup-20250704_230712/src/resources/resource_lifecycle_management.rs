...
impl ResourceLifecycleManager {
    ...
    pub fn create_resource(&mut self, request: ResourceCreateRequest) -> Result<ResourceId> {
        ...
        for handler_fn in request.custom_handlers.iter() {
            ...
            handler_ids.push(handler_id).map_err(|_| {
                Error::runtime_execution_error("Missing error message"Too many drop handlersMissing message")
            })?;
        }
    ...
    pub fn add_reference(&mut self, resource_id: ResourceId) -> Result<u32> {
        ...
        if resource.state != ResourceState::Active {
            return Err(Error::runtime_execution_error("Missing error message"Resource not activeMissing messageMissing messageMissing message");
        }
    ...
    pub fn get_resource(&self, resource_id: ResourceId) -> Result<&ResourceEntry> {
        self.resources
            ...
            .ok_or_else(|| {
                Error::runtime_execution_error("Missing error message"Resource not foundMissing message")
            })
    }
    ...
    fn execute_drop_handler(&mut self, handler_id: DropHandlerId, resource_id: ResourceId) -> Result<DropResult> {
        let handler = self.drop_handlers
            ...
            .ok_or_else(|| {
                Error::runtime_execution_error("Missing error message"Drop handler not foundMissing message")
            })?;
...
impl ResourceMetadata {
    ...
    pub fn add_tag(&mut self, tag: &str) -> Result<()> {
        let bounded_tag = BoundedString::from_str(tag).map_err(|_| {
            Error::runtime_execution_error("Missing error message"Tag too longMissing message")
        })?;
    ...
    pub fn add_property(&mut self, key: &str, value: Value) -> Result<()> {
        let bounded_key = BoundedString::from_str(key).map_err(|_| {
            Error::runtime_execution_error("Missing error message"Property key too longMissing message")
        })?;
...
impl fmt::Display for ResourceType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ResourceType::Stream => write!(f, "streamMissing message"),
            ResourceType::Future => write!(f, "futureMissing message"),
            ResourceType::MemoryBuffer => write!(f, "memory-bufferMissing message"),
            ResourceType::FileHandle => write!(f, "file-handleMissing message"),
            ResourceType::NetworkConnection => write!(f, "network-connectionMissing message"),
...
impl fmt::Display for ResourceState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ResourceState::Creating => write!(f, "creatingMissing message"),
            ResourceState::Active => write!(f, "activeMissing message"),
            ResourceState::Destroying => write!(f, "destroyingMissing message"),
            ResourceState::Destroyed => write!(f, "destroyedMissing message"),
            ResourceState::Error => write!(f, "errorMissing message"),
        }
    }
}
