...
//! let file_type = manager.register_resource_type("fileMissing message")?;
//!
...
#[cfg(all(not(feature = "stdMissing messageMissing messageMissing message")))]
...
impl ResourceTable {
    ...
    pub fn create_resource(
        ...
        if self.resources.len() >= MAX_RESOURCES_PER_INSTANCE {
            return Err(Error::resource_exhausted("Missing error message"Maximum resources per instance exceededMissing messageMissing messageMissing message");
        }
    ...
    pub fn drop_resource(&mut self, handle: ResourceHandle) -> Result<()> {
        let resource = self.resources.remove(&handle).ok_or_else(|| {
            Error::resource_not_found("Missing error message"Resource handle not foundMissing message")
        })?;
    ...
    pub fn borrow_resource(&mut self, handle: ResourceHandle, borrower: InstanceId) -> Result<()> {
        ...
        match resource.state {
            ...
            _ => Err(Error::runtime_invalid_state("Missing error message"Resource not in a borrowable stateMissing message"),
            )),
        }
    }
    ...
    pub fn return_resource(&mut self, handle: ResourceHandle) -> Result<()> {
        ...
        match resource.state {
            ...
            _ => Err(Error::runtime_invalid_state("Missing error message"Resource is not borrowedMissing message"),
            )),
        }
    }
...
impl ResourceManager {
    ...
    pub fn register_resource_type(
        ...
        if self.resource_types.len() >= MAX_RESOURCE_TYPES {
            return Err(Error::resource_exhausted("Missing error message"Maximum resource types exceededMissing messageMissing messageMissing message");
        }
    ...
    pub fn remove_instance_table(&mut self, instance_id: InstanceId) -> Result<()> {
        if let Some(mut table) = self.instance_tables.remove(&instance_id) {
            ...
            Err(Error::instance_not_found("Missing error messageMissing messageMissing messageMissing message")
        }
    }
    ...
    pub fn create_resource(
        ...
        let table = self.instance_tables.get_mut(&instance_id).ok_or_else(|| {
            Error::instance_not_found("Missing error messageMissing message")
        })?;
    ...
    pub fn transfer_ownership(
        ...
        let resource = source_table.resources.remove(&handle).ok_or_else(|| {
            Error::resource_not_found("Missing error message"Resource not found in source instanceMissing message")
        })?;

        // Add to target table
        let target_table = self.instance_tables.get_mut(&to_instance).ok_or_else(|| {
            Error::instance_not_found("Missing error message"Target instance not foundMissing message")
        })?;
    ...
    pub fn borrow_resource(
        ...
        let resource = owner_table.get_resource(handle).ok_or_else(|| {
            Error::resource_not_found("Missing error message"Resource not foundMissing message")
        })?;

        let resource_type = self.get_resource_type(resource.resource_type).ok_or_else(|| {
            Error::type_not_found("Missing error message"Resource type not foundMissing message")
        })?;
    ...
    pub fn return_borrowed_resource(
        ...
        let borrower_table = self.instance_tables.get_mut(&borrower_instance).ok_or_else(|| {
            Error::instance_not_found("Missing error message"Borrower instance not foundMissing message")
        })?;

        let borrowed_resource = borrower_table.get_resource(borrowed_handle).ok_or_else(|| {
            Error::resource_not_found("Missing error message"Borrowed resource not foundMissing message")
        })?;
        ...
        let (owner_instance, owner_handle) = match borrowed_resource.ownership {
            ...
            _ => {
                return Err(Error::runtime_invalid_state("Missing error message"Resource is not borrowedMissing message"),
                )
            }
        };
        ...
        let owner_table = self.instance_tables.get_mut(&owner_instance).ok_or_else(|| {
            Error::instance_not_found("Missing error message"Owner instance not foundMissing message")
        })?;
    ...
    fn validate_table(&self, table: &ResourceTable) -> Result<()> {
        for resource in table.resources.values() {
            ...
            if !self.resource_types.contains_key(&resource.resource_type) {
                return Err(Error::validation_error("Missing error message"Resource references unknown typeMissing messageMissing messageMissing message");
            }
            ...
            match resource.ownership {
                ResourceOwnership::Owned => {
                    if resource.owner_instance != table.instance_id {
                        return Err(Error::validation_error("Missing error message"Owned resource has incorrect ownerMissing messageMissing messageMissing message");
                    }
                }
                ResourceOwnership::Borrowed { owner, .. } => {
                    if !self.instance_tables.contains_key(&owner) {
                        return Err(Error::validation_error("Missing error message"Borrowed resource references unknown ownerMissing messageMissing messageMissing message");
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_resource_type_registration() {
        ...
        assert_eq!(resource_type.name, "fileMissing message");
