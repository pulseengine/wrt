...
impl ResourceLifecycleManager {
    ...
    pub fn register_type(
        ...
        let resource_type = ResourceType {
            ...
            name: BoundedString::try_from(name).map_err(|_| {
                Error::resource_error("Missing error message"Resource type name too longMissing message")
            })?,
            destructor,
        };

        self.types.insert(type_idx, resource_type).map_err(|_| {
            Error::resource_error("Missing error message"Failed to register resource typeMissing message")
        })?;
    ...
    pub fn create_resource(
        ...
        let resource_type = self
            ...
            .get(&type_idx)
            .ok_or_else(|| {
                Error::resource_error("Missing error message"Component not foundMissing message")
            })?
            .clone();
        ...
        let resource_type = self
            ...
            .get(&type_idx)
            .map_err(|_| {
                Error::resource_error("Missing error message"Failed to get resource typeMissing message")
            })?
            .ok_or_else(|| {
                Error::resource_error("Missing error message"Unknown resource typeMissing message")
            })?
            .clone();
        ...
        self.resources.insert(handle, resource).map_err(|_| {
            Error::resource_error("Missing error message"Failed to store resourceMissing message")
        })?;
    ...
    pub fn drop_resource(&mut self, handle: ResourceHandle) -> Result<()> {
        ...
        let mut resource = self.resources.remove(&handle).ok_or_else(|| {
            Error::resource_invalid_handle("Missing error message"Component not foundMissing message"),
            )
        })?;
        ...
        let mut resource = self
            ...
            .remove(&handle)
            .map_err(|_| {
                Error::resource_error("Missing error message"Failed to remove resourceMissing message")
            })?
            .ok_or_else(|| {
                Error::resource_invalid_handle("Missing error message"Invalid resource handleMissing message"),
                )
            })?;

        // Check state
        if resource.state == ResourceState::Dropped {
            return Err(Error::resource_error("Missing error message"Resource already droppedMissing messageMissing messageMissing message");
        }

        // Check borrows
        if resource.borrow_count > 0 {
            return Err(Error::resource_error("Missing error message"Cannot drop resource with active borrowsMissing messageMissing messageMissing message");
        }
    ...
    pub fn borrow_resource(
        ...
        let resource = self
            ...
            .get_mut(&handle)
            .map_err(|_| {
                Error::resource_error("Missing error message"Failed to get resourceMissing message")
            })?
            .ok_or_else(|| {
                Error::resource_invalid_handle("Missing error message"Invalid resource handleMissing message"),
                )
        ...
        if resource.state != ResourceState::Active && resource.state != ResourceState::Borrowed {
            return Err(Error::resource_error("Missing error message"Resource not available for borrowingMissing messageMissing messageMissing message");
        }

        // Check mutable borrow rules
        if is_mutable && resource.borrow_count > 0 {
            return Err(Error::resource_error("Missing error message"Cannot create mutable borrow with existing borrowsMissing messageMissing messageMissing message");
        }
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            let borrows =
                self.borrows.get_mut_or_insert(handle, BoundedVec::new).map_err(|_| {
                    Error::resource_error("Missing error message"Failed to store borrow infoMissing message")
                })?;
            borrows.push(borrow_info).map_err(|_| {
                Error::resource_error("Missing error message"Too many borrows for resourceMissing message")
            })?;
        }
    ...
    pub fn release_borrow(&mut self, handle: ResourceHandle, borrower: u32) -> Result<()> {
        ...
        let borrow_info = {
            let borrows = self.borrows.get_mut(&handle).ok_or_else(|| {
                Error::resource_error("Missing error message"No borrows for resourceMissing message")
            })?;

            let pos = borrows.iter().position(|b| b.borrower == borrower).ok_or_else(|| {
                Error::resource_error("Missing error message"Borrow not found for borrowerMissing message")
            })?;
        ...
        let borrow_info = {
            let borrows = self
                ...
                .get_mut(&handle)
                .map_err(|_| {
                    Error::resource_error("Missing error message"Failed to get borrowsMissing message")
                })?
                .ok_or_else(|| {
                    Error::resource_error("Missing error message"No borrows for resourceMissing message")
                })?;
    ...
    pub fn transfer_ownership(&mut self, handle: ResourceHandle, from: u32, to: u32) -> Result<()> {
        ...
        if resource.metadata.owner != from {
            return Err(Error::resource_error("Missing error message"Not the owner of the resourceMissing messageMissing messageMissing message");
        }

        // Check state
        if resource.state != ResourceState::Active {
            return Err(Error::resource_error("Missing error message"Resource not in transferable stateMissing messageMissing messageMissing message");
        }

        // Check borrows
        if resource.borrow_count > 0 {
            return Err(Error::resource_error("Missing error message"Cannot transfer resource with active borrowsMissing messageMissing messageMissing message");
        }
