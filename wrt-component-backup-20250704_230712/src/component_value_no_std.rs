...
pub fn serialize_component_value_no_std(
    ...
    let mut buffer = BoundedVec::new(provider).map_err(|_| {
        Error::capacity_exceeded("Missing error message"Failed to create serialization bufferMissing message")
    })?;

    match value {
        ComponentValue::Bool(b) => {
            buffer.push(if *b { 1 } else { 0 }).map_err(|_| {
                Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing BoolMissing message")
            })?;
        }
        ComponentValue::S8(v) => {
            buffer.push(*v as u8).map_err(|_| {
                Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing S8Missing message")
            })?;
        }
        ComponentValue::U8(v) => {
            buffer.push(*v).map_err(|_| {
                Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing U8Missing message")
            })?;
        }
        ComponentValue::S16(v) => {
            for byte in v.to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing S16Missing message")
                })?;
            }
        }
        ComponentValue::U16(v) => {
            for byte in v.to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing U16Missing message")
                })?;
            }
        }
        ComponentValue::S32(v) => {
            for byte in v.to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing S32Missing message")
                })?;
            }
        }
        ComponentValue::U32(v) => {
            for byte in v.to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing U32Missing message")
                })?;
            }
        }
        ComponentValue::S64(v) => {
            for byte in v.to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing S64Missing message")
                })?;
            }
        }
        ComponentValue::U64(v) => {
            for byte in v.to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing U64Missing message")
                })?;
            }
        }
        ComponentValue::F32(v) => {
            for byte in v.to_bits().to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing F32Missing message")
                })?;
            }
        }
        ComponentValue::F64(v) => {
            for byte in v.to_bits().to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing F64Missing message")
                })?;
            }
        }
        ComponentValue::Char(c) => {
            ...
            for byte in bytes {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing CharMissing message")
                })?;
            }
        }
        ComponentValue::String(s) => {
            ...
            for byte in len.to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing String lengthMissing message")
                })?;
            }
            ...
            for byte in s.as_bytes() {
                buffer.push(*byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing String contentMissing message")
                })?;
            }
        }
        ComponentValue::List(items) => {
            ...
            for byte in len.to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing List lengthMissing message")
                })?;
            }
            ...
            for item in items {
                ...
                for byte in item_size.to_le_bytes() {
                    buffer.push(byte).map_err(|_| {
                        Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing List item sizeMissing message")
                    })?;
                }
                ...
                for byte in item_data.iter() {
                    buffer.push(*byte).map_err(|_| {
                        Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing List item dataMissing message")
                    })?;
                }
            }
        }
        ComponentValue::Record(fields) => {
            ...
            for byte in field_count.to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Record field countMissing message")
                })?;
            }
            ...
            for (name, value) in fields {
                ...
                for byte in name_len.to_le_bytes() {
                    buffer.push(byte).map_err(|_| {
                        Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Record field name lengthMissing message")
                    })?;
                }
                ...
                for byte in name.as_bytes() {
                    buffer.push(*byte).map_err(|_| {
                        Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Record field name contentMissing message")
                    })?;
                }
                ...
                for byte in value_data.iter() {
                    buffer.push(*byte).map_err(|_| {
                        Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Record field valueMissing message")
                    })?;
                }
            }
        }
        ComponentValue::Tuple(items) => {
            ...
            for byte in len.to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Tuple lengthMissing message")
                })?;
            }
            ...
            for item in items {
                ...
                for byte in item_data.iter() {
                    buffer.push(*byte).map_err(|_| {
                        Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Tuple item dataMissing message")
                    })?;
                }
            }
        }
        ComponentValue::Variant(case, value) => {
            ...
            for byte in (*case as u32).to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Variant discriminantMissing message")
                })?;
            }
            ...
            if let Some(val) = value {
                ...
                for byte in value_data.iter() {
                    buffer.push(*byte).map_err(|_| {
                        Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Variant valueMissing message")
                    })?;
                }
            }
        }
        ComponentValue::Enum(case) => {
            ...
            for byte in (*case as u32).to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Enum discriminantMissing message")
                })?;
            }
        }
        ComponentValue::Option(value) => {
            ...
            if let Some(val) = value {
                buffer.push(1).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Option presence flagMissing message")
                })?;
                ...
                for byte in value_data.iter() {
                    buffer.push(*byte).map_err(|_| {
                        Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Option valueMissing message")
                    })?;
                }
            } else {
        ...
        ComponentValue::Result(result) => {
            match result {
                Ok(val) => {
                    ...
                    buffer.push(1).map_err(|_| {
                        Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Result success flagMissing message")
                    })?;
                    ...
                    if let Some(v) = val {
                        // Push presence flag as u8
                        buffer.push(1).map_err(|_| {
                            Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Result ok presence flagMissing message")
                        })?;
                        ...
                        for byte in value_data.iter() {
                            buffer.push(*byte).map_err(|_| {
                                Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Result ok valueMissing message")
                            })?;
                        }
                    } else {
                ...
                Err(val) => {
                    ...
                    buffer.push(0).map_err(|_| {
                        Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Result error flagMissing message")
                    })?;
                    ...
                    if let Some(v) = val {
                        // Push presence flag as u8
                        buffer.push(1).map_err(|_| {
                            Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Result err presence \
                                 flagMissing message")
                        })?;
                        ...
                        for byte in value_data.iter() {
                            buffer.push(*byte).map_err(|_| {
                                Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Result err valueMissing message")
                            })?;
                        }
                    } else {
                        // Push absence flag as u8
        ...
        ComponentValue::Flags(flags) => {
            ...
            for byte in (num_bytes as u32).to_le_bytes() {
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Flags byte countMissing message")
                })?;
            }
            ...
            for byte_idx in 0..num_bytes {
                ...
                buffer.push(byte).map_err(|_| {
                    Error::capacity_exceeded("Missing error message"Buffer capacity exceeded when serializing Flags bitsMissing message")
                })?;
            }
        }
        ComponentValue::U32(v) => {
...
pub fn convert_valtype_to_format<P: MemoryProvider + Default + Clone + PartialEq + Eq>(
    ...
    match val_type {
        ...
        _ => Err(Error::type_conversion_error("Missing error message"Component not foundMissing messageMissing messageMissing message")),
    }
}
