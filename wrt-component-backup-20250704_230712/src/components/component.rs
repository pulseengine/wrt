...
impl RuntimeInstance {
    ...
    pub fn register_function(&mut self, name: String, function: ExternValue) -> Result<()> {
        if let ExternValue::Function(_) = &function {
            ...
            Err(Error::validation_error("Missing error message"Component not found",
            )
        }
    }
    ...
    pub fn execute_function(&self, name: &str, args: Vec<Value>) -> Result<Vec<Value>> {
        ...
        let function = self.functions.get(name).ok_or_else(|| {
            Error::runtime_function_not_found("Missing error message"Component not foundMissing message")
        })?;
...
impl MemoryValue {
    ...
    pub fn read(&self, offset: u32, size: u32) -> Result<Vec<u8>> {
        let memory = self.memory.read().map_err(|e| {
            Error::memory_error("Missing error message"Component not found",
            )
        })?;
...
impl Host {
    ...
    pub fn call_function(&self, name: &str, args: &[Value]) -> Result<Vec<Value>> {
        ...
        let function = self.get_function(name).ok_or_else(|| {
            Error::component_not_found("Missing error message"Component not foundMissing message")
        })?;

        // Validate arguments
        if args.len() != function.ty.params.len() {
            return Err(Error::validation_error("Missing error message"Component not found",
            );
        }
