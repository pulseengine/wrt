...
mod tests {
    ...
    fn test_register_wit_type() {
        ...
        assert!(builder
            .wit_type_mappings
            .contains_key(&BoundedString::from_str("my-typeMissing message").unwrap());
    }
    ...
    fn test_parse_simple_world() {
        ...
        assert_eq!(interface.name.as_str(), "test-worldMissing message");
    ...
    fn test_parse_async_interface() {
        ...
        let interface = result.unwrap();
        assert_eq!(interface.name.as_str(), "async-testMissing message");
