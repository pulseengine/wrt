...
impl FuelPriorityInheritanceProtocol {
    ...
    pub fn register_blocking(
        ...
        self.blocking_relationships.insert(blocked_task, blocking_info).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many blocking relationships trackedMissing message")
        })?;
    ...
    pub fn initiate_priority_inheritance(
        ...
        let chain = match self.inheritance_chains.get_mut(&resource_id) {
            Some(existing_chain) => {
                ...
                existing_chain.waiters.push(blocked_task).map_err(|_| {
                    Error::resource_limit_exceeded("Missing error message"Inheritance chain too longMissing message")
                })?;
            ...
            None => {
                ...
                waiters.push(blocked_task).map_err(|_| {
                    Error::resource_limit_exceeded("Missing error message"Failed to add waiter to new chainMissing message")
                })?;
                ...
                self.inheritance_chains.insert(resource_id, new_chain).map_err(|_| {
                    Error::resource_limit_exceeded("Missing error message"Too many inheritance chainsMissing message")
                })?;
        ...
        if !self.original_priorities.contains_key(&holder_task) {
            ...
            self.original_priorities.insert(holder_task, Priority::Normal).map_err(|_| {
                Error::resource_limit_exceeded("Missing error message"Too many original priorities trackedMissing message")
            })?;
        }
        ...
        self.priority_donations.insert(blocked_task, donation).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many priority donations trackedMissing message")
        })?;
...
impl Default for FuelPriorityInheritanceProtocol {
    fn default() -> Self {
        Self::new(VerificationLevel::Standard).expect("Failed to create default priority inheritance protocolMissing message")
    }
}
