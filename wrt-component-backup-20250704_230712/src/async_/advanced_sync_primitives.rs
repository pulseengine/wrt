...
impl AdvancedSyncPrimitives {
    ...
    pub fn initialize_component_sync(
        ...
        self.component_contexts.insert(component_id, context).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many component sync contextsMissing message")
        })?;
    ...
    pub fn create_async_mutex(
        ...
        let context = self.component_contexts.get_mut(&component_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Component not initialized for sync operationsMissing message")
        })?;
        ...
        if current_mutexes >= context.sync_limits.max_mutexes {
            return Err(Error::resource_limit_exceeded("Missing error message"Component mutex limit exceededMissing messageMissing messageMissing message");
        }
        ...
        self.primitives.insert(primitive_id, primitive).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many sync primitivesMissing message")
        })?;

        context.owned_primitives.push(primitive_id).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Component primitive list fullMissing message")
        })?;
    ...
    pub fn create_async_semaphore(
        ...
        if permits == 0 {
            return Err(Error::validation_invalid_input("Missing error message"Semaphore must have at least 1 permitMissing messageMissing messageMissing message");
        }
    ...
    pub fn create_async_barrier(
        ...
        if parties == 0 {
            return Err(Error::validation_invalid_input("Missing error message"Barrier must have at least 1 partyMissing messageMissing messageMissing message");
        }
    ...
    pub fn create_async_condvar(
        ...
        if let Some(mutex_id) = associated_mutex {
            let mutex = self.primitives.get(&mutex_id).ok_or_else(|| {
                Error::validation_invalid_input("Missing error message"Associated mutex not foundMissing message")
            })?;

            if !matches!(mutex.primitive_type, SyncPrimitiveType::AsyncMutex { .. }) {
                return Err(Error::validation_invalid_input("Missing error message"Associated primitive is not a mutexMissing messageMissing messageMissing message");
            }
        }
    ...
    pub fn lock_async_mutex(
        ...
        let primitive = self.primitives.get_mut(&primitive_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Mutex not foundMissing message")
        })?;
        ...
        match &mut primitive.primitive_type {
            SyncPrimitiveType::AsyncMutex { locked, owner, lock_count, is_reentrant } => {
                ...
                if locked.load(Ordering::Acquire) {
                    ...
                    primitive.waiters.push(waiter).map_err(|_| {
                        Error::resource_limit_exceeded("Missing error message"Mutex waiter queue fullMissing message")
                    })?;
            ...
            _ => Err(Error::validation_invalid_input("Missing error message"Primitive is not a mutexMissing messageMissing messageMissing message")),
        }
    }
        ...
        match &mut primitive.primitive_type {
            SyncPrimitiveType::AsyncMutex { locked, owner, lock_count, is_reentrant } => {
                ...
                if owner.as_ref() != Some(&task_id) {
                    return Err(Error::invalid_state_error("Missing error message"Task does not own mutexMissing messageMissing messageMissing message");
                }

                let current_count = lock_count.load(Ordering::Acquire);
                if current_count == 0 {
                    return Err(Error::invalid_state_error("Missing error message"Mutex is not lockedMissing messageMissing messageMissing message");
                }
    ...
    pub fn acquire_semaphore(
        ...
        let primitive = self.primitives.get_mut(&primitive_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Semaphore not foundMissing message")
        })?;
        ...
        match &mut primitive.primitive_type {
            SyncPrimitiveType::AsyncSemaphore { permits, fair_scheduling, .. } => {
                ...
                if current_permits == 0 {
                    ...
                    primitive.waiters.push(waiter).map_err(|_| {
                        Error::resource_limit_exceeded("Missing error message"Semaphore waiter queue fullMissing message")
                    })?;
            ...
            _ => Err(Error::validation_invalid_input("Missing error message"Primitive is not a semaphoreMissing messageMissing messageMissing message")),
        }
    }
        ...
        match &mut primitive.primitive_type {
            SyncPrimitiveType::AsyncSemaphore { permits, max_permits, .. } => {
                ...
                if current_permits >= *max_permits {
                    return Err(Error::invalid_state_error("Missing error message"Semaphore already at maximum permitsMissing messageMissing messageMissing message");
                }
    ...
    pub fn wait_barrier(
        ...
        let primitive = self.primitives.get_mut(&primitive_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Barrier not foundMissing message")
        })?;

        match &mut primitive.primitive_type {
            SyncPrimitiveType::AsyncBarrier { parties, waiting, generation, broken } => {
                if broken.load(Ordering::Acquire) {
                    return Err(Error::invalid_state_error("Missing error message"Barrier is brokenMissing messageMissing messageMissing message");
                }
                ...
                if current_waiting + 1 == *parties {
                    ...
                    primitive.waiters.push(waiter).map_err(|_| {
                        Error::resource_limit_exceeded("Missing error message"Barrier waiter queue fullMissing message")
                    })?;

                    Ok(BarrierWaitResult::WouldBlock)
                }
            },
            _ => Err(Error::validation_invalid_input("Missing error message"Primitive is not a barrierMissing messageMissing messageMissing message")),
        }
    }
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        if let Some(sync_primitives) = self.sync_primitives.upgrade() {
            if let Ok(mut primitives) = sync_primitives.lock() {
                ...
                Poll::Ready(Err(Error::invalid_state_error("Missing error message"Sync primitives manager unavailableMissing messageMissing messageMissing message"))
            }
        } else {
            Poll::Ready(Err(Error::invalid_state_error("Missing error message"Sync primitives manager droppedMissing messageMissing messageMissing message"))
        }
    }
}
