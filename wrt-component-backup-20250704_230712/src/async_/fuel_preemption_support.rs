...
use core::{
    sync::atomic::{AtomicBool, AtomicU64, AtomicU32, Ordering},
    time::Duration,
};
use wrt_foundation::{
...
impl FuelPreemptionManager {
    ...
    pub fn register_task(
        ...
        self.task_states.insert(task_id, state).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many tasks registered for preemptionMissing message")
        })?;
    ...
    pub fn check_preemption(
        ...
        let state = self.task_states.get(&current_task).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Task not registered for preemptionMissing message")
        })?;
        ...
        match self.preemption_policy {
            ...
            PreemptionPolicy::PriorityBased => self.check_priority_preemption(current_task, state, executor),
            PreemptionPolicy::DeadlineDriven => self.check_deadline_preemption(current_task, state, executor),
    ...
    pub fn add_preemption_point(
        ...
        let points = self.preemption_points.get_mut(&task_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Task not registeredMissing message")
        })?;
        ...
        points.push(point).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many preemption pointsMissing message")
        })?;
...
pub trait PreemptibleExecutor {
    ...
    fn restore_preemption_state(&mut self, task_id: TaskId, checkpoint: StateCheckpoint) -> Result<(), Error>;
}
