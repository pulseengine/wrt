...
impl<T> FuelStream<T> {
    ...
    pub fn yield_item(&mut self, item: T) -> Result<()> {
        ...
        if self.state != StreamState::Active && self.state != StreamState::Waiting {
            return Err(Error::async_error("Missing error message"Cannot yield to inactive streamMissing messageMissing messageMissing message");
        }
    ...
    fn consume_fuel(&mut self, base_cost: u64) -> Result<()> {
        ...
        if self.fuel_consumed.saturating_add(total_cost) > self.fuel_budget {
            return Err(Error::resource_limit_exceeded("Missing error message"Stream fuel budget exceededMissing messageMissing messageMissing message");
        }
        ...
        if self.metadata.is_bounded {
            if let Some(max_items) = self.metadata.max_items {
                if self.value_stream.buffer.len() >= max_items {
                    return Err(Error::resource_limit_exceeded("Missing error message"Stream buffer limit exceededMissing messageMissing messageMissing message");
                }
            }
        }
...
impl FuelStreamManager {
    ...
    pub fn create_stream(
        ...
        if self.total_fuel_consumed.saturating_add(fuel_budget) > self.global_fuel_budget {
            return Err(Error::resource_limit_exceeded("Missing error message"Global stream fuel budget exceededMissing messageMissing messageMissing message");
        }
...
mod tests {
    ...
    fn test_stream_yield_and_poll() {
        ...
        match stream.poll_next(&mut cx) {
            Poll::Ready(Some(42)) => {},
            _ => panic!(Missing messageMissing messageMissing message"))Missing message"),
        }
        
        match stream.poll_next(&mut cx) {
            Poll::Ready(Some(43)) => {},
            _ => panic!("Expected Ready(Some(43))Missing message"),
        }
        
        // No more items
        match stream.poll_next(&mut cx) {
            Poll::Pending => {},
            _ => panic!("Expected PendingMissing message"),
        }
    }
