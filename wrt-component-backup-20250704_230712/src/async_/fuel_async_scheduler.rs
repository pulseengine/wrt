...
impl FuelAsyncScheduler {
    ...
    pub fn add_task(
        ...
        self.scheduled_tasks.insert(task_id, scheduled_task).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many scheduled tasksMissing message")
        })?;
        ...
        match self.policy {
            ...
            SchedulingPolicy::RoundRobin => {
                self.round_robin_queue.push(task_id).map_err(|_| {
                    Error::resource_limit_exceeded("Missing error message"Round-robin queue is fullMissing message")
                })?;
            }
        }
    ...
    fn insert_deadline_queue(&mut self, task_id: TaskId) -> Result<(), Error> {
        ...
        self.priority_queue.push(task_id).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Deadline queue is fullMissing message")
        })?;
