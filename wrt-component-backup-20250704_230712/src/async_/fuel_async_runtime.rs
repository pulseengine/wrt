...
impl FuelAsyncRuntime {
    ...
    pub fn spawn_task(
        ...
        if self.state != RuntimeState::Running && self.state != RuntimeState::Initializing {
            return Err(Error::async_executor_state_violation("Missing error message"Runtime not in running stateMissing messageMissing messageMissing message");
        }
        ...
        let component = self.components.get(&component_id).ok_or_else(|| {
            Error::component_resource_lifecycle_error("Missing error message"Component not registeredMissing message")
            )
        })?.clone();
    ...
    pub fn run(&mut self) -> Result<RuntimeExecutionSummary> {
        ...
        while self.has_active_tasks()? {
            ...
            if self.total_fuel_consumed >= self.global_fuel_budget {
                ...
                return Err(Error::async_fuel_exhausted("Missing error message"Global runtime fuel budget exhaustedMissing messageMissing messageMissing message");
            }
    ...
    pub fn get_task_result(&self, task_id: TaskId) -> Result<TaskResult> {
        if let Some(result) = self.task_results.get(&task_id) {
            ...
            Err(Error::async_task_execution_failed("Missing error message"Task result not availableMissing messageMissing messageMissing message")
        }
    }
    ...
    fn resolve_function_export(
        ...
        if !component.has_function_export(function_name) {
            return Err(Error::component_instantiation_runtime_error("Missing error message"Function not found in component exportsMissing messageMissing messageMissing message");
        }
        ...
        match component.get_function_index(function_name) {
            Some(function_index) => {
                ...
                if function_index >= 65536 {
                    return Err(Error::component_instantiation_runtime_error("Missing error message"Function index out of rangeMissing messageMissing messageMissing message");
                }
                Ok(function_index)
            },
            None => Err(Error::component_instantiation_runtime_error("Missing error message"Could not resolve function indexMissing messageMissing messageMissing message")),
        }
    }
    ...
    fn consume_runtime_fuel(&mut self, amount: u64) -> Result<()> {
        ...
        if self.total_fuel_consumed.saturating_add(total_cost) > self.global_fuel_budget {
            return Err(Error::async_fuel_exhausted("Missing error message"Runtime fuel budget exceededMissing messageMissing messageMissing message");
        }
...
impl SimpleAsyncExecutor {
    ...
    pub fn execute_async_function(
        ...
        match self.runtime.run_until_task_complete(task_id)? {
            ...
            TaskResult::Cancelled => Err(Error::async_task_execution_failed("Missing error message"Task was cancelledMissing messageMissing messageMissing message")),
        }
    }
}
