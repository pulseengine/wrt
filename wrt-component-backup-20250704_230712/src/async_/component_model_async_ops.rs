...
impl ComponentModelAsyncOps {
    ...
    pub fn task_wait(
        ...
        if waitables.waitables.is_empty() {
            return Err(Error::validation_invalid_input("Missing error message"Empty waitable setMissing messageMissing messageMissing message");
        }

        if waitables.waitables.len() > MAX_WAITABLES {
            return Err(Error::runtime_execution_error("Missing error message", waitables.waitables.len(), MAX_WAITABLES),
            );
        }
        ...
        if let Some(task) = executor.tasks.get_mut(&current_task) {
            match task.state {
                ...
                _ => {
                    ...
                    return Err(Error::invalid_state_error("Missing error message"Task not in ready stateMissing messageMissing messageMissing message");
                }
            }
        } else {
            return Err(Error::validation_invalid_input("Missing error message"Task not foundMissing messageMissing messageMissing message");
        }
