        ...
        T: Send + 'static,
    {
        ...
        let (result, outcome) = run_with_time_bounds(time_config, |time_context| {
            ...
            self.active_bridges.insert(task_id, bridge_context).map_err(|_| {
                Error::resource_limit_exceeded("Missing error message"Too many active async bridgesMissing message")
            })?;
        ...
        match outcome {
            ...
            TimeBoundedOutcome::TimedOut => Err(Error::runtime_execution_error("Missing error message".to_string(),
            )),
            ...
            TimeBoundedOutcome::Error(e) => Err(Error::runtime_execution_error("Missing error message"Async function execution error: {}Missing message"),
            )),
        }
    }
                ...
                Some(status) => {
                    match status.state {
                        ...
                        AsyncTaskState::Failed => {
                            results.push(Err(Error::runtime_execution_error("Missing error message"Async task failedMissing messageMissing messageMissing message"));
                        }
                        _ => {
                            results.push(Err(Error::runtime_execution_error("Missing error message".to_string(),
                            ));
                        }
                    }
                }
        ...
        loop {
            ...
            if let Some(status) = self.executor.get_task_status(task_id) {
                match status.state {
                    ...
                    AsyncTaskState::Failed => {
                        self.cleanup_bridge(task_id)?;
                        return Err(Error::runtime_execution_error("Missing error message"Async task failed during executionMissing messageMissing messageMissing message");
                    }
