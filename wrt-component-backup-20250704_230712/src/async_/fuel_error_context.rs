...
impl<T> ErrorContextExt<T> for Result<T> {
    fn context(self, context: &str) -> Result<T> {
        self.map_err(|e| {
            Error::runtime_execution_error("Missing error message"{}: {}Missing message"),
            )
        })
    }
...
mod tests {
    ...
    fn test_contextual_error() {
        let error = Error::async_async_error("Missing error message"Test errorMissing message");
    ...
    fn test_error_propagator() {
        ...
        let error = Error::component_error("Missing error message"Component errorMissing message");
