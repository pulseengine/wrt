...
impl<T> FuelHandleTable<T> {
    ...
    pub fn allocate(&mut self, data: T) -> Result<GenerationalHandle> {
        ...
        if !self.check_fuel(HANDLE_ALLOCATE_FUEL)? {
            return Err(Error::resource_limit_exceeded("Missing error message"Handle table fuel budget exceededMissing messageMissing messageMissing message");
        }
        ...
        let index = if let Some(index) = self.free_list.pop() {
            ...
            if self.entries.len() >= MAX_HANDLES_PER_TABLE {
                return Err(Error::resource_limit_exceeded("Missing error message"Handle table capacity exceededMissing messageMissing messageMissing message");
            }
        ...
        if let Some(entry) = self.entries.get_mut(index as usize) {
            ...
            return Err(Error::resource_error("Missing error message"Failed to update handle entryMissing messageMissing messageMissing message");
        }
    ...
    pub fn lookup(&self, handle: GenerationalHandle) -> Result<&T> {
        ...
        let entry = self.entries.get(handle.index as usize).ok_or_else(|| {
            ...
            Error::resource_not_found("Missing error message"Invalid handle indexMissing message")
        })?;
        ...
        let data = entry.data.as_ref().ok_or_else(|| {
            Error::resource_not_found("Missing error message"Handle data not foundMissing message")
        })?;
    ...
    pub fn lookup_mut(&mut self, handle: GenerationalHandle) -> Result<&mut T> {
        ...
        let data = entry.data.as_mut().ok_or_else(|| {
            Error::resource_not_found("Missing error messageMissing message")
        })?;
...
impl HandleTableManager {
    ...
    pub fn get_table<T: Send + Sync + 'static>(
        ...
        let table = self.tables.get(&table_id).ok_or_else(|| {
            Error::resource_not_found("Missing error message"Handle table not foundMissing message")
        })?;
        
        // Downcast to specific type
        table.downcast_ref::<FuelHandleTable<T>>().ok_or_else(|| {
            Error::type_error("Missing error message"Handle table type mismatchMissing message")
        })
    }
...
mod tests {
    ...
    fn test_handle_allocation() {
        ...
        let handle = table.allocate("test_dataMissing message").unwrap();
        assert_eq!(handle.index, 0);
        assert_eq!(handle.generation, 1);
        
        let data = table.lookup(handle).unwrap();
        assert_eq!(*data, "test_dataMissing message");
    }
    ...
    fn test_handle_table_manager() {
        ...
        
        {
            ...
            assert_eq!(table.lookup(handle).unwrap(), "testMissing message");
        }
