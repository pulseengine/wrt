...
impl ResourceAsyncOperations {
    ...
    pub fn initialize_component_resources(
        ...
        self.resource_contexts.insert(component_id, context).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many component resource contextsMissing message")
        })?;
    ...
    pub fn async_create_resource(
        ...
        let context = self.resource_contexts.get_mut(&component_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Component not initialized for async resourcesMissing message")
        })?;
        ...
        if context.owned_resources.len() >= context.resource_limits.max_owned_resources {
            return Err(Error::resource_limit_exceeded("Missing error messageMissing messageMissing messageMissing message");
        }
        ...
        self.active_operations.insert(operation_id, stored_operation).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many active resource operationsMissing message")
        })?;

        // Add to component context
        context.active_operations.push(operation_id).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Component operation list fullMissing message")
        })?;
    ...
    pub fn async_call_resource_method(
        ...
        let context = self.resource_contexts.get_mut(&component_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Component not initializedMissing message")
        })?;
        ...
        let resource_info = context.owned_resources.get(&resource_handle)
            .or_else(|| {
                context.borrowed_resources.get(&resource_handle)
                    .map(|borrow| &ResourceInfo {
                        ...
                        resource_type: ResourceType::new(Missing messageMissing messageMissing message")),
            ...
            .ok_or_else(|| {
                Error::validation_invalid_input("Missing error message"Resource not owned or borrowed by componentMissing message")
            })?;

        if resource_info.state != ResourceState::Active && resource_info.state != ResourceState::Borrowed {
            return Err(Error::validation_invalid_state("Missing error message"Resource not in valid state for method callsMissing messageMissing messageMissing message");
        }
        ...
        self.active_operations.insert(operation_id, stored_operation).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many active operationsMissing message")
        })?;
    ...
    pub fn async_borrow_resource(
        ...
        if context.borrowed_resources.len() >= context.resource_limits.max_borrowed_resources {
            return Err(Error::resource_limit_exceeded("Missing error message"Component borrow limit exceededMissing messageMissing messageMissing message");
        }
        ...
        context.borrowed_resources.insert(resource_handle, borrow_info).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Borrow table fullMissing message")
        })?;

        self.active_operations.insert(operation_id, operation).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many operationsMissing message")
        })?;
    ...
    pub fn async_drop_resource(
        ...
        let resource_info = context.owned_resources.get_mut(&resource_handle).ok_or_else(|| {
            Error::validation_invalid_input("Missing error messageMissing message")
        })?;

        if resource_info.state == ResourceState::Dropped {
            return Err(Error::validation_invalid_state("Missing error message"Resource already droppedMissing messageMissing messageMissing message");
        }
