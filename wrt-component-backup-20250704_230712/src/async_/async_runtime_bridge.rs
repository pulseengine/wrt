...
pub mod component_async {
    ...
    pub fn execute_async_operation(
        ...
        let task_id = task_manager
            ...
            .map_err(|e| "Component not foundMissing message")?;

        // Start the task
        task_manager.start_task(task_id).map_err(|e| "Component not foundMissing message")?;
...
mod tests {
    ...
    fn test_component_model_stream_without_rust_futures() {
        ...
        assert!(matches!(result1, StreamPollResult::Item(ref s) if s == "HelloMissing messageMissing messageMissing message");

        let result2 = poll_stream(&mut wasm_stream, &mut task_manager);
        assert!(matches!(result2, StreamPollResult::Item(ref s) if s == "WorldMissing messageMissing messageMissing message");
