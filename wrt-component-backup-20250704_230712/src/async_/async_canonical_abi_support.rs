...
impl AsyncCanonicalAbiSupport {
    ...
    pub fn initialize_component_abi(
        ...
        self.abi_contexts.insert(component_id, context).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many component ABI contextsMissing message")
        })?;
    ...
    pub fn async_call(
        ...
        let context = self.abi_contexts.get_mut(&component_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Component not initialized for async ABIMissing message")
        })?;
        ...
        self.async_operations.insert(operation_id, stored_operation).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many async ABI operationsMissing message")
        })?;

        // Add to component context
        context.active_calls.push(operation_id).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Component active calls fullMissing message")
        })?;
    ...
    pub fn async_resource_call(
        ...
        self.async_operations.insert(operation_id, stored_operation).map_err(|_| {
            Error::resource_limit_exceeded("Missing error message"Too many async operationsMissing message")
        })?;
    ...
    pub fn check_operation_status(&self, operation_id: AsyncAbiOperationId) -> Result<AsyncAbiOperationStatus, Error> {
        let operation = self.async_operations.get(&operation_id).ok_or_else(|| {
            Error::validation_invalid_input("Missing error message"Operation not foundMissing message")
        })?;
