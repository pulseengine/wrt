...
impl CanonicalABI {
    ...
    pub fn lift_char<M: CanonicalMemory>(&self, memory: &M, offset: u32) -> Result<ComponentValue> {
        ...
        let ch = char::from_u32(code_point).ok_or_else(|| {
            Error::validation_error("Missing error message"Invalid Unicode code pointMissing message")
        })?;
    ...
    pub fn lift_string<M: CanonicalMemory>(
        ...
        if len > MAX_STRING_LENGTH as u32 {
            return Err(Error::validation_error("Missing error message"String too longMissing messageMissing messageMissing message");
        }
        ...
        let string = match self.string_options.encoding {
            StringEncoding::Utf8 => String::from_utf8(bytes).map_err(|_| {
                Error::validation_error("Missing error message"Invalid UTF-8 stringMissing message")
            })?,
            StringEncoding::Utf16Le => {
                if bytes.len() % 2 != 0 {
                    return Err(Error::validation_error("Missing error message"UTF-16 byte sequence must have even lengthMissing messageMissing messageMissing message");
                }
                ...
                String::from_utf16(&code_units).map_err(|_| {
                    Error::validation_error("Missing error message"Invalid UTF-16 sequenceMissing message")
                })?
            }
    ...
    pub fn lift_list<M: CanonicalMemory>(
        ...
        if len > MAX_LIST_LENGTH as u32 {
            return Err(Error::validation_error("Missing error message"List too longMissing messageMissing messageMissing message");
        }
    ...
    pub fn lift_variant<M: CanonicalMemory>(
        ...
        if discriminant as usize >= cases.len() {
            return Err(Error::validation_error("Missing error message"Invalid variant discriminantMissing messageMissing messageMissing message");
        }
    ...
    pub fn lift_enum<M: CanonicalMemory>(
        ...
        if discriminant as usize >= cases.len() {
            return Err(Error::validation_error("Missing error message"Invalid enum discriminantMissing messageMissing messageMissing message");
        }
    ...
    pub fn lift_result<M: CanonicalMemory>(
        ...
        match discriminant {
            ...
            _ => Err(Error::validation_error("Missing error message"Invalid result discriminantMissing messageMissing messageMissing message")),
        }
    }
    ...
    pub fn lower_variant<M: CanonicalMemory>(
        ...
        let discriminant = cases.iter()
            ...
            .ok_or_else(|| Error::validation_invalid_type("Missing error message"Variant case not foundMissing messageMissing messageMissing message"))?;
        ...
        match discriminant_size {
            ...
            _ => return Err(Error::type_error("Missing error message"Invalid discriminant size calculatedMissing messageMissing messageMissing message")),
        }
    ...
    pub fn lower_enum<M: CanonicalMemory>(
        ...
        let discriminant = cases.iter()
            ...
            .ok_or_else(|| Error::validation_invalid_type("Missing error message"Enum case not foundMissing messageMissing messageMissing message"))?;
