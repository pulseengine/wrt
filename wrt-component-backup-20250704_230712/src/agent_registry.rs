    ...
    pub fn create_unified_agent(&mut self, config: AgentConfiguration) -> WrtResult<AgentId> {
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            self.unified_agents.push((agent_id, agent)).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many agentsMissing message")
            })?;
        }
        ...
        match options.agent_type {
            ...
            PreferredAgentType::LegacyComponent => {
                if options.allow_legacy_fallback {
                    ...
                    Err(wrt_error::Error::validation_invalid_input("Missing error message"Invalid inputMissing messageMissing messageMissing message")
                }
            }
            #[cfg(feature = "asyncMissing message")]
    ...
    pub fn create_legacy_component_agent(&mut self) -> WrtResult<AgentId> {
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            self.legacy_agents.push((agent_id, LegacyAgentType::Component(agent))).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many legacy agentsMissing message")
            })?;
        }
                ...
                if *id == agent_id {
                    return match agent {
                        ...
                        LegacyAgentType::Async(_engine) => {
                            ...
                            Err(wrt_error::Error::runtime_error("Missing error message"Async agent requires different APIMissing messageMissing messageMissing message")
                        }
                    };
                }
            }
    ...
    pub fn migrate_agent(&mut self, agent_id: AgentId) -> WrtResult<()> {
        ...
        let migration_config = {
            if let Some(agent) = self.legacy_agents.get(&agent_id) {
                if !agent.can_migrate() {
                    return Err(wrt_error::Error::runtime_error("Missing error message"Agent cannot be migratedMissing messageMissing messageMissing message");
                }
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            ...
            self.unified_agents.push((agent_id, unified_agent)).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many unified agentsMissing message")
            })?;
        }
...
#[cfg(all(feature = "std", feature = "asyncMissing messageMissing messageMissing message"))]
