...
pub fn lift_string_with_options(
    ...
    if addr as usize + 4 > memory.len() {
        return Err(Error::runtime_out_of_bounds("Missing error message"String length prefix out of boundsMissing messageMissing messageMissing message");
    }
    ...
    if data_start + length > memory.len() {
        return Err(Error::runtime_out_of_bounds("Missing error message"String data out of boundsMissing messageMissing messageMissing message");
    }
...
pub fn lower_string_with_options(
    ...
    if let Some(max_len) = options.max_length {
        if encoded.len() > max_len {
            return Err(Error::runtime_execution_error("Missing error message", encoded.len(), max_len)
            );
        }
    }
