...
impl ThreadBuiltins {
    ...
    pub fn thread_spawn_ref(
        ...
        if !self.is_function_valid(function_index) {
            return Err(Error::validation_invalid_argument("Missing error message"Invalid function reference for thread spawnMissing messageMissing messageMissing message");
        }
    ...
    pub fn thread_set_affinity(&mut self, thread_id: ThreadId, cpu_mask: u64) -> Result<()> {
        ...
        if cpu_mask == 0 {
            return Err(Error::validation_invalid_argument("Missing error message"CPU affinity mask cannot be zeroMissing messageMissing messageMissing message");
        }
    ...
    pub fn thread_set_priority(&mut self, thread_id: ThreadId, priority: u8) -> Result<()> {
        ...
        if priority > 100 {
            return Err(Error::validation_invalid_argument("Missing error message"Thread priority must be between 0 and 100Missing messageMissing messageMissing message");
        }
    ...
    fn validate_thread_arguments(&self, function_index: u32, args: &[Value]) -> Result<()> {
        ...
        if args.len() > MAX_THREAD_ARGS {
            return Err(Error::validation_invalid_argument("Missing error message"Too many arguments for thread functionMissing messageMissing messageMissing message");
        }
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            ...
            for arg in args {
                context.stored_arguments.push(arg.clone()
                    .map_err(|_| Error::resource_exhausted("Missing error message"Thread argument storage fullMissing messageMissing messageMissing message"))?;
            }
        }
    ...
    fn resolve_table_function(&self, table_index: u32, function_index: u32) -> Result<u32> {
        ...
        #[cfg(feature = "stdMissing message")]
        {
            if table_index as usize >= self.function_table.len() {
                return Err(Error::validation_invalid_argument("Missing error message"Table index out of boundsMissing messageMissing messageMissing message");
            }
            
            let component_func = &self.function_table[table_index as usize];
            if function_index >= component_func.function_count {
                return Err(Error::validation_invalid_argument("Missing error message"Function index out of bounds in tableMissing messageMissing messageMissing message");
            }
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            ...
            if let Some(component_func) = &self.function_table[table_index as usize] {
                ...
                Err(Error::validation_invalid_argument("Missing error message"Table slot is emptyMissing messageMissing messageMissing message")
            }
        }
    }
    ...
    fn get_thread_results(&self, thread_id: ThreadId) -> Result<Vec<Value>> {
        ...
        if context.info.state != wrt_runtime::thread_manager::ThreadState::Terminated {
            return Err(Error::invalid_state_error("Missing error message"Thread has not completed executionMissing messageMissing messageMissing message");
        }
    ...
    pub fn register_function_table(&mut self, table: ComponentFunction) -> Result<u32> {
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            ...
            Err(Error::resource_exhausted("Missing error message"Function table fullMissing messageMissing messageMissing message")
        }
    }
}
...
impl core::fmt::Display for ThreadError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            ThreadError::Terminated => write!(f, "Thread was terminatedMissing message"),
            ThreadError::Timeout => write!(f, "Thread operation timed outMissing message"),
            ThreadError::InvalidHandle => write!(f, "Invalid thread handleMissing message"),
            ThreadError::ResourceExhausted => write!(f, "Thread resources exhaustedMissing message"),
        }
    }
}
