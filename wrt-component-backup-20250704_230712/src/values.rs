...
pub fn deserialize_component_value(
    ...
    match format_type {
        WrtFormatValType::Bool => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize boolMissing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::S8 => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize S8Missing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::U8 => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize U8Missing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::S16 => {
            if offset + 2 > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize S16Missing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::U16 => {
            if offset + 2 > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize U16Missing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::S32 => {
            if offset + 4 > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize S32Missing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::U32 => {
            if offset + 4 > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize U32Missing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::S64 => {
            if offset + 8 > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize S64Missing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::U64 => {
            if offset + 8 > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize U64Missing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::F32 => {
            if offset + 4 > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize F32Missing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::F64 => {
            if offset + 8 > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize F64Missing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::Char => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize CharMissing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::String => {
            if offset + 4 > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize String lengthMissing messageMissing messageMissing message");
            }
            ...
            if offset + len as usize > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize StringMissing messageMissing messageMissing message");
            }
            let value =
                String::from_utf8(data[offset..offset + len as usize].to_vec()).map_err(|e| {
                    Error::parse_error("Missing error message"Component not foundMissing message")
                })?;
            Ok(ComponentComponentValue::String(value)
        }
        WrtFormatValType::List(elem_type) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize List lengthMissing messageMissing messageMissing message");
            }
            let len = u32::from_le_bytes([
                data[offset],
                data[offset + 1],
            ...
            if offset + len as usize * size_in_bytes(elem_type) > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize ListMissing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::Record(fields) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize RecordMissing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::Tuple(types) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize TupleMissing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::Variant(cases) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize VariantMissing messageMissing messageMissing message");
            }
            let len = u32::from_le_bytes([
                data[offset],
                data[offset + 1],
            ...
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize Variant caseMissing messageMissing messageMissing message");
            }
            ...
            if case_idx >= cases.len() {
                return Err(Error::parse_error("Missing error message"Component not foundMissing messageMissing messageMissing message");
            }
            ...
            match case_type_opt {
                Some(case_type) => {
                    if offset >= data.len() {
                        return Err(Error::parse_error("Missing error message"Not enough data to deserialize Variant valueMissing messageMissing messageMissing message");
                    }
        ...
        WrtFormatValType::Enum(variants) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize EnumMissing messageMissing messageMissing message");
            }
            let len = u32::from_le_bytes([
                data[offset],
                data[offset + 1],
        ...
        WrtFormatValType::Option(inner_type) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize OptionMissing messageMissing messageMissing message");
            }
            let value = data[offset] != 0;
            // No need to update offset anymore as we return immediately
            if value {
        ...
        WrtFormatValType::Result(result_type) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize ResultMissing messageMissing messageMissing message");
            }
            let value = data[offset] != 0;
            // No need to update offset anymore as we return immediately
            if value {
        ...
        WrtFormatValType::Own(idx) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize OwnMissing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::Borrow(idx) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize BorrowMissing messageMissing messageMissing message");
            }
            let value = data[offset] as u32;
            // No need to update offset anymore as we return immediately
        ...
        WrtFormatValType::Flags(names) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize FlagsMissing messageMissing messageMissing message");
            }
        ...
        WrtFormatValType::FixedList(elem_type, size) => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize FixedList lengthMissing messageMissing messageMissing message");
            }
            let len = u32::from_le_bytes([
                data[offset],
                data[offset + 1],
            ...
            offset += 4;
            if offset + len as usize * size_in_bytes(elem_type) > data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize FixedListMissing messageMissing messageMissing message");
            }
            let mut values = Vec::with_capacity(len as usize);
            for _ in 0..len {
                let value = deserialize_component_value(&data[offset..], elem_type)?;
        ...
        WrtFormatValType::ErrorContext => {
            if offset >= data.len() {
                return Err(Error::parse_error("Missing error message"Not enough data to deserialize ErrorContextMissing messageMissing messageMissing message");
            }
            let value = data[offset] != 0;
            // No need to update offset anymore as we return immediately
        ...
        _ => Err(Error::system_unsupported_operation("Missing error message"Type not supported for deserializationMissing messageMissing messageMissing message")),
    }
}
...
pub fn deserialize_component_value_with_stream<'a, P: wrt_foundation::MemoryProvider>(
    ...
    match format_type {
        ...
        WrtFormatValType::Record(fields) => {
            ...
            if count != fields.len() {
                return Err(Error::runtime_execution_error("Missing error message", fields.len(), count)
                );
            }
        ...
        WrtFormatValType::Tuple(types) => {
            ...
            if count != types.len() {
                return Err(Error::runtime_execution_error("Missing error message", types.len(), count)
                );
            }
        ...
        WrtFormatValType::Variant(cases) => {
            ...
            if has_value {
                if let Some(case_type) = case_type_opt {
                    ...
                    Err(Error::parse_error("Missing error message"Component not foundMissing messageMissing messageMissing message")
                }
            } else {
        ...
        WrtFormatValType::Flags(names) => {
            ...
            if count != names.len() {
                return Err(Error::runtime_execution_error("Missing error message", names.len(), count)
                );
            }
        ...
        // Handle any other unimplemented cases
        _ => Err(Error::system_unsupported_operation("Missing error message"Component not foundMissing messageMissing messageMissing message")),
    }
}
...
pub fn deserialize_component_values(
    ...
    if data.len() < 4 {
        return Err(Error::parse_error("Missing error message"Not enough data to read value countMissing messageMissing messageMissing message");
    }
    ...
    if count > types.len() {
        return Err(Error::validation_error("Missing error message"Validation errorMissing messageMissing messageMissing message");
    }
    ...
    for type_idx in 0..count {
        ...
        if offset + 4 > data.len() {
            return Err(Error::parse_error("Missing error message"Not enough data to read value sizeMissing messageMissing messageMissing message");
        }
        ...
        if offset + size > data.len() {
            return Err(Error::parse_error("Missing error message"Not enough data to read valueMissing messageMissing messageMissing message");
        }
...
pub fn core_to_component_value(value: &Value, ty: &WrtFormatValType) -> crate::WrtResult<ComponentComponentValue> {
    ...
    match (&component_value, &types_val_type) {
        ...
        _ => Err(Error::runtime_execution_error("Missing error message",
