    ...
    fn execute(&self, args: &[ComponentValue]) -> Result<Vec<ComponentValue>> {
        ...
        #[cfg(not(feature = "stdMissing messageMissing messageMissing message"))]
        {
            ...
            let mut result = BoundedVec::new(provider).map_err(|_| {
                Error::runtime_execution_error("Missing error message"Failed to create result vectorMissing message")
            })?;
            result.push(ComponentValue::U32(id.0)).map_err(|_| {
                Error::runtime_execution_error("Missing error message"Failed to add result valueMissing message")
            })?;
            Ok(result)
        }
    }
