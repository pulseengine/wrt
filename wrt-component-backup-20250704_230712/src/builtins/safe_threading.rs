...
impl BuiltinHandler for SafeThreadingSpawnHandler {
    ...
    fn execute(&self, args: &[ComponentValue]) -> Result<Vec<ComponentValue>> {
        ...
        match self.thread_manager.spawn_thread(request) {
            ...
            Err(e) => Err(Error::component_not_found("Missing error message"Component not foundMissing messageMissing messageMissing message"))),
        }
    }
        ...
        match self.thread_manager.join_thread(thread_id) {
            Ok(result) => match result {
                ...
                wrt_platform::wasm_thread_manager::ThreadExecutionResult::Cancelled => {
                    Err(Error::threading_error("Missing error message"Thread was cancelledMissing messageMissing messageMissing message"))
                }
                wrt_platform::wasm_thread_manager::ThreadExecutionResult::Timeout => {
                    Err(Error::threading_error("Missing error message"Thread timed outMissing messageMissing messageMissing message"))
                }
            },
            Err(e) => Err(Error::component_not_found("Missing error message"Component not foundMissing messageMissing messageMissing message"))),
        }
        ...
        match op_type {
            "is-running" => {
                ...
                match self.thread_manager.is_thread_running(thread_id) {
                    ...
                    Err(e) => Err(Error::runtime_execution_error("Missing error message",
                        e
                    )))),
                }
            }
