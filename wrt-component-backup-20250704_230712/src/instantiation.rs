...
impl ImportValues {
    ...
    pub fn add(&mut self, name: String, value: ImportValue) -> WrtResult<()> {
        self.imports.insert(name, value).map_err(|_| {
            wrt_error::Error::resource_exhausted("Missing error message"Too many imports (limit: 256)Missing message")
        })?;
        Ok(()
    }
    ...
    pub fn add(&mut self, name: BoundedString<64, InstantiationProvider>, value: ImportValue) -> WrtResult<()> {
        self.imports
            .push((name, value)
            .map_err(|_| wrt_error::Error::resource_exhausted("Missing error message"Too many importsMissing messageMissing messageMissing message")
    }
...
impl Component {
        ...
        #[cfg(feature = "stdMissing message")]
        {
            for import in &self.imports {
                match imports.get(&import.name) {
                    ...
                    None => {
                        return Err(wrt_error::Error::validation_invalid_input("Missing error message"Invalid inputMissing messageMissing messageMissing message");
                    }
                }
            }
        }
        ...
        match (&import.import_type, value) {
            (crate::import::ImportType::Function(expected), ImportValue::Function(actual)) => {
                ...
                if !self.is_function_compatible(expected, &actual.signature) {
                    return Err(wrt_foundation::wrt_error::Error::type_mismatch_error("Missing error message"Function import type mismatchMissing messageMissing messageMissing message");
                }
            }
            (crate::import::ImportType::Value(expected), ImportValue::Value(actual)) => {
                // Check value type compatibility
                if !self.is_value_compatible(expected, actual) {
                    return Err(wrt_foundation::wrt_error::Error::type_mismatch_error("Missing error message"Value import type mismatchMissing messageMissing messageMissing message");
                }
            }
            ...
            _ => {
                return Err(wrt_foundation::wrt_error::Error::type_mismatch_error("Missing error message"Import kind mismatchMissing messageMissing messageMissing message");
            }
        }
        Ok(()
    }
    ...
    fn create_resource_tables(&self) -> WrtResult<WrtVec<ResourceTable, {CrateId::Component as u8}, 16>> {
        ...
        for _type_id in 0..self.types.len().min(16) {
            ...
            tables.push(table).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many resource tables (limit: 16)Missing message")
            })?;
        }
    ...
    fn create_resource_tables(&self) -> WrtResult<BoundedVec<ResourceTable, 16, crate::bounded_component_infra::ComponentProvider>> {
        ...
        for _type_id in 0..self.types.len().min(16) {
            let table = ResourceTable::new()?;
            tables.push(table).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many resource tablesMissing message")
            })?;
        }
        ...
        for import in &self.imports {
            if let Some(value) = imports.get(&import.name) {
                ...
                resolved.push(resolved_import).map_err(|_| {
                    wrt_error::Error::resource_exhausted("Missing error message"Too many resolved imports (limit: 256)Missing message")
                })?;
            }
        }
        ...
        for import in &self.imports {
            ...
            for (name, value) in imports.imports.iter() {
                if name.as_str() == import.name.as_str() {
                    let resolved_import = self.resolve_import(import, value, context)?;
                    resolved.push(resolved_import).map_err(|_| {
                        wrt_error::Error::resource_exhausted("Missing error message"Too many resolved importsMissing message")
                    })?;
                    break;
                }
            }
    ...
    fn initialize_modules(
        ...
        for (module_index, _module) in self.modules.iter().enumerate() {
            ...
            instances.push(instance).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many module instances (limit: 64)Missing message")
            })?;
        }
    ...
    fn initialize_modules(
        ...
        for (module_index, _module) in self.modules.iter().enumerate() {
            let instance = ModuleInstance { module_index: module_index as u32 };
            instances.push(instance).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many module instancesMissing message")
            })?;
        }
    ...
    fn extract_exports(
        ...
        for export in &self.exports {
            ...
            exports.push(resolved).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many exports (limit: 256)Missing message")
            })?;
        }
    ...
    fn extract_exports(
        ...
        for export in &self.exports {
            ...
            exports.push(resolved).map_err(|_| {
                wrt_error::Error::resource_exhausted("Missing error message"Too many exportsMissing message")
            })?;
        }
