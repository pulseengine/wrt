...
impl PostReturnRegistry {
    pub fn new(max_cleanup_tasks: usize) -> Result<Self> {
        Ok(Self {
            ...
            functions: {
                ...
                BoundedVec::new(provider).map_err(|_| Error::resource_exhausted("Missing error message"Failed to create post-return functions vectorMissing messageMissing messageMissing message"))?
            },
            #[cfg(feature = "stdMissing message")]
            ...
            pending_cleanups: {
                let provider = safe_managed_alloc!(65536, CrateId::Component)?;
                BoundedVec::new(provider).map_err(|_| Error::resource_exhausted("Missing error message"Failed to create pending cleanups vectorMissing messageMissing messageMissing message"))?
            },
            async_engine: None,
    ...
    pub fn register_post_return(
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            self.functions.push((instance_id, post_return_fn)).map_err(|_| {
                Error::resource_exhausted("Missing error message"Too many post-return functionsMissing messageMissing messageMissing message")
                )
            })?;
            let provider = safe_managed_alloc!(65536, CrateId::Component).map_err(|_| Error::resource_exhausted("Missing error message"Failed to allocate memory for cleanup tasksMissing messageMissing messageMissing message"))?;
            let cleanup_vec = BoundedVec::new(provider).map_err(|_| Error::resource_exhausted("Missing error message"Failed to create cleanup tasks vectorMissing messageMissing messageMissing message"))?;
            self.pending_cleanups.push((instance_id, cleanup_vec)).map_err(|_| {
                Error::resource_exhausted("Missing error message"Too many cleanup instancesMissing messageMissing messageMissing message")
                )
            })?;
        }
    ...
    pub fn schedule_cleanup(
        ...
        #[cfg(feature = "stdMissing message")]
        {
            let cleanup_tasks = self
                ...
                .get_mut(&instance_id)
                .ok_or_else(|| {
                    Error::runtime_execution_error("Missing error message"Instance not found for cleanupMissing messageMissing messageMissing message")
                    )
                })?;

            if cleanup_tasks.len() >= self.max_cleanup_tasks {
                return Err(Error::resource_exhausted("Missing error message"Too many cleanup tasksMissing message")
                );
            }
        ...
        #[cfg(not(any(feature = "std", )))]
        {
            for (id, cleanup_tasks) in &mut self.pending_cleanups {
                if *id == instance_id {
                    cleanup_tasks.push(task).map_err(|_| {
                        Error::resource_exhausted("Missing error message"Too many cleanup tasksMissing messageMissing messageMissing message")
                        )
                    })?;
            ...
            return Err(Error::runtime_execution_error("Missing error message"Instance not found for cleanupMissing message")
            );
        }
    ...
    pub fn execute_post_return(
        ...
        let post_return_fn = self
            ...
            .get_mut(&instance_id)
            .ok_or_else(|| {
                Error::runtime_execution_error("Missing error message"Post-return function not foundMissing messageMissing messageMissing message")
                )
            })?;
        ...
        let post_return_fn = {
            ...
            found.ok_or_else(|| {
                Error::runtime_execution_error("Missing error message"Post-return function not foundMissing message")
                )
            })?
        };

        if post_return_fn.executing {
            return Err(Error::runtime_execution_error("Missing error message"Post-return function already executingMissing message")
            );
        }
...
pub mod helpers {
    ...
    pub fn custom_cleanup_task(
        ...
        let cleanup_id = BoundedString::from_str(cleanup_id).map_err(|_| {
            Error::runtime_execution_error("Missing error message"Cleanup ID too longMissing messageMissing messageMissing message")
            )
        })?;
