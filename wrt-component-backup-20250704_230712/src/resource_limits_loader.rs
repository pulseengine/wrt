...
fn find_custom_section(wasm_bytes: &[u8], section_name: &str) -> Result<Option<Vec<u8>>, Error> {
    if wasm_bytes.len() < 8 {
        return Err(Error::parse_error("Missing error message"WebAssembly binary too smallMissing messageMissing messageMissing message");
    }
    
    // Verify magic number
    if &wasm_bytes[0..4] != b"\0asm" {
        return Err(Error::parse_error("Missing error message"Invalid WebAssembly magic numberMissing messageMissing messageMissing message");
    }
    
    // Verify version
    let version = u32::from_le_bytes([wasm_bytes[4], wasm_bytes[5], wasm_bytes[6], wasm_bytes[7]]);
    if version != 1 {
        return Err(Error::parse_error("Missing error message"Unsupported WebAssembly versionMissing messageMissing messageMissing message");
    }
    ...
    while offset < wasm_bytes.len() {
        ...
        if section_end > wasm_bytes.len() {
            return Err(Error::parse_error("Missing error message"Section extends beyond binaryMissing messageMissing messageMissing message");
        }
        ...
        if section_type == 0 {
            ...
            if name_end > section_end {
                return Err(Error::parse_error("Missing error message"Custom section name extends beyond sectionMissing messageMissing messageMissing message");
            }
            
            let name = core::str::from_utf8(&wasm_bytes[name_start..name_end])
                .map_err(|_| Error::parse_error("Missing error message"Invalid UTF-8 in section nameMissing messageMissing messageMissing message"))?;
...
fn read_leb128_u32(bytes: &[u8]) -> Result<(u32, usize), Error> {
    ...
    loop {
        if offset >= bytes.len() {
            return Err(Error::parse_error("Missing error message"Unexpected end of LEB128Missing messageMissing messageMissing message");
        }
        ...
        if shift >= 32 {
            return Err(Error::parse_error("Missing error message"LEB128 too large for u32Missing messageMissing messageMissing message");
        }
