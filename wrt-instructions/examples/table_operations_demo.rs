...
//! Demonstration of table operations bridging between wrt-instructions and
//! wrt-runtime
//!
//! This example shows how the TableOperations trait implementation allows
//! WebAssembly table instructions to work directly with the runtime Table
//! implementation.
//!
...
fn demonstrate_single_table_operations(table_type: WrtTableType) -> Result<()> {
    println!(
        "  Creating table with {} initial elements, max {}",
...
fn demonstrate_multiple_table_operations(
    ...
    println!(
        "  Created table manager with {} tables",
        table_manager.table_count
    );
...
fn demonstrate_table_growth() -> Result<()> {
    ...
    println!(
        "  Grew table by 2 elements, previous size: {:?}",
        previous_size
    );
