...
//! - Floating-point comparison operations (equality, relational with NaN handling)
//! - Test operations (eqz for testing zero values)
//! - Edge cases and WebAssembly-specific semantics

use wrt_instructions::{
    ComparisonOp, ComparisonContext, PureInstruction,
};
...
fn main() -> Result<()> {
    ...
    if let Some(Value::I32(result)) = context.peek() {
        println!("   -1 < 10 (unsigned): {} (false, -1 as unsigned is very large)", result);
    }
    context.stack.clear();
    ...
    if let Some(Value::I32(result)) = context.peek() {
        println!("   -1 > 10 (unsigned): {} (true, -1 as unsigned is very large)", result);
    }
    context.stack.clear();
    ...
    ComparisonOp::F32Lt.execute(&mut context)?;
    if let Some(Value::I32(result)) = context.peek() {
        println!("   NaN < 42.0: {} (false - NaN comparisons are always false)", result);
    }
    context.stack.clear();
    ...
    ComparisonOp::I32LtU.execute(&mut context)?;
    if let Some(Value::I32(result)) = context.peek() {
        println!("   i32::MAX < i32::MIN (unsigned): {} (true - MIN as unsigned is 2^31)", result);
    }
