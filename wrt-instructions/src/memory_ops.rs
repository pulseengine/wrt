...
mod tests {
    ...
    impl MockMemory {
        ...
        fn new(size: usize) -> Result<Self> {
            ...
            let mut data = wrt_foundation::BoundedVec::new(provider)
                .map_err(|_| Error::memory_error("Failed to create BoundedVec"))?;
            for _ in 0..core::cmp::min(size, 65536) {
                data.push(0).map_err(|_| Error::memory_error("Failed to push to BoundedVec"))?;
            }
            Ok(Self { data })
        }
    }
    ...
    impl MemoryOperations for MockMemory {
        ...
        ) -> Result<wrt_foundation::BoundedVec<u8, 65_536, wrt_foundation::NoStdProvider<65_536>>>
        {
            ...
            for i in start..end {
                let byte =
                    self.data.get(i).map_err(|_| Error::memory_error("Index out of bounds"))?;
                result
                    .push(byte)
                    .map_err(|_| Error::memory_error("BoundedVec capacity exceeded"))?;
            }
            Ok(result)
        }
        ...
        fn write_bytes(&mut self, offset: u32, bytes: &[u8]) -> Result<()> {
            ...
            for (i, &byte) in bytes.iter().enumerate() {
                self.data
                    .set(start + i, byte)
                    .map_err(|_| Error::memory_error("Index out of bounds"))?;
            }
            Ok(())
        }
        ...
        fn copy(&mut self, dest: u32, src: u32, size: u32) -> Result<()> {
            ...
            if size > 0 {
                ...
                for chunk_start in (0..size as usize).step_by(256) {
                    ...
                    for i in 0..chunk_size {
                        self.data
                            .set(dest_start + chunk_start + i, temp_buffer[i])
    ...
    fn test_memory_store() {
        ...
        store
            .execute(&mut memory, &Value::I32(0), &Value::I64(0x0102030405060708))
            .unwrap();
    ...
    struct MockDataSegments {
        ...
        #[cfg(not(any(feature = "std",)))]
        segments: wrt_foundation::BoundedVec<
            Option<wrt_foundation::BoundedVec<u8, 65_536, wrt_foundation::NoStdProvider<65_536>>>,
            16,
        ...
        > {
            if (data_index as usize) < self.segments.len() {
                Ok(self
                    ...
                    .ok_or_else(|| Error::validation_error("Invalid data segment index"))?
                    .clone())
            } else {
                Err(Error::validation_error("Invalid data segment index"))
            }
