...
use wrt_component::{
    ...
    AgentRegistry, AgentId, Value, UnifiedExecutionState,
    RuntimeBridgeConfig,
};
...
impl WasmApplicationManager {
    ...
    pub fn execute_component_function(
        ...
        let agent_id = self.component_agents.get(component_name)
        ...
        let result = self.agent_registry.call_function(
            *agent_id,
            instance_id,
            function_index,
            &args,
        ...
        if component_result.success {
            println!("✅ Function '{}' executed successfully in {}ms", 
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app_manager.register_component(
        "ui".to_string(),
        ComponentType::UserInterface,
    )?;
    ...
    let batch_results = app_manager.execute_workflow(
        "batch_data_processing",
        Value::U32(1000),
    )?;
