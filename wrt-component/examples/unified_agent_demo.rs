    ...
    #[cfg(feature = "async")]
    {
        ...
        match agent.call_function(2, 200, &args) {
            Ok(result) => {
                ...
                println!("Async operations tracked: {}", 
                    agent.statistics().async_operations);
            }
    ...
    #[cfg(feature = "cfi")]
    {
        ...
        match agent.call_function(4, 400, &args) {
            Ok(result) => {
                ...
                println!("Result: {:?}", result);
                println!("CFI-protected instructions: {}", 
                    agent.statistics().cfi_instructions_protected);
                println!("CFI violations detected: {}", 
                    agent.statistics().cfi_violations_detected);
            }
...
fn demo_agent_registry() {
    ...
    let sync_agent_id = registry.create_unified_agent(
        AgentConfiguration::default()
    ...
    let stackless_agent_id = registry.create_unified_agent(stackless_config)
        .expect("Failed to create stackless agent");
...
fn demo_legacy_migration() {
    ...
    let legacy_id = registry.create_legacy_component_agent()
        .expect("Failed to create legacy agent");
