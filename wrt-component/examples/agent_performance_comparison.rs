...
use wrt_component::{
    // Unified system
    UnifiedExecutionAgent, AgentConfiguration, ExecutionMode, HybridModeFlags,
    AgentRegistry, AgentCreationOptions, PreferredAgentType,
    // Legacy agents (deprecated)
    ComponentExecutionEngine, ExecutionState,
    // Common types
    Value, UnifiedExecutionState,
};
...
fn test_agent_creation_performance() {
    ...
    // Results
    println!("Unified agent creation: {:?} total, {:?} per agent",
        unified_duration,
        unified_duration / ITERATIONS as u32
    );
    println!("Legacy agent creation:  {:?} total, {:?} per agent",
        legacy_duration,
...
fn test_execution_performance() {
    ...
    // Results
    println!("Unified execution: {:?} total, {:?} per call",
        unified_duration,
        unified_duration / ITERATIONS as u32
    );
    println!("Legacy execution:  {:?} total, {:?} per call",
        legacy_duration,
        legacy_duration / ITERATIONS as u32
    );
...
fn test_memory_usage() {
    ...
    let memory_savings = ((hybrid_legacy_size - unified_size) as f64 / hybrid_legacy_size as f64) * 100.0;
...
impl UnifiedExecutionAgent {
    ...
    fn drop_resource(&mut self, handle: wrt_component::ResourceHandle) -> wrt_component::WrtResult<()> {
