...
impl SimulatedAsyncWork {
    fn new(polls_until_ready: usize, fuel_per_poll: u64, result: u32, priority: Priority, can_block: bool) -> Self {
        Self {
...
mod tests {
    ...
    fn test_phase2_priority_inheritance_integration() {
        ...
        assert_eq!(stats.total_inheritances.load(core::sync::atomic::Ordering::Acquire), 1);
        assert_eq!(stats.inversions_prevented.load(core::sync::atomic::Ordering::Acquire), 1);
        ...
        assert_eq!(final_stats.active_chains.load(core::sync::atomic::Ordering::Acquire), 0);
    }
    ...
    fn test_phase2_async_channels_with_priorities() {
        ...
        let (sender, receiver) = channel_manager.create_channel(
            5, // capacity
            true, // enable priority inheritance
            TaskId::new(1), ComponentInstanceId::new(1), Priority::High,   // sender
            TaskId::new(2), ComponentInstanceId::new(1), Priority::Normal, // receiver
        ).unwrap();
        ...
        assert!(matches!(result, Err(super::super::async_::fuel_async_channels::ChannelError::WouldBlock(_))));
        ...
        assert_eq!(stats.total_channels_created.load(core::sync::atomic::Ordering::Acquire), 1);
        assert_eq!(stats.active_channels.load(core::sync::atomic::Ordering::Acquire), 1);
    ...
    fn test_phase2_preemptive_scheduler() {
        ...
        assert_eq!(stats.total_tasks_scheduled.load(core::sync::atomic::Ordering::Acquire), 3);
    ...
    fn test_phase2_integrated_async_system() {
        ...
        let mut preemptive_scheduler = FuelPreemptiveScheduler::new(config, VerificationLevel::Standard).unwrap();
        
        // 3. Create priority inheritance protocol
        let mut priority_protocol = FuelPriorityInheritanceProtocol::new(VerificationLevel::Standard).unwrap();
        ...
        let (sender, receiver) = channel_manager.create_channel(
            10, // capacity
            true, // enable priority inheritance
            TaskId::new(1), ComponentInstanceId::new(1), Priority::High,   // producer
            TaskId::new(2), ComponentInstanceId::new(1), Priority::Normal, // consumer
        ).unwrap();
        
        // 6. Add tasks to preemptive scheduler
        preemptive_scheduler.add_task(
        ...
        preemptive_scheduler.update_task_state(
            TaskId::new(1),
        ...
        if let Some(current_context) = preemptive_scheduler.current_task.as_ref() {
            let should_preempt = preemptive_scheduler.should_preempt_current_task(
                current_context,
                2000,
            ).unwrap();
        ...
        assert_eq!(scheduler_stats.active_tasks.load(core::sync::atomic::Ordering::Acquire), 3);
        assert!(scheduler_stats.total_context_switches.load(core::sync::atomic::Ordering::Acquire) > 0);
        assert!(channel_stats.total_messages_sent.load(core::sync::atomic::Ordering::Acquire) >= 2);
        assert!(channel_stats.total_messages_received.load(core::sync::atomic::Ordering::Acquire) >= 2);
        
        println!("Phase 2 Integration Test Results:");
        println!("- Executor fuel status: {}/{} fuel used", 
                 executor_stats.consumed, executor_stats.limit);
        println!("- Scheduler: {} active tasks, {} context switches", 
                 scheduler_stats.active_tasks.load(core::sync::atomic::Ordering::Acquire),
                 scheduler_stats.total_context_switches.load(core::sync::atomic::Ordering::Acquire));
        println!("- Channels: {} messages sent, {} received", 
                 channel_stats.total_messages_sent.load(core::sync::atomic::Ordering::Acquire),
                 channel_stats.total_messages_received.load(core::sync::atomic::Ordering::Acquire));
        println!("- Priority inheritance: {} total inheritances", 
    ...
    fn test_phase2_asil_b_compliance_scenario() {
        ...
        let (_safety_sender, _safety_receiver) = channel_manager.create_channel(
            5, // Small bounded capacity
            true, // Priority inheritance enabled
            safety_critical_task, ComponentInstanceId::new(1), Priority::Critical,
            application_task, ComponentInstanceId::new(2), Priority::Normal,
        ).unwrap();
        ...
        let total_scheduler_fuel = scheduler_stats.scheduler_fuel_consumed.load(core::sync::atomic::Ordering::Acquire);
...
mod examples {
    ...
    pub async fn create_phase2_async_system() -> Result<(), Error> {
        ...
        let preemptive_scheduler = FuelPreemptiveScheduler::new(scheduler_config, VerificationLevel::Standard)?;
