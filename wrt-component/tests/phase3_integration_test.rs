...
use wrt_component::{
    async_::{
        ...
        fuel_async_bridge::{FuelAsyncBridge, AsyncBridgeConfig},
        
        // Phase 2 components  
        fuel_priority_inheritance::{FuelPriorityInheritanceProtocol, ResourceId},
        fuel_async_channels::{FuelAsyncChannelManager, ChannelId},
        fuel_preemptive_scheduler::{FuelPreemptiveScheduler, PreemptiveSchedulerConfig},

        // Phase 3 components
        fuel_deadline_scheduler::{
            FuelDeadlineScheduler, DeadlineSchedulerConfig, AsilLevel, CriticalityMode,
        },
        fuel_wcet_analyzer::{
            FuelWcetAnalyzer, WcetAnalyzerConfig, WcetAnalysisMethod, WcetAnalysisResult,
        },
    },
    task_manager::TaskId,
    ComponentInstanceId,
...
impl DeadlineConstrainedTask {
    fn new_asil_c_control_task() -> Self {
        Self {
            ...
            execution_phases: vec![
                ExecutionPhase { fuel_cost: 15, min_duration: 5, variable_execution: false },  // Sensor reading
                ExecutionPhase { fuel_cost: 25, min_duration: 10, variable_execution: true }, // Control calculation
                ExecutionPhase { fuel_cost: 10, min_duration: 3, variable_execution: false }, // Actuator command
            ],
            current_phase: 0,
            fuel_consumed: 0,
            completed: false,
    ...
    fn new_asil_b_monitoring_task() -> Self {
        Self {
            ...
            execution_phases: vec![
                ExecutionPhase { fuel_cost: 20, min_duration: 8, variable_execution: true },  // Data collection
                ExecutionPhase { fuel_cost: 15, min_duration: 5, variable_execution: false }, // Analysis
                ExecutionPhase { fuel_cost: 8, min_duration: 2, variable_execution: false },  // Report
            ],
            current_phase: 0,
            fuel_consumed: 0,
            completed: false,
    ...
    fn new_background_task() -> Self {
        Self {
            ...
            execution_phases: vec![
                ExecutionPhase { fuel_cost: 30, min_duration: 15, variable_execution: true }, // Background processing
                ExecutionPhase { fuel_cost: 20, min_duration: 10, variable_execution: true }, // Cleanup
            ],
            current_phase: 0,
            fuel_consumed: 0,
            completed: false,
    ...
    fn calculate_wcet(&self) -> u64 {
        self.execution_phases.iter()
            .map(|phase| if phase.variable_execution { 
...
mod tests {
    ...
    fn test_phase3_wcet_analysis_integration() {
        ...
        let control_wcet = wcet_analyzer.analyze_task_wcet(
            control_task.task_id,
            control_task.component_id,
            Some(WcetAnalysisMethod::Static),
        ).unwrap();

        let monitoring_wcet = wcet_analyzer.analyze_task_wcet(
        ...
        println!("- ASIL-C Control Task: WCET = {} fuel, BCET = {} fuel", 
                 control_wcet.wcet_fuel, control_wcet.bcet_fuel);
        println!("- ASIL-B Monitoring Task: WCET = {} fuel, BCET = {} fuel", 
    ...
    fn test_phase3_deadline_scheduler_configuration() {
        ...
        assert_eq!(stats.total_tasks.load(core::sync::atomic::Ordering::Acquire), 3);
        assert_eq!(stats.active_tasks.load(core::sync::atomic::Ordering::Acquire), 3);
    ...
    fn test_phase3_hybrid_rm_edf_scheduling() {
        ...
        let stats = scheduler.get_statistics();
        assert_eq!(stats.successful_deadlines.load(core::sync::atomic::Ordering::Acquire), 1);
    }
    ...
    fn test_phase3_criticality_mode_switching() {
        ...
        scheduler.add_deadline_task(
            asil_d_task, ComponentInstanceId::new(1), AsilLevel::D,
            Duration::from_millis(20), Duration::from_millis(15), 10, 8
        ).unwrap();

        scheduler.add_deadline_task(
            asil_c_task, ComponentInstanceId::new(2), AsilLevel::C,
            Duration::from_millis(40), Duration::from_millis(30), 20, 15
        ).unwrap();

        scheduler.add_deadline_task(
            asil_b_task, ComponentInstanceId::new(3), AsilLevel::B,
            Duration::from_millis(60), Duration::from_millis(50), 25, 20
        ).unwrap();

        scheduler.add_deadline_task(
            asil_a_task, ComponentInstanceId::new(4), AsilLevel::A,
            Duration::from_millis(80), Duration::from_millis(70), 30, 25
        ).unwrap();

        scheduler.add_deadline_task(
            qm_task, ComponentInstanceId::new(5), AsilLevel::QM,
            Duration::from_millis(100), Duration::from_millis(90), 35, 30
        ).unwrap();

        // Initially in Low mode - all tasks active
        let stats = scheduler.get_statistics();
        assert_eq!(stats.active_tasks.load(core::sync::atomic::Ordering::Acquire), 5);
        ...
        let stats = scheduler.get_statistics();
        assert_eq!(stats.tasks_dropped.load(core::sync::atomic::Ordering::Acquire), 2);
        assert_eq!(stats.criticality_switches.load(core::sync::atomic::Ordering::Acquire), 1);
        ...
        let stats = scheduler.get_statistics();
        assert_eq!(stats.tasks_dropped.load(core::sync::atomic::Ordering::Acquire), 3);
        assert_eq!(stats.criticality_switches.load(core::sync::atomic::Ordering::Acquire), 2);
    ...
    fn test_phase3_wcet_enforcement_and_validation() {
        ...
        let mut wcet_analyzer = FuelWcetAnalyzer::new(wcet_config, VerificationLevel::Full).unwrap();

        let scheduler_config = DeadlineSchedulerConfig {
            enable_wcet_enforcement: true,
            ..Default::default()
        };
        let mut scheduler = FuelDeadlineScheduler::new(scheduler_config, VerificationLevel::Full).unwrap();
        ...
        let refined_result = wcet_analyzer.analyze_task_wcet(
            task_id,
            component_id,
        ...
        assert_eq!(wcet_stats.total_samples.load(core::sync::atomic::Ordering::Acquire), 7);
        assert_eq!(wcet_stats.underestimations.load(core::sync::atomic::Ordering::Acquire), 1);
        ...
        println!("- Confidence: {:.2}%", refined_result.confidence_level * 100.0);
    }
    ...
    fn test_phase3_asil_c_compliance_scenario() {
        ...
        for &task_id in &[engine_control_task, brake_monitor_task, steering_assist_task] {
            // Register critical paths
            wcet_analyzer.register_control_flow_path(
                task_id,
                1, // Normal path
                &[1, 2, 3],
                if task_id == engine_control_task { 35 } else if task_id == brake_monitor_task { 55 } else { 110 },
            ).unwrap();
            ...
            wcet_analyzer.register_control_flow_path(
                ...
                &[1, 4, 5, 6],
                if task_id == engine_control_task { 40 } else if task_id == brake_monitor_task { 60 } else { 120 },
            ).unwrap();
            ...
            println!("  WCET: {} fuel, BCET: {} fuel", wcet_result.wcet_fuel, wcet_result.bcet_fuel);
        ...
        while execution_cycles < max_cycles {
            ...
            if let Some(next_task) = scheduler.schedule_next_task().unwrap() {
                ...
                let validation_result = wcet_analyzer.validate_wcet_estimate(next_task, actual_fuel);
                if let Ok(within_estimate) = validation_result {
                    assert!(within_estimate, "Task {} exceeded WCET estimate", next_task.0);
                }
            }
        ...
        assert_eq!(scheduler_stats.total_deadline_misses.load(core::sync::atomic::Ordering::Acquire), 0);
        
        // All scheduled tasks completed successfully
        assert!(scheduler_stats.successful_deadlines.load(core::sync::atomic::Ordering::Acquire) > 0);
        
        // No WCET violations
        assert_eq!(wcet_stats.underestimations.load(core::sync::atomic::Ordering::Acquire), 0);

        // Utilization remains within safe bounds
        let current_util = scheduler_stats.current_utilization.load(core::sync::atomic::Ordering::Acquire) as f64 / 1000.0;
        assert!(current_util <= 0.5);

        println!("ASIL-C Compliance Verification Results:");
        println!("✓ Zero deadline misses: {}", scheduler_stats.total_deadline_misses.load(core::sync::atomic::Ordering::Acquire) == 0);
        println!("✓ Zero WCET violations: {}", wcet_stats.underestimations.load(core::sync::atomic::Ordering::Acquire) == 0);
        println!("✓ Conservative utilization: {:.2}% ≤ 50%", current_util * 100.0);
        println!("✓ Successful task executions: {}", scheduler_stats.successful_deadlines.load(core::sync::atomic::Ordering::Acquire));
        println!("✓ WCET analysis samples: {}", wcet_stats.total_samples.load(core::sync::atomic::Ordering::Acquire));
...
mod examples {
    ...
    pub async fn create_phase3_asil_c_system() -> Result<(), Error> {
        ...
        let deadline_scheduler = FuelDeadlineScheduler::new(scheduler_config, VerificationLevel::Full)?;
        ...
        let preemptive_scheduler = FuelPreemptiveScheduler::new(preemptive_config, VerificationLevel::Full)?;
    ...
    pub async fn asil_c_engine_control_example() -> Result<(), Error> {
        let mut wcet_analyzer = FuelWcetAnalyzer::new(WcetAnalyzerConfig::default(), VerificationLevel::Full)?;
        let mut scheduler = FuelDeadlineScheduler::new(DeadlineSchedulerConfig::default(), VerificationLevel::Full)?;
