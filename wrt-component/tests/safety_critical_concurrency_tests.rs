...
use wrt_component::{
    ...
    resources::{
        resource_lifecycle::{ResourceHandle, ResourceState, Resource, ResourceType},
        ResourceStrategy,
    },
};
use wrt_foundation::{
...
mod concurrency_tests {
    ...
    fn test_resource_table_concurrent_access() {
            ...
            let handle = thread::spawn(move || {
                ...
                for i in 0..resources_per_thread {
                    ...
                    match table.allocate() {
                        ...
                        Err(e) => {
                            ...
                            eprintln!("Thread {} failed to allocate resource {}: {:?}", 
                                     thread_id, i, e);
                        }
                    }
                }
                ...
                for (i, handle) in local_handles.iter().take(resources_per_thread / 2).enumerate() {
                    ...
                    match table.deallocate(*handle) {
                        Ok(_) => {},
                        Err(e) => {
                            eprintln!("Thread {} failed to deallocate resource {}: {:?}", 
                                     thread_id, i, e);
                        }
                    }
                }
        ...
        let total_allocated: usize = handles.into_iter()
            .map(|h| h.join().unwrap())
            .sum();
    ...
    fn test_export_map_concurrent_operations() {
        ...
        let total_ops: usize = handles.into_iter()
            .map(|h| h.join().unwrap())
            .sum();
    ...
    fn test_resource_lifecycle_concurrent() {
        ...
        let results: Vec<(usize, usize, usize)> = handles.into_iter()
            .map(|h| h.join().unwrap())
            .collect();
    ...
    fn test_concurrent_memory_pressure() {
            ...
            let handle = thread::spawn(move || {
                ...
                for i in 0..10 {
                    ...
                    match i % 4 {
                        ...
                        1 => {
                            match new_export_map::<String>() {
                                Ok(_) => allocations += 1,
                                Err(_) => failures += 1,
                            }
                        }
                        2 => {
                            match new_resource_vec::<u32>() {
                                Ok(_) => allocations += 1,
                                Err(_) => failures += 1,
                            }
                        }
                        3 => {
                            match new_call_stack::<u32>() {
                                Ok(_) => allocations += 1,
                                Err(_) => failures += 1,
                            }
                        }
        ...
        let results: Vec<(usize, usize)> = handles.into_iter()
            .map(|h| h.join().unwrap())
            .collect();
