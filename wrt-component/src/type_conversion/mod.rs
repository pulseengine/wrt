...
mod tests {
    ...
    fn test_bidirectional_conversion_compatibility() {
        ...
        let registry = TypeConversionRegistry::with_defaults();
        let types_val_via_registry =
            registry.convert::<FormatValType, TypesValType>(&format_val).unwrap();

        // They should produce the same result
        assert_eq!(types_val_via_bidirectional, types_val_via_registry);
    }

    // Note: The original integration_test.rs contained 214 lines of
    // comprehensive integration tests covering complex type conversion
    // scenarios, cross-crate compatibility, and wrapper type functionality.
    // These should be systematically migrated as the type conversion system
    // stabilizes.
}
