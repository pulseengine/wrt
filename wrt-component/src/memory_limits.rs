...
mod validation {
    ...
    fn validate_canonical_abi_budget() {
        let budget = 
            canonical_abi::TUPLE_VALUES_LIMIT * BYTES_PER_VALUE +
        ...
        assert!(budget < 150 * 1024, "Canonical ABI budget exceeds 150KB: {}KB", budget / 1024);
    }
    
    #[test]
    fn validate_resource_budget() {
        let budget = 
            resources::RESOURCE_TABLE_LIMIT * BYTES_PER_RESOURCE +
            resources::RESOURCE_TABLE_LIMIT * resources::BORROWS_PER_RESOURCE_LIMIT * 16 +
            resources::INTERCEPTORS_LIMIT * 64;
            
        assert!(budget < 300 * 1024, "Resource budget exceeds 300KB: {}KB", budget / 1024);
    }
    
    #[test]
    fn validate_instantiation_budget() {
        let budget = 
            instantiation::IMPORTS_LIMIT * BYTES_PER_IMPORT +
        ...
        assert!(budget < 100 * 1024, "Instantiation budget exceeds 100KB: {}KB", budget / 1024);
    }
    
    #[test]
    fn validate_communication_budget() {
        let budget = 
            communication::INSTANCE_REGISTRY_LIMIT * 64 +
        ...
        assert!(budget < 50 * 1024, "Communication budget exceeds 50KB: {}KB", budget / 1024);
    }
    
    #[test]
    fn validate_total_budget() {
        // This is a conservative estimate as not all limits will be reached simultaneously
        ...
        assert!(total <= TOTAL_BUDGET + 30 * 1024, 
            "Total budget exceeds 512KB + overhead: {}KB", total / 1024);
    }
}
