...
mod validation {
    ...
    fn validate_custom_sections_budget() {
        let budget = 
            custom_sections::SECTIONS_LIMIT * BYTES_PER_SECTION +
    ...
    fn validate_branch_hints_budget() {
        let budget = 
            branch_hints::FUNCTIONS_WITH_HINTS_LIMIT * BYTES_PER_HINT +
            branch_hints::FUNCTIONS_WITH_HINTS_LIMIT * branch_hints::HINTS_PER_FUNCTION_LIMIT * BYTES_PER_HINT;
    ...
    fn validate_total_budget() {
        ...
        assert!(total <= TOTAL_BUDGET, 
            "Total budget exceeds 512KB: {}KB", total / 1024);
    }
}
