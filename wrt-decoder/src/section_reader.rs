...
use wrt_format::section::{CUSTOM_ID, IMPORT_ID, TYPE_ID, TABLE_ID, FUNCTION_ID, MEMORY_ID, GLOBAL_ID, EXPORT_ID, START_ID, ELEMENT_ID, CODE_ID, DATA_ID, DATA_COUNT_ID};
...
impl<'a> SectionReader<'a> {
    ...
    pub fn new(binary: &'a [u8]) -> Result<Self> {
        ...
        if actual_version != binary::WASM_VERSION {
            return Err(section_error::unsupported_version(4, binary::WASM_VERSION, actual_version));
        }
    ...
    pub fn find_section(&mut self, section_id: u8) -> Result<Option<(usize, usize)>> {
        ...
        while self.current_offset < self.binary.len() {
            ...
            let (section_size, bytes_read) = match binary::read_leb128_u32(self.binary, self.current_offset) {
                Ok(result) => result,
        ...
        while self.current_offset < self.binary.len() {
            ...
            let (section_size, bytes_read) = match binary::read_leb128_u32(self.binary, self.current_offset) {
                ...
                Err(e) => {
                    ...
                    return Err(convert_to_wrt_error(e));
                }
            };
            self.current_offset += bytes_read;
            ...
            if id == CUSTOM_ID && section_size > 0 {
                ...
                let (section_name, name_size) = match binary::read_string(self.binary, section_start) {
                    Ok(result) => result, 
                    Err(e) => {
                        // Restore original position
                        self.current_offset = saved_offset;
            ...
            if id == CUSTOM_ID {
                ...
                if !section_data.is_empty() {
                    if let Ok((name, name_size)) = binary::read_string(section_data, 0) {
                        ...
                        println!("  Data: {:?}", &section_data[name_size.min(section_data.len())..]);
                    } else {
