...
# Fix remaining unterminated string patterns in async files (second pass)

find wrt-component/src/async_/ -name "*.rs" | while read -r file; do
    echo "Fixing remaining strings in $file (pass 2)"
    ...
    # Fix pattern: "Channel full Error message" -> "Channel full"
    perl -i -pe 's/"Channel full Error message"/"Channel full"/g' "$file"
...
echo "Fixed remaining unterminated string literals in async files (pass 2)"