...
mod validation {
    ...
    fn validate_host_functions_budget() {
        let budget = 
            host_functions::HOST_FUNCTIONS_LIMIT * BYTES_PER_HANDLER +
        ...
        assert!(budget < 50 * 1024, "Host functions budget exceeds 50KB: {}KB", budget / 1024);
    }
    
    #[test]
    fn validate_function_calls_budget() {
        let budget = 
            function_calls::MAX_FUNCTION_PARAMS * BYTES_PER_PARAM +
            function_calls::MAX_RETURN_VALUES * BYTES_PER_PARAM +
            function_calls::CALL_STACK_DEPTH_LIMIT * BYTES_PER_CALL_FRAME;
            
        assert!(budget < 16 * 1024, "Function calls budget exceeds 16KB: {}KB", budget / 1024);
    }
    ...
    fn validate_total_budget() {
        ...
        assert!(total <= TOTAL_BUDGET / 8, // Use only 1/8 of budget for host layer
            "Host layer exceeds 64KB allocation: {}KB", total / 1024);
    }
}
