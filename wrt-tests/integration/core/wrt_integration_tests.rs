...
impl wrt_test_registry::TestCase for CoreFeaturesTest {
    ...
    fn run(&self) -> TestResult {
        ...
        let table_type =
            wrt::TableType::new(wrt_foundation::reference::RefType::FuncRef, 10, Some(20));
    ...
    fn run(&self) -> TestResult {
        #[cfg(feature = "std")]
        {
            ...
            assert_test!(
                ...
                "New component should not be initialized"
            );
...
impl wrt_test_registry::TestCase for ErrorHandlingTest {
    ...
    fn run(&self) -> TestResult {
        ...
        assert_eq_test!(
            memory_error.kind(),
            kinds::ErrorKind::Memory,
            "Error kind should be Memory"
        );
...
impl wrt_test_registry::TestCase for DecoderTest {
    ...
    fn run(&self) -> TestResult {
        ...
        assert_test!(
            result.is_ok(),
            "Should successfully decode valid WASM module"
        );
        ...
        // Verify structure
        assert_test!(
            !decoded_module.functions.is_empty(),
            "Decoded module should have functions"
        );
        assert_test!(
            !decoded_module.exports.is_empty(),
            "Decoded module should have exports"
        );
...
fn run_integration_tests() {
    ...
    // In no_std, we just statically register the tests but can't run them all
    // automatically
    #[cfg(not(feature = "std"))]
    {
