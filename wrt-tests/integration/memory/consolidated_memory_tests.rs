...
//!
//! This module consolidates all memory safety test functionality from across
//! the WRT project, eliminating duplication and providing comprehensive testing
...
mod foundation_memory_tests {
    ...
    fn test_std_memory_provider() {
        ...
        // Test with different verification levels
        test_all_verification_levels(|level| {
            let provider_with_level =
                StdMemoryProvider::with_verification_level(data.clone(), level);
...
mod runtime_memory_tests {
    ...
    fn test_table_bounds_and_growth() -> Result<()> {
        ...
        assert!(
            table.set(10, func_value.clone()).is_err(),
...
mod memory_performance_tests {
    ...
    fn test_memory_operation_performance() -> Result<()> {
        ...
        assert!(
            ...
            "Memory write performance regression"
        );
        ...
        assert!(
            duration.as_millis() < 100,
            "Memory read performance regression"
        );
    ...
    fn test_verification_performance_impact() -> Result<()> {
        ...
        assert!(
            full_duration > off_duration,
            "Full verification should have some overhead"
        );
        assert!(
            full_duration.as_millis() < off_duration.as_millis() * 10,
    ...
    fn test_safe_stack_performance() -> Result<()> {
        ...
        assert!(
            duration.as_millis() < 100,
            "SafeStack performance regression"
        );
