...
//!
//! These tests verify the integration between atomic operations, memory
//! management, and thread synchronization in the WRT runtime.
...
fn benchmark_atomic_operations() -> Result<()> {
    ...
    println!(
        "  Load:  {:?} ({:.2} ns/op)",
        load_duration,
        load_duration.as_nanos() as f64 / NUM_OPERATIONS as f64
    );
    println!(
        "  Store: {:?} ({:.2} ns/op)",
        store_duration,
        store_duration.as_nanos() as f64 / NUM_OPERATIONS as f64
    );
    println!(
        "  RMW:   {:?} ({:.2} ns/op)",
        rmw_duration,
        rmw_duration.as_nanos() as f64 / NUM_OPERATIONS as f64
    );
