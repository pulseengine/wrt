...
use crate::formal_verification::test_registry::TestRegistry;
use crate::formal_verification::utils::{bounded_vec, BoundedVec};
...
pub mod hardware_error_detection {
    ...
    impl ControlFlowMonitor {
        ...
        pub fn record_step(&mut self, step: u32) -> Result<(), &'static str> {
            self.actual_sequence.push(step)
                .map_err(|_| "Actual sequence too long")?;
        ...
        pub fn verify_complete(&self) -> bool {
            self.actual_sequence.len() == self.expected_sequence.len() &&
            self.violations == 0
        }
    }
}
...
mod proofs {
    use super::*;
    use super::lockstep_execution::*;
    use super::redundant_computation::*;
    use super::hardware_error_detection::*;
