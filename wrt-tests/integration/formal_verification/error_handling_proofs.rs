...
pub fn verify_asil_error_classification() {
    ...
    let chain_error =
        Error::system_error("Chained error", AsilLevel::QM).with_source(error.clone());
...
pub fn verify_graceful_degradation() {
    ...
    match failure_mode {
            ...
            loop {
                ...
                match safe_managed_alloc!(4096, CrateId::Component) {
                    ...
                    Err(err) => {
                        ...
                        match minimal_alloc {
                            Ok(_) | Err(_) => {
                                // Either outcome is acceptable for minimal
                                // allocation
                            },
                        }
                        break;
        ...
        1 => {
            ...
            match safe_managed_alloc!(1024, invalid_crate_id) {
                ...
                Err(capability_error) => {
                    ...
                    assert!(
                        capability_error.is_recoverable() || capability_error.is_safety_critical()
                    );
                },
            }
        },
        ...
        _ => {
            ...
            let secondary_error =
                Error::runtime_execution_error("Secondary failure", AsilLevel::AsilA)
...
pub fn register_tests(registry: &TestRegistry) -> TestResult {
    ...
    registry.register_test("error_propagation_basic", || {
        ...
        let secondary =
            Error::runtime_execution_error("Secondary", AsilLevel::AsilA).with_source(primary);
...
pub fn property_count() -> usize {
    4 // verify_asil_error_classification, verify_error_propagation,
      // verify_graceful_degradation, verify_error_recovery
}
...
mod tests {
    ...
    fn test_error_chaining() {
        ...
        let chained =
            Error::runtime_execution_error("Surface error", AsilLevel::AsilA).with_source(root);
