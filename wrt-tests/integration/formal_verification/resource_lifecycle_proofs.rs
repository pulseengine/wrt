...
impl Resource {
    pub fn new(
        id: u32,
        repr: ResourceRepr,
        _name: Option<WasmName<MAX_WASM_NAME_LENGTH, NoStdProvider<4096>>>,
        verification_level: VerificationLevel,
    ) -> Self {
        Self {
            id,
...
/// component boundaries, preventing resource confusion and isolation
/// violations.
///
/// # Verified Properties
///
...
pub fn verify_resource_lifecycle_correctness() {
    ...
    assert_eq!(
        mutable_resource.verification_level(),
...
pub fn verify_resource_table_bounds() {
    ...
    let safe_increment = if table_idx.0 < u32::MAX - 1 { table_idx.0 + 1 } else { table_idx.0 };
...
pub fn property_count() -> usize {
    6 // verify_resource_id_uniqueness, verify_resource_lifecycle_correctness,
      // verify_resource_table_bounds, verify_cross_component_isolation,
