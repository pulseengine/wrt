...
use crate::utils::{
    ...
    any_crate_id,
    any_memory_size,
    MAX_VERIFICATION_ALLOCATIONS,
    MAX_VERIFICATION_MEMORY,
};
...
pub fn verify_end_to_end_safety_preservation() {
    ...
    for _op in 0..operation_count {
        ...
        let effective_level1 =
            if required_asil.safety_criticality() > component1_asil.safety_criticality() {
                required_asil
            } else {
                component1_asil
            };

        let effective_level2 =
            if required_asil.safety_criticality() > component2_asil.safety_criticality() {
                required_asil
            } else {
                component2_asil
            };
    ...
    assert_eq!(
        safety_context1.compile_time_asil.safety_criticality(),
        component1_asil.safety_criticality()
    );
    assert_eq!(
        safety_context2.compile_time_asil.safety_criticality(),
        component2_asil.safety_criticality()
    );
}
...
pub fn property_count() -> usize {
    7 // verify_cross_component_memory_isolation,
