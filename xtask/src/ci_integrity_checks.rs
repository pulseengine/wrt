...
fn get_rust_version_from_toolchain_file() -> Result<(String, Option<String>, String)> {
    ...
    let rust_image_spec = match (version_opt.as_ref(), channel.as_str()) {
        ...
        (None, "stable") => {
            bail!("'version' must be specified in rust-toolchain.toml for the 'stable' channel.")
        }
        (None, "nightly") => "rust:nightly".to_string(),
        (None, "beta") => "rust:beta".to_string(),
        (None, other_channel) => bail!(
            "Unsupported channel '{}' without a specific version in rust-toolchain.toml. Please \
             specify a version or use 'stable', 'nightly', or 'beta'.",
            other_channel
        ),
    };
...
// This is a simplified check; a more robust solution might involve walking
// specific crate types.
...
pub async fn run(client: &Query) -> Result<()> {
    ...
    let toolchain_to_set: Option<String> =
        match (&toolchain_version_opt, toolchain_channel.as_str()) {
            (Some(v), _) => Some(v.clone()), // If version is specified, use it. E.g., "1.78.0",
            // "nightly-2023-10-25"
            (None, "stable") => {
                // This case should have been caught by get_rust_version_from_toolchain_file,
                // which bails. If it somehow reaches here, it implies an issue.
                // For safety, bail or log error.
                // However, get_rust_version_from_toolchain_file ensures version is Some for
                // stable or bails. So, for stable, toolchain_version_opt will
                // be Some(v).
                None // Should not happen for stable due to earlier checks
            }
            (None, non_stable_channel) => Some(non_stable_channel.to_string()), /* e.g. "nightly"
                                                                                 * , "beta" */
        };
    ...
    if let Some(ref ts) = toolchain_to_set {
        ...
        // This case should be prevented by get_rust_version_from_toolchain_file for
        // stable channel
        bail!(
            "Logic error: Stable channel reached toolchain setup without a specific version. This \
    ...
    // Ensure previous setup steps (like rustup default) are completed before
    // checks. The `container` object has the rustup commands defined.
    // We sync it here to ensure those are processed before we try to read `rustc
    // --version` etc. This sync doesn't change the 'container' variable itself
    // if it returns an ID, so 'container' remains the Container object we use
    ...
    let rustc_version_stdout = if let Some(ref toolchain_name) = toolchain_to_set {
        ...
        // Fallback if toolchain_to_set was None (should not happen for stable with
        // version)
        container
    ...
    if active_toolchain_stdout.contains("rustup not found") {
        ...
        if toolchain_channel == "stable" {
            if let Some(version_str) = &toolchain_version_opt {
                    ...
                    && !rustc_version_stdout.contains("beta")
                {
                    ...
                    failure_reason = format!(
                        "rustc --version ('{}') mismatch. Expected stable version '{}', without \
                         'nightly' or 'beta'.",
                        rustc_version_stdout.trim(),
                        version_str
                    );
                ...
                // This case should ideally be caught by get_rust_version_from_toolchain_file
                // for stable
                failure_reason = "Stable channel specified in toolchain file, but no version was \
            ...
            if rustc_version_stdout.contains(&toolchain_channel) {
                ...
                // If a specific version (like a date for nightly) is also in
                // toolchain_version_opt, check it too.
                if let Some(version_str) = &toolchain_version_opt {
                    if rustc_version_stdout.contains(version_str) {
                        checks_passed = true;
                        info!(
                            "rustc --version ('{}') contains expected channel '{}' and version \
                             '{}'.",
                            rustc_version_stdout.trim(),
                            toolchain_channel,
                            version_str
                        ...
                        // It contains the channel, but not the specific version string from
                        // rust-toolchain.toml This might be acceptable for
                        // some nightly setups (e.g., toolchain file has date, but rustc --version
                        // doesn't show it) For now, consider it a pass if
                        // channel matches, but log a detailed message.
                        checks_passed = true; // Or false, depending on strictness desired. Let's keep current behavior.
                        info!(
                            "rustc --version ('{}') contains expected channel '{}', but not the \
                             specific version string '{}' from toolchain file. Proceeding as \
                             channel match is primary.",
                            rustc_version_stdout.trim(),
                            toolchain_channel,
                            version_str
                        );
                    ...
                    checks_passed = true;
                    info!(
                        "rustc --version ('{}') contains expected channel '{}' (no specific \
                         version in toolchain file).",
                        rustc_version_stdout.trim(),
                        toolchain_channel
                    );
                }
        ...
        if toolchain_channel == "stable" {
            ...
            } // If version_str is None for stable, it's an error state, handled
              // by bail earlier or leading to false here.
        } else {
                ...
                && rustc_version_stdout.contains(&toolchain_channel)
            {
                ...
                // If a specific version (like a date for nightly) was provided, log if not
                // explicitly in version strings.
                if let Some(version_str) = toolchain_version_opt
                    .as_ref()
                    .filter(|v| toolchain_channel == "nightly" && v.contains('-'))
                {
                    ...
                    {
                        info!(
                            "Note: rustup/rustc matches nightly channel '{}'. The specific date \
                             '{}' from toolchain file was not found in rustc version ('{}') or \
                             rustup output ('{}'). This may be acceptable if the installed \
                             nightly is the correct one without the exact date in the version \
                             string.",
                            toolchain_channel,
                            version_str,
                            rustc_version_stdout.trim(),
        ...
        if rustup_confirms_channel_and_version {
            checks_passed = true;
            info!(
                "rustup active toolchain ('{}') and rustc --version ('{}') match expected channel \
                 '{}' and version {:?}.",
                active_toolchain_stdout.trim(),
                rustc_version_stdout.trim(),
                toolchain_channel,
    ...
    //   Consider using a dedicated tool if available, or careful regex/string
    // matching.
    info!("TODO: Implement license header checks.");

    // Final sync to ensure all checks within the container (if any were execs) are
    // done. For checks done on host like file presence, this is less critical
    // for the Dagger part.
    let _ = container
        .sync()
        .await
