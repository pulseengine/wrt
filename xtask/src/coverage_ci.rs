    ...
    let container = client
        ...
        .with_exec(vec![
            ...
            "apt-get update -y && \
             ...
             cargo install cargo-llvm-cov --version 0.5.31 --locked"
        ])
        ...
        .with_exec(vec![
            "sh", "-c",
            "cargo llvm-cov test --package wrt-error --json --output-path /src/target/coverage/coverage.json || \
             echo '{\"version\":\"0.1.0\",\"coverage_type\":\"ci\",\"summary\":{\"line\":{\"count\":0,\"covered\":0,\"percentage\":0.0}},\"files\":[]}' > /src/target/coverage/coverage.json"
        ]);
