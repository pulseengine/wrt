...
fn test_asil_level_detection() {
    ...
    let val_error = Error::new(ErrorCategory::Validation, codes::VALIDATION_ERROR, "Validation error");
...
fn test_safe_state_requirements() {
    ...
    let memory_error = Error::new(ErrorCategory::Memory, codes::MEMORY_CORRUPTION_DETECTED, "Memory corruption");
    assert!(memory_error.requires_safe_state());
    
    // Runtime trap errors require safe state
    let trap_error = Error::new(ErrorCategory::RuntimeTrap, codes::RUNTIME_TRAP_ERROR, "Trap occurred");
    assert!(trap_error.requires_safe_state());
    
    // Component errors do not require immediate safe state
    let comp_error = Error::new(ErrorCategory::Component, codes::COMPONENT_LINKING_ERROR, "Component issue");
