...
pub fn run_fuzz_with_options_impl(build_system: &BuildSystem, options: &FuzzOptions) -> BuildResult<FuzzResults> {
    ...
    let targets_to_run = if options.targets.is_empty() {
        all_targets
    } else {
    ...
    // Generate summary
    if success {
        println!("{} Fuzzing campaign completed successfully!", "✅".bright_green());
    } else {
        println!(
...
fn run_single_fuzz_target_impl(build_system: &BuildSystem, target: &str, options: &FuzzOptions) -> BuildResult<bool> {
...
fn generate_fuzz_report_impl(
    ...
    for target in targets_run {
        let status = if crashed_targets.contains(target) {
            "❌ ISSUES FOUND"
        } else {
            "✅ CLEAN"
        };
    ...
    if !crashed_targets.is_empty() {
        ...
        for target in crashed_targets {
            report.push_str(&format!("### {}\n", target));
            report.push_str("Crashes or timeouts detected. Check artifacts directory for details.\n\n");
        }
    }
