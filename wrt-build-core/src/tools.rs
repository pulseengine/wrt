...
use crate::tool_versions::{ToolVersionConfig, VersionComparison, extract_version_from_output};
...
impl ToolManager {
    ...
    pub fn new() -> Self {
        ...
        tools.insert("cargo".to_string(), ToolInfo {
            name: "cargo".to_string(),
            description: "Rust package manager".to_string(),
            install_command: "Install Rust from https://rustup.rs/".to_string(),
            required: true,
            used_by: vec!["build", "test", "check", "clean"].into_iter().map(String::from).collect(),
        });
        
        tools.insert("rustc".to_string(), ToolInfo {
        ...
        tools.insert("rustfmt".to_string(), ToolInfo {
        ...
        tools.insert("kani".to_string(), ToolInfo {
            name: "kani".to_string(),
            description: "Formal verification tool for Rust".to_string(),
            install_command: "cargo install --locked kani-verifier && cargo kani setup".to_string(),
            required: false,
            used_by: vec!["kani-verify", "verify"].into_iter().map(String::from).collect(),
        });
        
        tools.insert("cargo-fuzz".to_string(), ToolInfo {
        ...
        tools.insert("git".to_string(), ToolInfo {
            name: "git".to_string(),
        ...
        // Documentation tools
        tools.insert("python3".to_string(), ToolInfo {
        ...
        tools.insert("python-venv".to_string(), ToolInfo {
    ...
    pub fn check_tool_for_target(&self, tool_name: &str, target: Option<&str>) -> ToolStatus {
        ...
        if let Some(target) = target {
            if !self.version_config.is_target_supported(tool_name, target) {
                return ToolStatus {
                    available: false,
                    version: None,
                    error: Some(format!("Target '{}' not supported for tool '{}'", target, tool_name)),
        ...
        let basic_status = match tool_name {
            ...
            "riscv32imac-unknown-none-elf" => self.check_rustup_target("riscv32imac-unknown-none-elf"),
    ...
    fn enhance_with_version_check_for_target(&self, tool_name: &str, target: Option<&str>, mut status: ToolStatus) -> ToolStatus {
        if !status.available {
            return status;
        }
            ...
            Some(version) => {
                match self.version_config.check_version_compatibility(tool_name, &version) {
                    Some(VersionComparison::Satisfies) => {
                        (VersionStatus::Compatible, false)
                    },
                    Some(VersionComparison::TooOld { installed, required }) => {
                        (VersionStatus::TooOld { installed, required }, true)
                    },
                    Some(VersionComparison::Newer { installed, required }) => {
                        (VersionStatus::Newer { installed, required }, false)
                    },
                    Some(VersionComparison::Mismatch { installed, required }) => {
                        (VersionStatus::Mismatch { installed, required }, true)
                    },
    ...
    pub fn generate_missing_tool_error(&self, tool_name: &str, command: &str) -> BuildError {
        if let Some(tool_info) = self.tools.get(tool_name) {
            ...
            BuildError::Tool(format!("Unknown tool '{}' required for command '{}'", tool_name, command))
        }
    }
        ...
        for (tool_name, tool_info) in &self.tools {
            if !tool_info.required {
                if let Some(status) = results.get(tool_name) {
                    ...
                    if !status.available {
                        println!("      💿 Install: {}", tool_info.install_command.bright_green());
                        println!("      📋 Used by: {}", tool_info.used_by.join(", ").bright_magenta());
                    }
                }
            }
        }
    ...
    fn format_version_status(&self, status: &ToolStatus) -> (String, String) {
        match (&status.version, &status.version_status) {
            ...
            (Some(version), VersionStatus::TooOld { installed: _, required }) => {
                (format!(" ({})", version), format!(" ⚠️ → {}", required).bright_yellow().to_string())
            },
            (Some(version), VersionStatus::Newer { installed: _, required }) => {
                (format!(" ({})", version), format!(" ⬆️ (need {})", required).bright_blue().to_string())
            },
            (Some(version), VersionStatus::Mismatch { installed: _, required }) => {
                (format!(" ({})", version), format!(" ❌ → {}", required).bright_red().to_string())
            },
    ...
    pub fn install_tool_if_needed(&self, tool_name: &str) -> BuildResult<bool> {
        ...
        if status.available && !status.needs_action {
            match &status.version_status {
                VersionStatus::Compatible => {
                    println!("  ✅ {} is already installed with compatible version", tool_name.bright_cyan());
                    return Ok(false); // No action needed
                },
                VersionStatus::Newer { installed, required } => {
                    println!("  ⚠️  {} has newer version {} (required: {})", 
                        tool_name.bright_cyan(), 
        ...
        let install_cmd = match self.version_config.get_install_command(tool_name) {
            ...
            None => {
                return Err(BuildError::Tool(format!(
                    "No installation command configured for tool '{}'", tool_name
                )));
            }
        };
        
        match &status.version_status {
            VersionStatus::TooOld { installed, required } => {
            ...
            VersionStatus::Mismatch { installed, required } => {
                println!("  🔄 Replacing {} version {} with {}", 
                    tool_name.bright_cyan(), 
                    installed.bright_red(), 
                    required.bright_green()
                );
            ...
            _ => {
                println!("  📦 Installing {} for {}", tool_name.bright_cyan(), "build system functionality".bright_blue());
            }
        }
        ...
        if install_cmd.starts_with("cargo install") {
            ...
            println!("    💡 Manual installation required: {}", install_cmd.bright_yellow());
            Ok(false)
        }
    }
        ...
        if output.status.success() {
            ...
            if tool_name == "kani" && install_cmd.contains("kani setup") {
                ...
                if setup_output.status.success() {
                    ...
                    println!("    ⚠️  Kani setup had issues: {}", 
            ...
            Err(BuildError::Tool(format!(
                "Failed to install {}: {}", tool_name, error_msg
            )))
        }
    }
    ...
    fn execute_rustup_command(&self, install_cmd: &str) -> BuildResult<bool> {
        ...
        if output.status.success() {
            ...
            let error_msg = String::from_utf8_lossy(&output.stderr);
            Err(BuildError::Tool(format!(
                "Failed to execute rustup command: {}", error_msg
            )))
        }
    }
