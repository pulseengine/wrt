...
use wrt_math::{SafeArithmetic, SafeFloat, SafeRounding, RoundingMode, check_simd_bounds};
...
fn demo_integer_safety() {
    ...
    match max_val.safe_add(1) {
        Ok(result) => {
            ...
            println!("Overflow handled with saturation: {} + 1 = {}", max_val, result);
...
fn demo_simd_bounds() {
    ...
    match check_simd_bounds(0, 64, memory_size) {
        Ok(()) => println!("Valid SIMD access: offset=0, len=64, memory_size={}", memory_size),
        Err(e) => println!("SIMD bounds check failed: {:?}", e),
    }
    
    // Out of bounds access
    match check_simd_bounds(1000, 64, memory_size) {
        Ok(()) => println!("SIMD access allowed: offset=1000, len=64, memory_size={}", memory_size),
        Err(e) => {
