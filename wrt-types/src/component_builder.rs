...
//!
//! This module provides builders for complex types in the WebAssembly Component Model,
//! ensuring proper initialization, validation, and resource management.
...
use crate::{
    ...
    component::{
        ComponentAlias, ComponentInstance, ComponentType, CoreInstance, CoreType, Export, ExternType,
        Import, ImportKey, MAX_COMPONENT_ALIASES, MAX_COMPONENT_EXPORTS, MAX_COMPONENT_IMPORTS,
        MAX_COMPONENT_INSTANCES, MAX_COMPONENT_TYPES, MAX_CORE_INSTANCES, MAX_CORE_TYPES,
        MAX_NAME_LEN, MAX_NAMESPACE_ELEMENTS, Namespace, ResourceType,
    },
...
impl<P: MemoryProvider + Default + Clone + PartialEq + Eq> ComponentTypeBuilder<P> {
    ...
    pub fn with_instances(mut self, instances: impl IntoIterator<Item = ComponentInstance<P>>) -> Self {
    ...
    pub fn with_core_instances(mut self, core_instances: impl IntoIterator<Item = CoreInstance<P>>) -> Self {
    ...
    pub fn with_component_types(mut self, component_types: impl IntoIterator<Item = TypeRef>) -> Self {
...
impl<P: MemoryProvider + Default + Clone + PartialEq + Eq> ImportBuilder<P> {
    ...
    pub fn build(self) -> WrtResult<Import<P>> {
        let namespace = self.namespace.ok_or_else(|| {
            Error::validation_error("Import namespace is required")
        })?;

        let name = self.name.ok_or_else(|| {
            Error::validation_error("Import name is required")
        })?;

        let ty = self.ty.ok_or_else(|| {
            Error::validation_error("Import type is required")
        })?;
...
impl<P: MemoryProvider + Default + Clone + PartialEq + Eq> ExportBuilder<P> {
    ...
    pub fn build(self) -> WrtResult<Export<P>> {
        let name = self.name.ok_or_else(|| {
            Error::validation_error("Export name is required")
        })?;

        let ty = self.ty.ok_or_else(|| {
            Error::validation_error("Export type is required")
        })?;
...
impl<P: MemoryProvider + Default + Clone + PartialEq + Eq> ResourceTypeBuilder<P> {
    ...
    pub fn as_record(
        mut self,
        field_names: Vec<BoundedString<MAX_WASM_NAME_LENGTH, P>>,
    ) -> Self {
    ...
    pub fn build(self) -> WrtResult<ResourceType<P>> {
        let variant = self.variant.ok_or_else(|| {
            Error::validation_error("Resource type variant must be specified")
        })?;
...
mod tests {
    ...
    fn test_component_type_builder() {
        ...
        let component_type = ComponentTypeBuilder::new()
    ...
    fn test_namespace_builder() {
        ...
        let namespace_builder = NamespaceBuilder::new()
            .with_provider(provider.clone())
            .with_element_str("wasm").unwrap()
            .with_element_str("component").unwrap();
