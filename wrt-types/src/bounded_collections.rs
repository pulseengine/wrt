...
// Crate-level imports
use crate::{
    bounded::{BoundedVec, BoundedError, BoundedErrorKind, CapacityError},
    validation::BoundedCapacity,
    safe_memory::{SafeMemoryHandler, NoStdProvider},
    verification::{Checksum, VerificationLevel},
    traits::{Checksummable, FromBytes, ToBytes, ReadStream, WriteStream, SerializationError},
    ...
    P: Default + Clone,
{
    ...
    pub fn enqueue(&mut self, item: T) -> Result<(), BoundedError> {
        ...
        if item_size > item_bytes_buffer.len() {
            return Err(BoundedError::new(BoundedErrorKind::ItemTooLarge, "Item exceeds max buffer size for enqueue"));
        }
        ...
        self.handler
            ...
            .map_err(|e| BoundedError::new(
                ...
                "Failed to write data for enqueue"
            ))?;
    ...
    pub fn dequeue(&mut self) -> Result<Option<T>, BoundedError> {
        ...
        let slice_view = self.handler
            .get_slice(offset, self.item_serialized_size)
            .map_err(|e| BoundedError::new(
                BoundedErrorKind::SliceError, 
                "Failed to get slice for dequeue"
            ))?;

        // Deserialize the item
        let (item, _) = T::from_bytes(slice_view.as_ref())
            .map_err(|_| BoundedError::new(
                BoundedErrorKind::ConversionError, 
...
///
/// This implements a key-value store that ensures it never exceeds the specified
/// capacity N_ELEMENTS. It uses a MemoryProvider for storing serialized elements.
#[derive(Debug)]
...
///
/// This implements a collection of unique elements that ensures it never exceeds
/// the specified capacity N_ELEMENTS. It uses a MemoryProvider for storing
/// serialized elements.
#[derive(Debug)]
    ...
    P: Default + Clone + PartialEq + Eq,
{
    ...
    /// Returns an error if the set is full and the element is not already present.
...
///
/// This implements a deque that allows adding and removing elements from both ends.
/// It ensures it never exceeds the specified capacity N_ELEMENTS.
    ...
    P: Default + Clone,
{
    ...
    pub fn push_front(&mut self, item: T) -> Result<(), BoundedError> {
        ...
        if item_size > item_bytes_buffer.len() {
            return Err(BoundedError::new(BoundedErrorKind::ItemTooLarge, "Item exceeds max buffer size for push_front"));
        }
        ...
        self.handler
            ...
            .map_err(|e| BoundedError::new(
                BoundedErrorKind::SliceError, 
                "Failed to write data for push_front"
            ))?;
    ...
    pub fn push_back(&mut self, item: T) -> Result<(), BoundedError> {
        ...
        if item_size > item_bytes_buffer.len() {
            return Err(BoundedError::new(BoundedErrorKind::ItemTooLarge, "Item exceeds max buffer size for push_back"));
        }
        ...
        self.handler
            ...
            .map_err(|e| BoundedError::new(
                BoundedErrorKind::SliceError, 
                "Failed to write data for push_back"
            ))?;
    ...
    pub fn pop_back(&mut self) -> Result<Option<T>, BoundedError> {
        ...
        let physical_index = if self.back == 0 {
            N_ELEMENTS - 1
        } else {
            self.back - 1
        };
...
impl<const N_BITS: usize> BoundedBitSet<N_BITS> {
    ...
    ///
    /// Returns `true` if the bit is now set (1), `false` if it is now clear (0).
    /// Returns an error if the index is out of bounds.
    ...
    pub fn set_all(&mut self) {
        ...
        // Calculate the exact count (handling the case where the last chunk is partially used)
        ...
        if remaining_bits > 0 {
            ...
            if self.verification_level >= VerificationLevel::Full {
                ...
                self.storage[last_index].0.update_checksum(&mut self.storage[last_index].1);
            }
        }
    ...
    pub fn verify_checksum(&self) -> bool {
        ...
        let calculated_count = self.storage.iter()
            .map(|(bits, _)| bits.count_ones() as usize)
            .sum::<usize>();
    ...
    ///
    /// Modifies this bitset in-place to contain only the bits that are set in both bitsets.
    ///
    /// # Examples
    ///
    /// ```
    /// use wrt_types::BoundedBitSet;
    /// 
    ...
    ///
    /// Modifies this bitset in-place to contain bits that are set in either bitset.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// Modifies this bitset in-place to contain bits that are set in either bitset but not both.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// Modifies this bitset in-place to contain the complement of its current state.
    /// This inverts every bit in the valid range (0 to N_BITS-1).
    ///
    /// # Examples
    ///
    /// ```
        ...
        for (i, (bits, checksum)) in self.storage.iter_mut().enumerate() {
            if i < full_chunks {
                ...
                *bits = !*bits;
                ...
                // For the last chunk, only invert valid bits (mask with ones up to remaining_bits)
                let mask = (1u32 << remaining_bits) - 1;
                *bits = (*bits ^ mask) & mask; // XOR with mask and keep only valid bits
            }
    ...
    /// Returns the index of the first clear bit (0), or None if all bits are set.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Returns the number of bits that were newly set (excluding bits that were already set).
    ...
    /// Returns the number of bits that were newly cleared (excluding bits that were already clear).
    /// Returns an error if any index is out of bounds.
    ///
    /// # Examples
    ///
    ...
    ///
    /// Sets all bits in the range [start_index, end_index) to the specified value.
    /// Returns the number of bits that were changed.
    /// Returns an error if either index is out of bounds or if end_index <= start_index.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// }
    /// ```
    pub fn set_range(&mut self, start_index: usize, end_index: usize, value: bool) -> Result<usize, BoundedError> {
    ...
    /// Returns the position of the lowest set bit in the range [start_index, end_index).
    ///
    /// Returns `None` if no bits are set in the range or if the range is invalid.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// ```
    pub fn lowest_set_bit_in_range(&self, start_index: usize, end_index: usize) -> Result<Option<usize>, BoundedError> {
        if start_index >= N_BITS || end_index > N_BITS || start_index >= end_index {
            return Err(BoundedError::new(
                BoundedErrorKind::SliceError,
    ...
    /// Returns the position of the highest set bit in the range [start_index, end_index).
    ///
    /// Returns `None` if no bits are set in the range or if the range is invalid.
    ///
    /// # Examples
    ...
    /// ```
    pub fn highest_set_bit_in_range(&self, start_index: usize, end_index: usize) -> Result<Option<usize>, BoundedError> {
        if start_index >= N_BITS || end_index > N_BITS || start_index >= end_index {
            return Err(BoundedError::new(
                BoundedErrorKind::SliceError,
    ...
    /// Returns the number of leading zeros (unset bits) at the end of the bitset.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// Returns the starting index of the first such sequence found, or None if no
    /// such sequence exists. This is useful for finding available space in a bitmap.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Counts the number of bits set to 1 in the range [start_index, end_index).
    ///
    ...
    /// ```
    pub fn count_bits_in_range(&self, start_index: usize, end_index: usize) -> Result<usize, BoundedError> {
        if start_index >= N_BITS || end_index > N_BITS || start_index >= end_index {
            return Err(BoundedError::new(
                BoundedErrorKind::SliceError,
    ...
    ///
    /// Returns `true` if all bits set in this bitset are also set in the other bitset.
    ///
    /// # Examples
    ///
    /// ```
...
impl<T, const N_ELEMENTS: usize, P: MemoryProvider> BoundedCapacity for BoundedQueue<T, N_ELEMENTS, P>
where
    T: Sized + Checksummable + ToBytes + FromBytes + Default,
{
    fn capacity(&self) -> usize {
...
impl<K, V, const N_ELEMENTS: usize, P: MemoryProvider> BoundedCapacity for BoundedMap<K, V, N_ELEMENTS, P>
where
    K: Sized + Checksummable + ToBytes + FromBytes + Default + Eq + Clone + PartialEq,
    V: Sized + Checksummable + ToBytes + FromBytes + Default + Clone + PartialEq + Eq,
...
impl<T, const N_ELEMENTS: usize, P: MemoryProvider> BoundedCapacity for BoundedDeque<T, N_ELEMENTS, P>
where
    T: Sized + Checksummable + ToBytes + FromBytes + Default,
{
    fn capacity(&self) -> usize {
...
impl<'a, const N_BITS: usize> Iterator for BitSetOnesIterator<'a, N_BITS> {
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        let remaining = self.bitset.count.saturating_sub(
            self.bitset.count_bits_in_range(0, self.next_index).unwrap_or(0)
        );
...
impl<'a, const N_BITS: usize> Iterator for BitSetZerosIterator<'a, N_BITS> {
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        // We know the exact number of remaining elements
        let ones_remaining = self.bitset.count.saturating_sub(
            self.bitset.count_bits_in_range(0, self.next_index).unwrap_or(0)
        );
...
impl<const N_BITS: usize> FromBytes for BoundedBitSet<N_BITS> {
    fn from_bytes_with_provider<'a, P: crate::MemoryProvider>(
        ...
        let verification_level = VerificationLevel::from_bytes_with_provider(reader, stream_provider)?;
        ...
        let calculated_count = bitset.storage.iter()
            .map(|(bits, _)| bits.count_ones() as usize)
            .sum::<usize>();
