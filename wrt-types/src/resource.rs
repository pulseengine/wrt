...
use crate::bounded::{WasmName, BoundedVec, BoundedString, MAX_WASM_NAME_LENGTH};
use crate::MemoryProvider;
use crate::verification::Checksum;
use crate::traits::{Checksummable, FromBytes, ToBytes, SerializationError, ReadStream, WriteStream};
...
use crate::prelude::{str, Eq, PartialEq};

// use crate::prelude::{format, ToString, String as PreludeString, Vec as PreludeVec};
// use crate::prelude::{Debug, Display, Eq, PartialEq};
...
pub enum ResourceRepresentation {
    ...
    #[cfg(feature = "alloc")]
    Record(BoundedVec<BoundedString<MAX_RESOURCE_FIELD_NAME_LEN, NoStdProvider>, MAX_RESOURCE_FIELDS, NoStdProvider>),
            ...
            "aggregate" => {
                ...
                #[cfg(not(feature = "alloc"))]
                {
                    Err(wrt_error::Error::new(
                        wrt_error::ErrorCategory::Parse,
                        wrt_error::codes::PARSE_ERROR,
                        "ResourceRepresentation::Aggregate is not available without the 'alloc' feature",
                    ))
                }
            }
            _ => Err(wrt_error::Error::new(
...
pub enum ResourceRepr<P: MemoryProvider + Default + Clone + Eq + Debug> {
    ...
    List(ValueType),
    /// The resource is an aggregate of other resource IDs (implementation-defined).
...
impl<P: MemoryProvider + Default + Clone + Eq + Debug> FromBytes for ResourceRepr<P> {
    fn from_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        ...
        match tag {
            ...
            1 => {
                let fields = BoundedVec::<BoundedString<MAX_RESOURCE_FIELD_NAME_LEN, P>, MAX_RESOURCE_FIELDS, P>::from_bytes_with_provider(reader, stream_provider)
            ...
            3 => {
                let ids = BoundedVec::<u32, MAX_RESOURCE_AGGREGATE_IDS, P>::from_bytes_with_provider(reader, stream_provider)?;
...
impl<P: MemoryProvider + Default + Clone + Eq + Debug> FromBytes for Resource<P> {
    fn from_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        ...
        let name =
            Option::<WasmName<MAX_WASM_NAME_LENGTH, P>>::from_bytes_with_provider(reader, stream_provider)?;
        let verification_level = VerificationLevel::from_bytes_with_provider(reader, stream_provider)?; // VerificationLevel is simple
        Ok(Resource {
            id,
            repr,
            name,
...
/// Represents the type of a resource, which can be a record or an aggregate (handle to other resources).
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
...
/// Represents a single resource item in the store, including its ID, optional name, and type.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
...
impl ToBytes for ResourceTableIdx {
    fn to_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        ...
        self.0.to_bytes_with_provider(writer, _provider) // u32's to_bytes doesn't take provider
    }
}
    ...
    fn from_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        ...
        match tag {
            DISCRIMINANT_RESOURCE_TYPE_RECORD => {
                ...
                let fields =
                    BoundedVec::<BoundedString<MAX_RESOURCE_FIELD_NAME_LEN, P>, MAX_RESOURCE_FIELDS, P>::from_bytes_with_provider(
                        reader,
                        stream_provider,
                    )
                    .map_err(wrt_error::Error::from)?;
            ...
            DISCRIMINANT_RESOURCE_TYPE_AGGREGATE => {
                ...
                let ids = BoundedVec::<u32, MAX_RESOURCE_AGGREGATE_IDS, P>::from_bytes_with_provider(
                    reader, 
                    stream_provider
                )?;
