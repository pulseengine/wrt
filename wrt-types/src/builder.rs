...
// Crate-level imports
use crate::{
    bounded::{BoundedVec, BoundedStack, BoundedString, WasmName},
    resource::{ResourceRepresentation, Resource, ResourceRepr, ResourceId, ResourceOperation, ResourceType, ResourceItem, MAX_RESOURCE_FIELDS, MAX_RESOURCE_AGGREGATE_IDS, MAX_RESOURCE_FIELD_NAME_LEN},
    safe_memory::{NoStdProvider, SafeMemoryHandler, DEFAULT_MEMORY_PROVIDER_CAPACITY, DefaultNoStdProvider},
...
///
/// This builder simplifies the creation and configuration of bounded collections
/// like BoundedVec, BoundedStack, etc. with proper resource management for
...
impl<const N: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> StringBuilder<N, P> {
    ...
    pub fn build_string(self) -> WrtResult<BoundedString<N, P>> {
        match (self.initial_content, self.truncate_if_needed) {
            (Some(content), true) => {
                BoundedString::from_str_truncate(content, self.provider)
                    .map_err(Error::from)
            },
            (Some(content), false) => {
                BoundedString::from_str(content, self.provider)
                    .map_err(Error::from)
            },
            (None, _) => {
                BoundedString::from_str_truncate("", self.provider)
                    .map_err(Error::from)
            }
        }
    }
    ...
    pub fn build_wasm_name(self) -> WrtResult<WasmName<N, P>> {
        match (self.initial_content, self.truncate_if_needed) {
            (Some(content), true) => {
                WasmName::from_str_truncate(content, self.provider)
                    .map_err(Error::from)
            },
            (Some(content), false) => {
                WasmName::from_str(content, self.provider)
                    .map_err(Error::from)
            },
            (None, _) => {
                WasmName::new(self.provider)
                    .map_err(Error::from)
            }
        }
    }
...
impl<P: MemoryProvider + Default + Clone + Eq + fmt::Debug> ResourceBuilder<P> {
    ...
    pub fn build(self) -> WrtResult<Resource<P>> {
        let id = self.id.ok_or_else(|| Error::new_static(
            codes::VALIDATION_ERROR, // Using VALIDATION_ERROR instead of missing MISSING_REQUIRED_VALUE
            "Resource ID is required"
        ))?;

        let repr = self.repr.ok_or_else(|| Error::new_static(
            codes::VALIDATION_ERROR, // Using VALIDATION_ERROR instead of missing MISSING_REQUIRED_VALUE
        ...
        let name = match self.name {
            Some(name_str) => {
                let wasm_name = WasmName::from_str_truncate(name_str, P::default())
                    .map_err(Error::from)?;
                Some(wasm_name)
            },
            None => None,
...
#[cfg(feature = "alloc")]
    ...
    pub fn as_record<S: AsRef<str>>(mut self, field_names: Vec<S>) -> Result<Self, Error> {
...
#[cfg(not(feature = "alloc"))]
    ...
    pub fn as_record<S: AsRef<str>>(mut self, field_name: S) -> Result<Self, Error> {
...
#[cfg(feature = "alloc")]
    /// Configures this as an Aggregate resource type with the given resource IDs.
...
#[cfg(not(feature = "alloc"))]
    /// Configures this as an Aggregate resource type with the given resource ID.
...
#[cfg(feature = "alloc")]
    ...
    pub fn build(self) -> Result<ResourceType<P>, Error> {
        let variant = self.variant.ok_or_else(|| Error::new_static(
...
impl<P: MemoryProvider + Default + Clone + Eq + fmt::Debug> ResourceItemBuilder<P> {
    ...
    pub fn build(self) -> Result<ResourceItem<P>, Error> {
        let id = self.id.ok_or_else(|| Error::new_static(
            codes::VALIDATION_ERROR,
            "Resource ID is required"
        ))?;

        let type_ = self.type_.ok_or_else(|| Error::new_static(
            codes::VALIDATION_ERROR,
...
impl<P: MemoryProvider + Default + Clone> MemoryBuilder<P> {
    ...
    pub fn build_safe_memory_handler(self) -> WrtResult<SafeMemoryHandler<P>> {
        ...
        // If a specific memory size requirement was provided, ensure it's properly handled
...
///
/// This builder allows creating a `NoStdProvider` with a specific capacity at compile time
/// through the const generic parameter N.
...
/// Backwards compatibility type for code using the old non-generic NoStdProviderBuilder
...
mod tests {
    ...
    fn test_string_builder() {
        let builder = StringBuilder::<256, NoStdProvider>::new()
            .with_content("test")
    ...
    fn test_resource_type_builder() {
        ...
        let resource_type = builder.as_record(vec!["field1", "field2"])
            .unwrap()
            .build()
            .unwrap();
        ...
        // Test Aggregate type
        let builder = ResourceTypeBuilder::<NoStdProvider>::new();
        let resource_type = builder.as_aggregate(vec![1, 2, 3])
            .build()
            .unwrap();
    ...
    fn test_no_std_provider_builder() {
        ...
        let builder = MediumNoStdProviderBuilder::new()
            .with_verification_level(VerificationLevel::Critical);
        ...
        let builder = SmallNoStdProviderBuilder::new()
            .with_init_size(1000); // Larger than 512 capacity
