...
use crate::{
    ...
    component_value::{
        ComponentValue, ValType, ValTypeRef, 
        MAX_STORED_COMPONENT_VALUES
    },
    
    // Bounded collections
    bounded::{
        BoundedVec, BoundedString, BoundedError,
        WasmName, MAX_WASM_NAME_LENGTH,
        MAX_COMPONENT_TUPLE_ITEMS, MAX_COMPONENT_FLAGS,
        MAX_COMPONENT_ERROR_CONTEXT_ITEMS, MAX_WASM_STRING_LENGTH
    },
    
    // Traits
    traits::{
        FromBytes, ToBytes, SerializationError, 
        BytesWriter, ReadStream, WriteStream,
        Checksummable
    },
...
#[cfg(feature = "alloc")]
use crate::prelude::{format, HashMap};

use crate::prelude::*;
use crate::validation::BoundedCapacity; // Added import
...
const MAX_STORE_VALUES: usize = 1024; // Example capacity
const MAX_STORE_TYPES: usize = 256; // Example capacity for types

// Capacity for the type_to_ref_map, should be related to MAX_STORE_TYPES
#[cfg(feature = "alloc")]
const MAX_TYPE_TO_REF_MAP_ENTRIES: usize = MAX_STORE_TYPES;
#[cfg(not(feature = "alloc"))]
const MAX_TYPE_TO_REF_MAP_ENTRIES: usize = MAX_STORE_TYPES; // Provide a default for no_alloc
...
impl<P: MemoryProvider + Default + Clone + PartialEq + Eq> ComponentValueStore<P> {
    /// Creates a new, empty `ComponentValueStore` with the given memory provider.
        ...
        match self.values.get(val_ref.index()) {
            Some(ComponentValue::String(s_name)) => {
                s_name.as_str()
                    ...
                    .map_err(|_utf8_err| { // _utf8_err is not boxed or passed as source
                        Error::new(
            ...
            Some(_other_val) => Err(Error::type_error(
                // format!("Expected ComponentValue::String, found {:?}", other_val) // format! requires alloc
                "Type mismatch: Expected ComponentValue::String"
            )),
            None => Err(Error::new(
                // format!("ValueRef {:?} not found in ComponentValueStore for get_string", val_ref) // format! requires alloc
    ...
    where P: Clone, // Needed for WasmName::from_str which takes P by value
    {
        // Create BoundedString with the target capacity for ComponentValue::String
        let bounded_s = BoundedString::<{crate::bounded::MAX_WASM_STRING_LENGTH}, P>::from_str(s, self.provider.clone())
          ...
          P: Clone, // Needed for WasmName::from_str
    {
        ...
        // let wasm_case_name = WasmName::<MAX_WASM_NAME_LENGTH, P>::from_str(case_name_str.as_ref(), self.provider.clone()).map_err(Error::from)?;
        // let component_value_payload = value.cloned(); // This would require ComponentValue to be Clone
        // let variant_cv = ComponentValue::Variant(wasm_case_name, component_value_payload.map(|cv| /* add cv to store, get ValueRef */ todo!() ));
        ...
        P: Clone,
    {
        ...
        for core_val in values {
            ...
            item_value_refs.push(value_ref).map_err(|_e: BoundedError| { // e is BoundedError
                Error::new(
                    ErrorCategory::Capacity, // Changed from Storage
                    codes::CAPACITY_EXCEEDED, // Changed from BOUNDED_OPERATION_FAILED
    ...
    where
        ...
        P: Clone, 
    {
        let mut flag_values = BoundedVec::<(WasmName<MAX_WASM_NAME_LENGTH, P>, bool), MAX_COMPONENT_FLAGS, P>::new(self.provider.clone()).map_err(|_e| Error::new(
            wrt_error::ErrorCategory::Memory,
            codes::MEMORY_ALLOCATION_ERROR,
            // #[cfg(any(feature = "std", feature = "alloc"))]
            // format!("Failed to create BoundedVec for flags: {:?}", e), // format! requires alloc
            // #[cfg(not(any(feature = "std", feature = "alloc")))]
            "Failed to create BoundedVec for flags",
        ))?;

        for (name_str, val) in flags {
            let name = WasmName::from_str(name_str.as_ref(), self.provider.clone()).map_err(|_e| Error::new(
                wrt_error::ErrorCategory::Validation,
                codes::INVALID_STATE, // Changed from INVALID_NAME
                "Invalid flag name",
            ))?;
            flag_values.push((name, val)).map_err(|_e| Error::new(
                wrt_error::ErrorCategory::Resource,
                codes::RESOURCE_LIMIT_EXCEEDED,
    ...
    where
        P: Clone, 
    {
        let name = WasmName::from_str(case.as_ref(), self.provider.clone()).map_err(|_e| Error::new(
            wrt_error::ErrorCategory::Validation,
            codes::INVALID_STATE, // Changed from INVALID_NAME
    ...
    pub fn intern_type(&mut self, ty: ValType<P>) -> Result<ValTypeRef> {
        ...
        self.types.push(ty.clone()).map_err(|e| { // ty needs to be Clone for this path
            Error::new(
        ...
        #[cfg(feature = "alloc")]
        {
            ...
            self.type_to_ref_map.insert(ty, type_ref).map_err(|e: BoundedError| {
                Error::new(
                    wrt_error::ErrorCategory::Resource, // Or Capacity
                    match e.kind {
                        crate::bounded::BoundedErrorKind::CapacityExceeded => codes::CAPACITY_EXCEEDED,
    ...
    pub fn resolve_type(&self, type_ref: ValTypeRef) -> Option<ValType<P>> {
        self.types.get(type_ref.0 as usize) // Assuming ValTypeRef.0 is the index
    }

    /// Returns a clone of the provider used by this store.
    /// Useful if external code needs to create P-dependent types like WasmName or BoundedVec
    /// that are compatible with this store's memory management.
    ...
    where P: Clone, // Assuming provider clone might be needed
    {
        match core_value {
            ...
            _ => Err(Error::new(
                ...
                "Unsupported core Value to ComponentValue conversion in add_tuple/core_value_to_component_value stub",
            )),
        }
    }
}
