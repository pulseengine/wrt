...
// Allow unused imports in no_std environments
#[cfg(not(feature = "std"))]
...
/// Default maximum size for an item to be serialized onto a stack buffer within BoundedVec/BoundedStack.
...
// #[cfg(all(feature = "alloc", not(feature = "std")))] // This line was importing `alloc::{};`
// use alloc::{}; // Removed empty import
...
// use core::mem::MaybeUninit; // No longer needed here if SafeMemoryHandler doesn't expose it directly
#[cfg(feature = "std")]
use std::fmt;
...
use crate::codes;
use crate::traits::{ReadStream, WriteStream};
use core::hash::Hash;
...
use crate::prelude::{Debug, Display, Eq, PartialEq, Clone, Default, Ord, PartialOrd, Sized};
use crate::traits::{SerializationError, BytesWriter}; // Added BytesWriter
...
use crate::{
    ...
    traits::{Checksummable, FromBytes, ToBytes, DefaultMemoryProvider, BytesWriter as BytesWriterTrait},
    verification::{Checksum, VerificationLevel, Hasher as VerificationHasher}, // Removed Checksum here if it's only used via the trait, otherwise keep if directly instantiated
    WrtResult, // Import WrtResult for the crate
    Error, // Import WrtError re-exported as Error from lib.rs
};
use wrt_error::{ErrorCategory as WrtErrorCategory}; // And added here as a top-level import - Keep ErrorCategory qualified
use core::hash::{Hash as CoreHash, Hasher as CoreHasher}; // Renamed Hasher to CoreHasher to avoid conflict if P also brings a Hasher
...
impl BoundedError {
    ...
    pub fn conversion_error(msg_part: &str) -> Self { // Changed S: AsRef<str> to &str for simplicity with format!
        #[cfg(any(feature = "alloc", feature = "std"))]
        {
            // Assuming prelude brings in `format` correctly
    ...
    pub fn deserialization_error(msg: &'static str) -> Self {
        ...
        #[cfg(not(any(feature = "alloc", feature = "std")))]
        {
            Self::new(BoundedErrorKind::ConversionError, msg) // Use the static msg directly
        }
    }
    ...
    pub fn index_out_of_bounds(index: usize, length: usize) -> Self {
        #[cfg(any(feature = "alloc", feature = "std"))]
        {
            Self::new(BoundedErrorKind::SliceError, format!("Index {} out of bounds for length {}", index, length))
        }
        #[cfg(not(any(feature = "alloc", feature = "std")))]
        {
...
impl From<BoundedError> for crate::Error {
    fn from(err: BoundedError) -> Self {
        ...
        let message = if err.description_static != static_message_prefix {
            // This branch is tricky if we want to combine them and still return &'static str.
            // For now, let's prioritize the more specific static message from BoundedError if it's different.
            // However, this might lead to losing the category/code context conveyed by static_message_prefix.
...
impl From<crate::Error> for BoundedError {
    fn from(err: crate::Error) -> Self {
        ...
        let kind = match err.category() {
            ...
            WrtErrorCategory::Memory => BoundedErrorKind::SliceError, // Or another memory related kind
            WrtErrorCategory::Parse | WrtErrorCategory::Validation => BoundedErrorKind::ConversionError,
        ...
        #[cfg(not(any(feature = "alloc", feature = "std")))]
        {
            // No alloc, so we can't use err.to_string(). Use a static description based on kind.
    ...
    P: MemoryProvider + Default + Clone, // Ensure P has necessary bounds for methods
{
    ...
    /// initialization or if `T::default().serialized_size()` is 0 and N_ELEMENTS > 0,
    /// as this indicates an attempt to create a stack of ZSTs with a non-zero element count
    /// where item_serialized_size would be zero, potentially leading to division by zero
    /// or incorrect memory calculations if not handled carefully.
    /// For ZSTs, N_ELEMENTS should typically be 0, or specific ZST handling should be ensured.
    pub fn with_verification_level(
        ...
        if item_serialized_size == 0 && N_ELEMENTS > 0 {
            // Prevent division by zero or logical errors if N_ELEMENTS > 0 but items are ZSTs.
            // Or, if this is allowed, ensure memory_needed is handled correctly.
            // For now, consider it an invalid configuration for typical BoundedStack usage.
    ...
    /// Returns `BoundedError` if writing the item to memory fails or if checksum verification fails.
    pub fn push(&mut self, item: T) -> core::result::Result<(), BoundedError> {
        ...
        let item_size = item.serialized_size();
         if item_size > MAX_ITEM_SERIALIZED_SIZE {
             return Err(BoundedError::new(BoundedErrorKind::ItemTooLarge, "Item exceeds max buffer size for push"));
        }

        if item_size == 0 { // Handling ZSTs
            self.length += 1;
            ...
            if self.verification_level >= VerificationLevel::Full { // Was should_recalculate_checksum_on_mutate
        ...
        let bytes_written = item.write_bytes(&mut item_bytes_buffer[..item_size])
            .map_err(|_| BoundedError::new(BoundedErrorKind::ConversionError, "Conversion error in BoundedVec"))?;

        self.handler
            .write_data(offset, &item_bytes_buffer[..bytes_written])
            .map_err(|e| BoundedError::new(BoundedErrorKind::SliceError, e.message().unwrap_or("Write data failed")))?;
    ...
    /// # Errors
    ///
    /// Returns `BoundedError` if reading the item from memory fails or if checksum verification fails.
    pub fn pop(&mut self) -> core::result::Result<Option<T>, BoundedError> {
        ...
        let slice_view = self
            ...
            .map_err(|e| BoundedError::new(BoundedErrorKind::SliceError, e.message().unwrap_or("Get slice failed for pop")))?;


        // Before deserializing, if verification is high, consider if a checksum of this specific item was stored
        // or if we rely on the whole-collection checksum.
        // For now, assuming whole-collection checksum.

        let item_data = slice_view.as_ref(); // This now works as Slice implements AsRef<[u8]>
        let (item, _read_len) =
            T::from_bytes(item_data).map_err(|_e| BoundedError::new(BoundedErrorKind::ConversionError, "Failed to deserialize item for pop"))?;

        if self.verification_level >= VerificationLevel::Full { // Was should_recalculate_checksum_on_mutate
            self.recalculate_checksum();
        }

        // Optionally, zero out the popped memory for security/safety if required by policy
        // slice_view.fill(0); // Example
    ...
    pub fn peek(&self) -> core::result::Result<Option<T>, BoundedError> {
        ...
        match slice_view_result {
            Ok(slice_view) => {
                // Assuming T::from_bytes doesn't modify the underlying slice if it's just a view
                match T::from_bytes(slice_view.as_ref()) { // Added .as_ref()
                    Ok((item, _)) => Ok(Some(item)),
                    Err(_) => Ok(None), // Failed to deserialize, treat as if item isn't there or is corrupt
                }
            }
    ...
    pub fn verify_checksum(&self) -> bool {
        ...
        if self.item_serialized_size == 0 && self.length > 0 { // ZST handling
    ...
    /// This should be called after operations that might invalidate the checksum
    /// if per-item updates are not feasible or verification level is high.
    pub fn recalculate_checksum(&mut self) {
        self.checksum.reset();
        if self.item_serialized_size == 0 { // ZST handling
             for _ in 0..self.length {
            ...
            if let Ok(slice_view) = self.handler.get_slice(offset, self.item_serialized_size) {
                ...
                match T::from_bytes(slice_view.as_ref()) { // Added .as_ref()
                    ...
                    Err(_) => {
                        ...
                        // reflects the attempt to checksum all current data, even if parts are corrupt.
                        // A "poisoned" checksum state could also be an option.
                        // For simplicity, the current checksum will just not match the true one.
    ...
    P: MemoryProvider + Clone + PartialEq + Eq,
{
    ...
    pub fn new(provider_arg: P) -> WrtResult<Self> {
        ...
        if item_s_size == 0 && N_ELEMENTS > 0 {
             return Err(crate::Error::new( // Using WrtError directly
    ...
    pub fn pop(&mut self) -> core::result::Result<Option<T>, BoundedError> {
        ...
        // The slice from MemoryProvider is assumed to be &[u8] if P is e.g. GlobalBufferProvider
        // or P directly returns &[u8] via its get_slice.
        ...
        // Let's assume for now P::get_slice returns something T::from_bytes can handle or it's Slice.
        let (item, _read_len) =
            T::from_bytes(slice_view.as_ref()).map_err(|_e| BoundedError::new(BoundedErrorKind::ConversionError, "Failed to deserialize item for pop"))?;
                ...
                match T::from_bytes_with_provider(&mut read_stream, &self.provider) {
                    Ok(item) => {
                        ...
                        if self.checksum_size > 0 && self.item_serialized_size > 0 {
                            ...
                            if let Ok(checksum_slice) = self
                                .provider
                                .borrow_slice(checksum_offset, self.checksum_size)
                            {
                                let mut cs_stream = ReadStream::new(checksum_slice);
                                if let Ok(stored_checksum) =
                                    Checksum::from_bytes_with_provider(&mut cs_stream, &self.provider)
                                {
                                    let mut current_checksum = Checksum::new();
                                    item.update_checksum(&mut current_checksum);
    ...
    /// Note: This is a low-level operation. Prefer `get_mut` or `set` for most use cases.
    ...
    fn verify_item_checksum_at_offset(&self, offset: usize) -> WrtResult<()> {
        ...
        match self.provider.borrow_slice(offset, self.item_serialized_size) {
            Ok(slice_view) => {
                ...
                match self
                    .provider
                    .borrow_slice(checksum_offset, Checksum::SERIALIZED_SIZE)
                {
                    Ok(checksum_slice_view) => {
                        // This part needs careful implementation based on how ReadStream handles reading into a buffer
                        // Assuming ReadStream has a method like read_exact or similar
                        // For now, let's assume direct access for checksum bytes, though this is unsafe.
                        ...
                        // A temporary workaround might be to re-deserialize the checksum.
                         ...
                         let stored_checksum = Checksum::from_bytes_with_provider(&mut checksum_read_stream, &self.provider)?;
    ...
    pub fn set(&mut self, index: usize, value: T) -> core::result::Result<T, BoundedError> {
        ...
        let current_value = match self.get(index) {
            Ok(value) => value,
            Err(_) => return Err(BoundedError::new(
                BoundedErrorKind::ConversionError,
        ...
        if self.item_serialized_size == 0 {
            ...
            if self.verification_level >= VerificationLevel::Full {
                ...
                // contribution to the checksum, but for now we'll recalculate the entire checksum
                self.recalculate_checksum();
        ...
        let bytes_written = value.write_bytes(&mut item_bytes_buffer[..item_size])
            .map_err(|e| BoundedError::new(
                BoundedErrorKind::ConversionError,
        ...
        self.provider
            ...
            .map_err(|e| BoundedError::new(
                BoundedErrorKind::SliceError,
                "Failed to write data for set",
            ))?;
        
        // Update checksum if needed
        if self.verification_level >= VerificationLevel::Full {
            // Option 1: Recalculate the entire checksum (more expensive but ensures correctness)
            self.recalculate_checksum();
            
            // Option 2: Update incrementally (more efficient but potentially less reliable)
            // Let's use option 1 for now to ensure correctness
        }
    ...
    pub fn insert(&mut self, index: usize, value: T) -> core::result::Result<(), BoundedError> {
        ...
        for i in (index..self.length).rev() {
            ...
            let bytes_written = current_item.write_bytes(&mut item_bytes_buffer[..item_size])
                .map_err(|e| BoundedError::new(
                    BoundedErrorKind::ConversionError,
        ...
        self.provider
            ...
            .map_err(|e| BoundedError::new(
                BoundedErrorKind::SliceError,
                "Failed to write data for insert",
            ))?;
    ...
    /// Removes the element at the specified index, shifting all elements after it to the left.
    ///
    ...
    pub fn remove(&mut self, index: usize) -> core::result::Result<T, BoundedError> {
        ...
        for i in index..(self.length - 1) {
            ...
            let bytes_written = next_item.write_bytes(&mut item_bytes_buffer[..item_size])
                .map_err(|e| BoundedError::new(
                    BoundedErrorKind::ConversionError,
    ...
    /// Completes the implementation of get_item_mut_slice_for_write that was previously a placeholder.
    /// This method provides a mutable slice of the underlying data for an item at the given index.
    /// 
    /// Note: This is a low-level operation intended for internal use or special cases.
    /// For most use cases, prefer `set()` instead.
    /// 
    /// # Safety
    /// This method can potentially bypass checksum validation if used incorrectly.
    /// The caller must ensure that the written data maintains the validity of the collection.
    ...
    /// This method operates in place, modifying the vector.
    ///
    /// # Examples
    ///
    /// ```
        ...
        F: FnMut(&T) -> bool,
    {
        ...
        for i in 0..self.length {
            ...
            // If predicate returns false, we skip this item (don't increment write_idx)
        }
    ...
    ///
    /// If the vector contains the given value, this returns `Ok(index)` where `index` is
    /// the position of the value. If the vector doesn't contain the given value, this
    /// returns `Err(insertion_index)` where `insertion_index` is where the value would
    /// need to be inserted to maintain sorted order.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// the underlying vector, returning an ordering according to the comparison.
    ///
    /// If the vector contains an element equal to the given one, the returned index is
    /// the first such element's index. If the vector doesn't contain an element equal
    /// to the given one, the returned index is the index where such an element could be
    ...
    /// ```
    pub fn binary_search_by<F>(&self, mut f: F) -> core::result::Result<Result<usize, usize>, BoundedError>
    where
    ...
    ///
    /// If the vector contains an element with a key equal to the provided key, the
    /// returned index is the first such element's index. If the vector doesn't contain
    /// an element with a key equal to the provided key, the returned index is the
    /// index where such an element could be inserted while maintaining sorted order.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// assert_eq!(result, Ok(1));
    /// ```
    pub fn binary_search_by_key<B, F>(&self, key: &B, mut f: F) -> core::result::Result<Result<usize, usize>, BoundedError>
    where
        B: Ord,
        F: FnMut(&T) -> B,
    {
    ...
    /// Removes consecutive duplicate elements from the vector according to the `==` operator.
    ///
    ...
    /// Removes consecutive duplicate elements using the given equality function.
    ///
    ...
    /// Removes consecutive duplicate elements from the vector using the given key function.
    ///
    /// If the vector is sorted such that all duplicates (according to the key function)
    /// are next to each other, this will remove all duplicates.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// ```
    #[cfg(feature = "alloc")]
    pub fn replace_range<R>(&mut self, range: R, replacement: &[T]) -> core::result::Result<(), BoundedError>
    where
    ...
    P: MemoryProvider + Clone + PartialEq + Eq,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.current_index < self.vec.len() {
            ...
            // A more robust iterator might return Result<T, Error> or handle errors differently.
...
impl<T, const N_ELEMENTS: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> Checksummable for BoundedVec<T, N_ELEMENTS, P>
where
    T: Sized + Checksummable + ToBytes + FromBytes + Default + Clone + PartialEq + Eq,
    P: MemoryProvider + Default + Clone + PartialEq + Eq,
{
    fn update_checksum(&self, checksum: &mut Checksum) {
        ...
        // An alternative for raw data collections might be to checksum the underlying byte buffer.
        // However, for structured data, checksumming logical items is more robust.
        if self.item_serialized_size > 0 { // Only iterate if items have size
            for i in 0..self.length {
                // self.get(i) returns Option<T>.
                // If an item can't be retrieved (e.g., deserialization error, though get() currently swallows this),
                // it won't be part of the checksum. This might be acceptable if `get` failing implies corruption.
                if let Some(item) = self.get(i) {
                    item.update_checksum(checksum);
                } else {
                    // This case implies an issue with get(i) for a valid index, 
                    // which shouldn't happen unless memory is corrupted or T::from_bytes fails unexpectedly.
                    // For robustness, one might log this or handle it based on verification level.
                    // For now, if get fails, that part of data won't contribute to checksum.
                }
            }
        }
    }
...
where
    T: Hash + Checksummable + ToBytes + FromBytes + Default + Clone + PartialEq + Eq + core::fmt::Debug, // Corrected T bounds
    P: MemoryProvider + Default + Clone + PartialEq + Eq,
{
...
impl<T, const N_ELEMENTS: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> Checksummed for BoundedVec<T, N_ELEMENTS, P>
where
    T: Sized + Checksummable + ToBytes + FromBytes + Default + Clone + PartialEq + Eq,
    P: MemoryProvider + Default + Clone + PartialEq + Eq,
{
...
impl<T, const N_ELEMENTS: usize, P: MemoryProvider + Clone + PartialEq + Eq> ToBytes for BoundedVec<T, N_ELEMENTS, P>
where
    T: Sized + Checksummable + ToBytes + FromBytes + Default + Clone + PartialEq + Eq,
{
    fn to_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        ...
        // Write each element
        // This requires iterating over the elements stored in SafeMemoryHandler via self.provider
        // The current BoundedVec structure doesn't directly expose an iterator easily here
        // without potentially unsafe memory access or more complex SafeMemoryHandler methods.
        // For now, this is a conceptual loop. A real implementation needs safe access to elements.
        
        // Assuming self.get(i) provides a way to get T by value or ref
        // And that T implements ToBytes correctly using stream_provider
        for i in 0..self.length {
            if let Some(item) = self.get(i) { // get() needs to be infallible or error handled
                item.to_bytes_with_provider(writer, stream_provider)?;
            } else {
                // This case should ideally not happen if length is correct.
                return Err(crate::Error::new(WrtErrorCategory::System, codes::SYSTEM_ERROR, "BoundedVec inconsistency during serialization"));
            }
        }
        Ok(())
    }
...
impl<T, const N_ELEMENTS: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> FromBytes for BoundedVec<T, N_ELEMENTS, P>
where
    T: Sized + Checksummable + ToBytes + FromBytes + Default + Clone + PartialEq + Eq,
{
    fn from_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        ...
        if count > N_ELEMENTS {
            return Err(crate::Error::from(SerializationError::Custom("Decoded vector length exceeds capacity")));
        }
        ...
        for _ in 0..count {
            // T::from_bytes_with_provider might need its own provider if T is also generic over one.
            // Here, stream_provider is passed as per the FromBytes trait signature for T.
            let item = T::from_bytes_with_provider(reader, stream_provider)?;
            vec.push(item).map_err(|e| crate::Error::from(SerializationError::Custom("Failed to push item to BoundedVec during deserialization")))?; // Convert BoundedError
        }
        Ok(vec)
    }
...
impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> BoundedVec<u8, N_BYTES, P> {
    /// Get the internal slice if the memory provider supports direct slice access.
    pub fn as_internal_slice(&self) -> core::result::Result<Slice<'_>, crate::Error> {
        ...
        // For a NoStdProvider, this would be a slice of its internal array up to self.length.
        // For StdProvider, it's a slice of its Vec<u8>.

        // We need to get the raw slice from the BoundedVec's internal provider `self.provider`.
        // BoundedVec<T,N,P> has `provider: P`. Access its used portion.
        self.provider.borrow_slice(0, self.length) // Assuming provider stores items from offset 0
    }
}
...
impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> ToBytes for BoundedString<N_BYTES, P> {
    fn to_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        &self,
        writer: &mut WriteStream<'a>,
        stream_provider: &PStream,
...
impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> FromBytes for BoundedString<N_BYTES, P> {
    fn from_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        reader: &mut ReadStream<'a>,
        stream_provider: &PStream,
    ) -> WrtResult<Self> {
...
impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> Checksummable for BoundedString<N_BYTES, P> {
    fn update_checksum(&self, checksum: &mut Checksum) {
        self.bytes.update_checksum(checksum); // Delegate to inner BoundedVec<u8>
    }
}

impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> Hash for BoundedString<N_BYTES, P> {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.bytes.hash(state);
    }
}

/// A type representing a valid WebAssembly name string, typically for functions, locals, etc.
/// It is a newtype wrapper around `BoundedString` to provide a distinct type for WASM identifiers
/// and potentially enforce WASM-specific validation rules in the future.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct WasmName<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> {
    inner: BoundedString<N_BYTES, P>,
}

impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> Default for WasmName<N_BYTES, P> {
    fn default() -> Self {
        Self { inner: BoundedString::default() }
    }
}

impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> WasmName<N_BYTES, P> {
...
{
    fn hash<H: CoreHasher>(&self, state: &mut H) { // Changed to CoreHasher
        self.length.hash(state);
        self.checksum.hash(state); // Assuming Checksum is Hashable
                                   // Hash elements if verification level suggests deep hashing
        if self.verification_level >= VerificationLevel::Full {
            for i in 0..self.length {
                if let Some(item) = self.peek_at_index(i) { // Define peek_at_index if needed
                    item.hash(state);
                }
            }
        }
...
impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> BoundedString<N_BYTES, P> {
    /// Creates a new BoundedString from a string slice.
    ///
    /// The string is truncated if it's longer than `N_BYTES`.
    /// Returns an error if the provider fails to initialize the internal BoundedVec.
    pub fn from_str_truncate(s: &str, provider: P) -> Result<Self, BoundedError> {
    ...
    pub fn from_str(s: &str, provider: P) -> Result<Self, SerializationError> {
        ...
        // Basic UTF-8 validation can be done by str::from_utf8 on the slice to be stored.
        // Since `s` is already a &str, it's valid UTF-8. We just need to ensure it fits.
        let mut bytes_vec = BoundedVec::<u8, N_BYTES, P>::new(provider)
            .map_err(|e| SerializationError::Custom("Failed to create BoundedVec for BoundedString"))?;
        for byte in s_bytes.iter() {
            bytes_vec.push(*byte)
                .map_err(|e| SerializationError::Custom("Failed to push byte to BoundedVec for BoundedString"))?;
        }
        Ok(Self { bytes: bytes_vec })
    }
    ...
    ///
    /// Returns an error if the internal bytes are not valid UTF-8 or if there's a problem
    ...
    ///
    /// ```
    /// # use wrt_types::bounded::BoundedString;
    /// # use wrt_types::NoStdProvider;
    /// # use wrt_types::VerificationLevel;
    /// #
    /// # let provider = NoStdProvider::new(1024, VerificationLevel::default());
    ...
    > BoundedVec<T, N_ELEMENTS, P>
{
    ...
    /// This is useful when you need to get a copy of the data, not just a reference.
    /// 
    /// Note: This is only available when the `alloc` or `std` feature is enabled.
    #[cfg(any(feature = "alloc", feature = "std"))]
    ...
    /// Returns a direct byte slice of the raw memory if the provider supports it.
    /// This is a more efficient alternative to `to_bytes_vec()` when a copy is not needed.
    /// 
    /// This method will fail if the provider does not support direct memory access or
    /// if the data is not stored contiguously.
...
// Fix for BoundedString::new in WasmName tests (if any, this was in build log for BoundedString::new itself)
// The main fix is in WasmName::new above.
// If BoundedString::new was called directly elsewhere:
// `BoundedString::<CAP, _>::new(provider)` would become `BoundedString::<CAP, _>::from_str_truncate("", provider)`
    ...
    > BoundedVec<T, N_ELEMENTS, P>
{
        ...
        T: Clone, // Added Clone bound for items
    {
        if self.len + other_slice.len() > N_ELEMENTS {
            return Err(BoundedError::capacity_error(
                self.len + other_slice.len(),
            ));
        }
        for item in other_slice {
            // This will use self.push(item.clone()), which handles serialization and checksums
...
impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> Checksummable for WasmName<N_BYTES, P> {
    fn update_checksum(&self, checksum: &mut crate::verification::Checksum) {
        self.inner.update_checksum(checksum);
    }
}

impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> ToBytes for WasmName<N_BYTES, P> {
    fn to_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        &self,
        writer: &mut WriteStream<'a>,
        provider: &PStream,
...
impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> FromBytes for WasmName<N_BYTES, P> {
    fn from_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        reader: &mut ReadStream<'a>,
        provider: &PStream,
    ) -> WrtResult<Self> {
    ...
    #[cfg(feature = "default-provider")]
    fn from_bytes<'a>(reader: &mut ReadStream<'a>) -> WrtResult<Self> {
        BoundedString::<N_BYTES, P>::from_bytes(reader)
            .map(|inner_bs| Self { inner: inner_bs })
    }
}

impl<T, const N_ELEMENTS: usize, P: MemoryProvider + Clone + PartialEq + Eq> BoundedVec<T, N_ELEMENTS, P>
where
    T: Sized + Checksummable + ToBytes + FromBytes + Default + Clone + PartialEq + Eq,
{
    // ... (existing methods) ...
    ...
    /// This is a conceptual helper; actual implementation depends on SafeMemoryHandler's API.
    fn get_item_mut_slice_for_write(&mut self, index: usize) -> WrtResult<SliceMut<'_>> {
        if index >= self.length {
            return Err(crate::Error::index_out_of_bounds(index, self.length));
        }
        ...
        // This will likely require `P` to have methods for this or `BoundedVec` needs a `SafeMemoryHandler`.
        // For now, this will cause an error as `P` doesn't have `get_slice_mut`.
        // This indicates a deeper refactoring might be needed for direct item deserialization.

        // Placeholder: In a real scenario, SafeMemoryHandler owned by BoundedVec would be used.
        // For now, let's assume this is part of a strategy that will be refined.
        // If BoundedVec holds a SafeMemoryHandler 'handler' field:
        ...
        // self.provider.get_slice_mut(offset, self.item_serialized_size) // This won't compile

        // Fallback to an error indicating this path is not yet fully implemented for direct write
        Err(crate::Error::new(
            WrtErrorCategory::NotImplemented, 
            codes::NOT_IMPLEMENTED, 
            "Direct mutable slice access for item write not fully implemented on BoundedVec provider"
        ))
    }
}


// ... (other BoundedVec impl methods, make sure to use `Error::` where it was `Error::` before)
// For example, in BoundedVec::get:
...
// T::from_bytes(&mut item_reader).map_err(|e| Error::deserialization_error("Failed to deserialize item from BoundedVec"))
...
#[cfg(feature = "alloc")]
impl<const N_BYTES: usize, P: MemoryProvider + Default + Clone + PartialEq + Eq> BoundedString<N_BYTES, P> {
    /// Splits the string by the given delimiter and returns a vector of BoundedStrings.
    ///
    /// # Examples
    ///
    /// ```
