...
// Internal imports
use crate::{
    component_value_store::{ComponentValueStore, MAX_STORE_VALUES, MAX_STORE_TYPES, MAX_TYPE_TO_REF_MAP_ENTRIES},
...
impl<P: MemoryProvider + Default + Clone + PartialEq + Eq> Default for ComponentValueStoreBuilder<P> {
    fn default() -> Self {
        Self {
            provider: P::default(),
...
impl<P: MemoryProvider + Default + Clone + PartialEq + Eq> ComponentValueStoreBuilder<P> {
    ...
    ///
    /// This is a hint only - the actual capacity is limited by MAX_STORE_VALUES.
