...
impl BoundedWitParser {
    ...
    fn bounded_parse(&mut self) -> Result<()> {
        ...
        while position < self.input_len {
            ...
            // Handle comments
            if !in_comment && byte == b'/' && position + 1 < self.input_len && self.input_buffer[position + 1] == b'/' {
                in_comment = true;
                position += 2;
                continue;
            }
    ...
    fn add_warning(&mut self, warning: WitParseWarning) {
        if self.warnings.len() < 100 { // Limit warnings to prevent memory bloat
...
pub fn parse_wit_with_limits(wit_source: &[u8], limits: WitParsingLimits) -> Result<WitParseResult> {
