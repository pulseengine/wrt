...
impl DataSegmentBridge {
    ...
    pub fn extract_runtime_data(segment: &PureDataSegment) -> RuntimeDataExtraction {
        ...
        let offset_expr_bytes = {
            ...
            let mut bounded_vec = crate::WasmVec::new(provider).unwrap();
...
impl ModuleBridge {
    ...
    pub fn extract_module_runtime_data(module: &crate::module::Module) -> ModuleRuntimeData {
        ...
        #[cfg(feature = "std")]
        let mut data_extractions = Vec::new();
        #[cfg(not(feature = "std"))]
        let mut data_extractions = {
            let provider = DefaultProvider::default();
            crate::WasmVec::new(provider).unwrap()
        };
        for data_segment in module.data.iter() {
            // Convert module::Data to runtime extraction
            let extraction = RuntimeDataExtraction {
                is_active: matches!(data_segment.mode, crate::module::DataMode::Active),
                memory_index: Some(data_segment.memory_idx),
                offset_expr_bytes: {
                    // Handle offset expression bytes - for now use empty vec
                    #[cfg(feature = "std")]
                    { std::vec::Vec::new() }
                    #[cfg(not(feature = "std"))]
                    { 
                        let provider = DefaultProvider::default();
                        crate::WasmVec::new(provider).unwrap_or_default()
                    }
                },
                data_size: data_segment.init.len(),
                requires_initialization: matches!(data_segment.mode, crate::module::DataMode::Active),
            };
            #[cfg(feature = "std")]
        ...
        #[cfg(feature = "std")]
        let mut element_extractions = Vec::new();
        #[cfg(not(feature = "std"))]
        ...
        for element_segment in module.elements.iter() {
            let extraction = RuntimeElementExtraction {
                is_active: matches!(element_segment.mode, crate::module::ElementMode::Active { .. }),
                table_index: match element_segment.mode {
                    crate::module::ElementMode::Active { table_index, .. } => Some(table_index),
                    _ => None,
                },
                ...
                init_data_type: match &element_segment.init {
                    crate::module::ElementInit::FuncIndices(_) => ElementInitType::FunctionIndices,
                    crate::module::ElementInit::Expressions(_) => ElementInitType::ExpressionBytes,
                },
                requires_initialization: matches!(element_segment.mode, crate::module::ElementMode::Active { .. }),
            };
            #[cfg(feature = "std")]
        ...
        ModuleRuntimeData {
            ...
            requires_initialization: module.start.is_some() || 
                module.data.iter().any(|d| matches!(d.mode, crate::module::DataMode::Active)) ||
                module.elements.iter().any(|e| matches!(e.mode, crate::module::ElementMode::Active { .. })),
        }
    }
    ...
    pub fn create_initialization_plan(module: &crate::module::Module) -> ModuleInitializationPlan {
        ...
        for (index, data_segment) in module.data.iter().enumerate() {
            let hint = DataInitializationHint {
                segment_type: if matches!(data_segment.mode, crate::module::DataMode::Active) {
                    DataSegmentType::Active
                } else {
                    DataSegmentType::Passive
                },
                memory_target: Some(data_segment.memory_idx),
                offset_evaluation_needed: matches!(data_segment.mode, crate::module::DataMode::Active),
                data_bytes_ref: DataBytesReference {
                    start_offset: 0,
                    length: data_segment.init.len(),
                    requires_copy: true,
                },
            };
            #[cfg(feature = "std")]
        ...
        for (index, element_segment) in module.elements.iter().enumerate() {
            let hint = ElementInitializationHint {
                segment_type: match element_segment.mode {
                    crate::module::ElementMode::Active { .. } => ElementSegmentType::Active,
                    crate::module::ElementMode::Passive => ElementSegmentType::Passive,
                    crate::module::ElementMode::Declared => ElementSegmentType::Declared,
                },
                table_target: match element_segment.mode {
                    crate::module::ElementMode::Active { table_index, .. } => Some(table_index),
                    _ => None,
                },
                offset_evaluation_needed: matches!(element_segment.mode, crate::module::ElementMode::Active { .. }),
                element_count: match &element_segment.init {
                    crate::module::ElementInit::FuncIndices(indices) => indices.len(),
                    crate::module::ElementInit::Expressions(exprs) => exprs.len(),
                },
            };
            #[cfg(feature = "std")]
...
pub struct ModuleRuntimeData {
    ...
    pub data_extractions: Vec<RuntimeDataExtraction>,
    /// Runtime data for all element segments
    pub element_extractions: Vec<RuntimeElementExtraction>,
...
pub struct ModuleInitializationPlan {
    ...
    pub data_initialization_order: Vec<(usize, DataInitializationHint)>,
    /// Element segments with their initialization hints (index, hint)
    pub element_initialization_order: Vec<(usize, ElementInitializationHint)>,
...
fn calculate_initialization_steps(module: &crate::module::Module) -> usize {
    ...
    steps += module.data.iter()
        .filter(|d| matches!(d.mode, crate::module::DataMode::Active))
        .count() * 2;
        
    // Count active element segments (each requires offset evaluation + table.init)
    steps += module.elements.iter()
        .filter(|e| matches!(e.mode, crate::module::ElementMode::Active { .. }))
        .count() * 2;
