...
impl IncrementalParser {
    ...
    pub fn apply_change(&mut self, change: SourceChange) -> Result<()> {
        match change.change_type {
            ChangeType::Insert { offset, length: _ } => {
                self.apply_insert(offset, change.text.as_ref().ok_or_else(|| 
                    Error::parse_error("Insert change requires text")
                )?)?;
            }
            ...
            ChangeType::Replace { offset, old_length, new_length: _ } => {
                self.apply_replace(offset, old_length, change.text.as_ref().ok_or_else(|| 
                    Error::parse_error("Replace change requires text")
                )?)?;
            }
        }
    ...
    fn apply_insert(&mut self, offset: u32, text: &BoundedString<1024, NoStdProvider<1024>>) -> Result<()> {
    ...
    fn apply_replace(&mut self, offset: u32, old_length: u32, text: &BoundedString<1024, NoStdProvider<1024>>) -> Result<()> {
