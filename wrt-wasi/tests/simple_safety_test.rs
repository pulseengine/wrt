...
use wrt_foundation::{safety_aware_alloc, safe_managed_alloc};
...
fn test_safety_aware_allocation() {
    ...
    let small_result = safety_aware_alloc!(1024, WASI_CRATE_ID);
    assert!(small_result.is_ok(), "Small allocation should succeed");
    
    // Test allocation at various sizes
    let test_sizes = [8192, 16384, 32768, 65536];
    
    for &size in &test_sizes {
        println!("Testing allocation of {} bytes", size);
        let result = safe_managed_alloc!(size, WASI_CRATE_ID);
