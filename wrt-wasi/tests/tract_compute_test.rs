...
use wrt_wasi::nn::{
    capabilities::{DynamicNNCapability, NeuralNetworkCapability, create_nn_capability, NNVerificationLevel as VerificationLevel},
    backend::{NeuralNetworkBackend, ModelCapability, ComputeCapable},
    tract_backend::TractBackend,
    GraphEncoding, TensorType, TensorDimensions, Tensor,
};
...
fn test_tract_context_stores_runnable() {
    ...
    if let Ok(model) = backend.load_model(&onnx_model, GraphEncoding::ONNX) {
        ...
        // We can't directly check this due to privacy, but creating it should work
    }
}
