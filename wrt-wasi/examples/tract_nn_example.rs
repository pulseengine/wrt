...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    use wrt_wasi::{
        nn::{
            initialize_nn, capabilities::{create_nn_capability, NNVerificationLevel as VerificationLevel},
            backend::{initialize_backends, BackendRegistry},
    ...
    registry.register(
        GraphEncoding::ONNX,
        Box::new(TractBackendProvider::new())
    );
    ...
    let context_id = wrt_wasi::nn::sync_bridge::nn_init_execution_context(graph_id)?;
