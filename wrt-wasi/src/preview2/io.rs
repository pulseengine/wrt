...
pub fn wasi_stream_check_write(
    _target: &mut dyn Any,
    args: Vec<Value>,
) -> Result<Vec<Value>> {
    let stream_handle = extract_stream_handle(&args)?;
...
pub fn wasi_poll_one_off(
    ...
    let wasi_results: Vec<Value> = results.into_iter()
        .map(|ready| Value::Bool(ready))
        .collect();
