...
use crate::prelude::*;
use super::{
    NeuralNetworkBackend, ModelCapability, TensorCapability, NeuralNetworkCapability,
    Tensor, TensorType, TensorDimensions, GraphEncoding, BackendProvider, DynBackend,
    ComputeCapable,
};
...
impl ComputeCapable for TractContext {
    fn compute(&mut self, inputs: &[Tensor], model: &dyn ModelCapability) -> Result<Vec<Tensor>> {
        #[cfg(feature = "tract")]
        {
            ...
            // Run inference
            let outputs = self.runnable.run(tract_inputs)
...
fn tensor_type_to_datum(tensor_type: TensorType) -> Result<DatumType> {
    match tensor_type {
        ...
        _ => Err(Error::wasi_unsupported_operation("Unsupported tensor type for Tract")),
    }
}
...
impl<C: NeuralNetworkCapability + 'static> NeuralNetworkBackend for TractBackend<C> {
    ...
    fn load_model(&self, data: &[u8], encoding: GraphEncoding) -> Result<Self::Model> {
        ...
        #[cfg(feature = "tract")]
        {
            ...
            let model = match encoding {
                ...
                _ => return Err(Error::wasi_invalid_encoding("Only ONNX is currently supported")),
            };
    ...
    fn create_tensor(
        ...
        let size = num_elements.checked_mul(data_type.size_bytes())
    ...
    fn compute(&self, context: &mut Self::Context) -> Result<()> {
        #[cfg(feature = "tract")]
        {
            ...
            let inputs: Result<TVec<TValue>> = context.inputs
                .iter()
                .enumerate()
                .map(|(idx, opt)| {
                    opt.as_ref()
    ...
    fn get_output(
        ...
        #[cfg(feature = "tract")]
        {
            if let Some(ref outputs) = context.outputs {
                ...
                Err(Error::wasi_runtime_error("No outputs available - compute not called"))
            }
        }
...
impl BackendProvider for TractBackendProvider {
    fn create_backend(&self, capability: &dyn NeuralNetworkCapability) -> Result<Box<dyn DynBackend>> {
        ...
        match capability.verification_level() {
            ...
            _ => Err(Error::wasi_unsupported_operation("Unsupported verification level")),
        }
    }
...
mod tests {
    use super::*;
    use crate::nn::capabilities::{DynamicNNCapability, StaticNNCapability};
