...
use crate::prelude::*;
use super::{
    get_nn_capability, with_nn_capability, get_graph_store, get_context_store,
    GraphEncoding, ExecutionTarget, TensorType, NNOperation, WitTypeConversion,
};

/// Async load a neural network graph
///
/// Future implementation of `wasi:nn/inference.load` for Preview3
pub async fn nn_load_async(
    data: Vec<u8>,
    encoding: u8,
    target: u8,
) -> Result<u32> {
...
///
/// Future implementation of `wasi:nn/inference.init-execution-context` for Preview3
...
pub async fn nn_get_output_async(
    context_id: u32,
    index: u32,
...
pub async fn nn_stream_inference(
    ...
    Err(Error::wasi_unsupported_operation("Stream inference not yet implemented"))
}
...
pub async fn nn_multi_model_inference(
    ...
    Err(Error::wasi_unsupported_operation("Multi-model inference not yet implemented"))
}
