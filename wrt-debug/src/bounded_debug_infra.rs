...
// Import traits from wrt_foundation for compatibility
...
pub fn create_debug_string(s: &str) -> WrtResult<BoundedString<MAX_FILE_PATH_LEN, DebugProvider>> {
    let guard = managed_alloc!(DEBUG_PROVIDER_SIZE, CrateId::Debug)?;
    BoundedString::from_str(s, guard.provider().clone())
