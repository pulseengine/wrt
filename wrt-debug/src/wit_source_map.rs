...
pub struct WitSourceFile {
    ...
    pub path: BoundedString<256, NoStdProvider<1024>>,
    
    /// Source content lines for context display
    pub lines: Vec<BoundedString<1024, NoStdProvider<1024>>>,
...
pub enum WitTypeKind {
    ...
    Primitive(BoundedString<16, NoStdProvider<1024>>),
...
pub struct ComponentBoundary {
    ...
    pub name: Option<BoundedString<64, NoStdProvider<1024>>>,
...
pub struct WitDiagnostic {
    ...
    pub suggestion: Option<BoundedString<256, NoStdProvider<1024>>>,
...
impl WitSourceMap {
    ...
    pub fn map_error_to_diagnostic(&self, error: &Error, binary_offset: Option<u32>) -> Option<WitDiagnostic> {
        ...
        let message = BoundedString::from_str(
            &format!("Runtime error: {}", error), 
            provider.clone()
        ).unwrap_or_else(|_| 
            BoundedString::from_str("Runtime error (message too long)", provider.clone()).unwrap()
        );
...
pub struct SourceContext {
    /// File path
    pub file_path: BoundedString<256, NoStdProvider<1024>>,
...
pub struct ContextLine {
    ...
    /// Line content
    pub content: BoundedString<1024, NoStdProvider<1024>>,
...
impl WitSourceFile {
    ...
    #[cfg(feature = "wit-integration")]
    pub fn line(&self, line_number: u32) -> Option<&BoundedString<1024, NoStdProvider<1024>>> {
        if line_number == 0 {
            return None;
        }
