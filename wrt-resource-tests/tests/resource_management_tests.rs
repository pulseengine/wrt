...
fn test_host_resource_management() {
    ...
    let retrieved = resource_manager.get_host_resource::<i32>(id).unwrap();
    assert_eq!(retrieved, 42);
    
    // Try to retrieve with wrong type
    let wrong_type_result = resource_manager.get_host_resource::<String>(id);
    ...
    // Verify it's gone
    let not_found = resource_manager.get_host_resource::<i32>(id);
...
fn test_resource_lifecycle() {
    ...
    // Create resources
    let id1 = resource_manager.add_host_resource(String::from("resource1"));
    let id2 = resource_manager.add_host_resource(42);
    ...
    // Get resource types
    let type1 = resource_manager.get_resource_type(id1).unwrap();
    let type2 = resource_manager.get_resource_type(id2).unwrap();
    assert!(type1.contains("String"));
    assert!(type2.contains("i32"));
...
fn test_memory_manager_integration() {
    ...
    // For our test implementation, we're using fixed data in the memory manager
    ...
    assert_eq!(original, data); // Not modified
...
fn test_thread_safety() {
    ...
    // Collect results
    let ids: Vec<ResourceId> = handles.into_iter()
        .map(|h| h.join().unwrap())
        .collect();
...
fn test_resource_system_integration() {
    ...
    let complex_id = resource_manager.add_host_resource((String::from("name"), 42, true));
    ...
    let complex_res = resource_manager.get_host_resource::<(String, i32, bool)>(complex_id).unwrap();
