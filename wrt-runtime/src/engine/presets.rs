...
//!
//! This module provides pre-configured capability contexts for QM, ASIL-A, and ASIL-B
//! safety levels. Each preset configures memory capabilities appropriately.

use wrt_foundation::{
    budget_aware_provider::CrateId,
    capabilities::{MemoryCapabilityContext, verified::VerificationProofs},
...
///
/// This is the full ASIL-B implementation that requires formal verification proofs
/// for each memory allocation. Use this when formal verification is available.
#[allow(dead_code)]
pub fn asil_b_verified(proofs: &[(CrateId, VerificationProofs)]) -> Result<MemoryCapabilityContext> {
    let mut context = MemoryCapabilityContext::new(
        VerificationLevel::Redundant, // Highest verification level
        true, // Strict runtime verification
    );
...
mod tests {
    ...
    fn test_capability_sizes() {
        // Verify the hierarchy of resource constraints: QM > ASIL-A > ASIL-B > ASIL-C > ASIL-D
