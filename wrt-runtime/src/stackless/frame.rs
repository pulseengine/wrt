...
use wrt_foundation::{
    ...
    values::{
        FloatBits32,
        FloatBits64,
    }, // Added for floating-point values
    BlockType,
    BoundedCapacity,
    FuncType,
    Validatable,
...
impl StacklessFrame {
    /// Helper function to pop a value from the execution stack and handle the
    /// Result<Option<T>, E> return type
...
impl FrameBehavior for StacklessFrame {
    ...
    fn get_local(&self, index: usize) -> Result<Value> {
        ...
        #[cfg(not(feature = "std"))]
        {
            self.locals
                .get(index)
                .map_err(|_| Error::memory_out_of_bounds("Local variable access failed"))
        }
    }
    ...
    fn set_local(&mut self, index: usize, value: Value) -> Result<()> {
        ...
        #[cfg(not(feature = "std"))]
        {
            self.locals
                .set(index, value)
                .map(|_| ())
                .map_err(|_| Error::memory_out_of_bounds("Local variable assignment failed"))
        }
    }
    ...
    fn step(&mut self, engine: &mut StacklessEngine) -> Result<ControlFlow> {
        ...
        if self.pc >= instructions.len() {
            ...
            if self.arity == 0 {
                ...
                #[cfg(not(feature = "std"))]
                {
                    ...
                    let empty_vec: crate::types::ValueStackVec =
                        wrt_foundation::bounded::BoundedVec::new(provider)?;
                    return Ok(ControlFlow::Return { values: empty_vec });
                }
            } else {
        ...
        let instruction = instructions
            .get(self.pc)
            .map_err(|_| Error::runtime_error("Invalid program counter"))?;
        self.pc += 1;
        ...
        match instruction {
            ...
            Instruction::Block { block_type_idx } => {
                ...
                let block_context = BlockContext {
                    ...
                    end_pc: 0,                          /* Will be set when we encounter the
            ...
            Instruction::If { block_type_idx } => {
                ...
                let condition_val_opt = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?;
                ...
                if condition {
                    ...
                    return Err(Error::runtime_not_implemented(
                        "If false branch - need to implement else/end scanning",
                    ));
                }
            },
            Instruction::Else => {
                ...
                Ok(ControlFlow::Trap(Error::runtime_not_implemented(
            ...
            Instruction::End => {
                ...
                if !has_blocks {
                    ...
                    #[cfg(feature = "std")]
                    {
                        ...
                        for _ in 0..self.arity {
                            ...
                            match value {
                                ...
                                None => {
                                    return Err(Error::runtime_stack_underflow(
                                        "Stack underflow during return",
                                    ))
                                },
                            }
                        }
                    ...
                    #[cfg(feature = "std")]
                    {
                        let _block_context = self
                            .block_depths
                            .pop()
                            .ok_or_else(|| Error::runtime_invalid_state("No block to end"))?;
                    }
                    #[cfg(not(feature = "std"))]
                    {
                    ...
                    return Ok(ControlFlow::Next); // Continue after ending the
                                                  // block
                }
            },
            Instruction::Br(label_idx) => {
                // Branch to the specified label (relative depth)
                // For now, simplified implementation - need to implement proper label
                // resolution
                Ok(ControlFlow::Branch(label_idx as usize))
            },
            Instruction::BrIf(label_idx) => {
                // Pop condition from stack
                let condition_val = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?;
            ...
            Instruction::Return => {
                ...
                {
                    let provider = crate::bounded_runtime_infra::create_runtime_provider()?;
                    let mut return_values: crate::types::ValueStackVec =
                        wrt_foundation::bounded::BoundedVec::new(provider)?;
                    for _ in 0..self.arity {
                        let value =
                            engine.exec_stack.values.pop().map_err(|e| {
            ...
            Instruction::Call(func_idx_val) => {
                ...
                {
                    let provider = crate::bounded_runtime_infra::create_runtime_provider()?;
                    let mut args: crate::types::ValueStackVec =
                        wrt_foundation::bounded::BoundedVec::new(provider)?;
            ...
            Instruction::CallIndirect(type_idx, table_idx) => {
                ...
                let func_ref = func_ref_opt
                    .ok_or_else(|| Error::runtime_trap("CallIndirect: null function reference"))?;
                ...
                let actual_func_idx = match func_ref {
                    ...
                    Value::FuncRef(None) => {
                        return Err(Error::runtime_trap("CallIndirect: null function reference"))
                    },
                    _ => {
                        return Err(Error::validation_error(
                            "CallIndirect: table element not a function reference",
                        ))
                    },
                };

                // 4. Type checking - get expected type and actual type
                let expected_func_type = self
                    .module_instance
                    .module()
                    .types
                    .get(type_idx as usize)
                    .map_err(|_| Error::validation_error("CallIndirect: invalid type index"))?;
                ...
                {
                    return Err(Error::validation_error(
                        "CallIndirect: function signature mismatch",
                    ));
                }
                ...
                #[cfg(feature = "std")]
                {
                    ...
                    for _ in 0..actual_func_type.params.len() {
                        ...
                        match value {
                            ...
                            None => {
                                return Err(Error::runtime_stack_underflow(
                                    "Stack underflow during call indirect",
                                ))
                            },
                        }
                    }
            ...
            Instruction::LocalSet(local_idx) => {
                let value = engine
                    ...
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack underflow on local.set"))?
            ...
            Instruction::LocalTee(local_idx) => {
                // Get the top value without popping it - always use BoundedVec::peek since
                // values is always BoundedVec
                let value = engine
                    .exec_stack
                    .values
                    .get(engine.exec_stack.values.len() - 1)
                    .unwrap()
                    .clone();
                // Handle both Vec and BoundedVec cases
                #[cfg(feature = "std")]
            ...
            Instruction::GlobalGet(global_idx) => {
                ...
                engine
                    .exec_stack
                    .values
                    .push(global.get_value().clone())
                    .map_err(|e| Error::runtime_stack_overflow("Stack overflow on global.get"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::GlobalSet(global_idx) => {
                ...
                let value = engine
                    ...
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack underflow on global.set"))?
            ...
            Instruction::TableGet(table_idx) => {
                ...
                match table.get(elem_idx)? {
                    Some(val) => {
                        engine.exec_stack.values.push(val).map_err(|e| {
            ...
            Instruction::TableSet(table_idx) => {
                ...
                let elem_idx_val = engine
                    ...
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?;
                let elem_idx = elem_idx_val
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("TableSet index not i32"))?
                    as u32;
            ...
            Instruction::TableSize(table_idx) => {
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(table.size() as i32))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::TableGrow(table_idx) => {
                ...
                let init_val = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .ok_or_else(|| {
                        Error::runtime_stack_underflow("Stack empty for table init value")
                    })?;
                let delta_val = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?;
                let delta = delta_val
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("TableGrow delta not i32"))?
                    as u32;
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(old_size as i32))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            ...
            Instruction::I32Load(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I32Load address overflow"))?;
                ...
                if effective_addr
                    .checked_add(4)
                    .map_or(true, |end| end as usize > memory.size_in_bytes())
                {
            ...
            Instruction::I64Load(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Load address overflow"))?;
            ...
            Instruction::F32Load(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("F32Load address overflow"))?;
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::F32(FloatBits32::from_bits(bits)))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::F64Load(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("F64Load address overflow"))?;
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::F64(FloatBits64::from_bits(bits)))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::I32Load8S(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I32Load8S address overflow"))?;
            ...
            Instruction::I32Load8U(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I32Load8U address overflow"))?;
            ...
            Instruction::I32Load16S(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I32Load16S address overflow"))?;
            ...
            Instruction::I32Load16U(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I32Load16U address overflow"))?;
            ...
            Instruction::I64Load8S(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Load8S address overflow"))?;
            ...
            Instruction::I64Load8U(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Load8U address overflow"))?;
            ...
            Instruction::I64Load16S(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Load16S address overflow"))?;
            ...
            Instruction::I64Load16U(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Load16U address overflow"))?;
            ...
            Instruction::I64Load32S(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Load32S address overflow"))?;
            ...
            Instruction::I64Load32U(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Load32U address overflow"))?;
            ...
            Instruction::I32Store(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I32Store address overflow"))?;
            ...
            Instruction::I64Store(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Store address overflow"))?;
            ...
            Instruction::F32Store(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("F32Store address overflow"))?;
            ...
            Instruction::F64Store(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("F64Store address overflow"))?;
            ...
            Instruction::I32Store8(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I32Store8 address overflow"))?;
            ...
            Instruction::I32Store16(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I32Store16 address overflow"))?;
            ...
            Instruction::I64Store8(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Store8 address overflow"))?;
            ...
            Instruction::I64Store16(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Store16 address overflow"))?;
            ...
            Instruction::I64Store32(mem_arg) => {
                ...
                let effective_addr = addr
                    .checked_add(mem_arg.offset)
                    .ok_or_else(|| Error::memory_error("I64Store32 address overflow"))?;
            ...
            Instruction::MemorySize(_mem_idx) => {
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(mem.size_pages() as i32))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::MemoryGrow(_mem_idx) => {
                ...
                let delta_pages_val = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?;
                let delta_pages = delta_pages_val
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("MemoryGrow delta not i32"))?
                    as u32;
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(old_size_pages as i32))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            ...
            Instruction::I32Add => {
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(a.wrapping_add(b)))
                    .map_err(|_| Error::runtime_stack_overflow("Stack overflow"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::I32Sub => {
                let b = Self::pop_i32(engine)?;
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(a.wrapping_sub(b)))
                    .map_err(|_| Error::runtime_stack_overflow("Stack overflow"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::I32Mul => {
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(a.wrapping_mul(b)))
                    .map_err(|_| Error::runtime_stack_overflow("Stack overflow"))?;
                Ok(ControlFlow::Next)
            },
            ...
            Instruction::I32And => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32And second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32And first operand not i32"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(a & b))
            ...
            Instruction::I32Or => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Or second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Or first operand not i32"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(a | b))
            ...
            Instruction::I32Xor => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Xor second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Xor first operand not i32"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(a ^ b))
            ...
            Instruction::I32Shl => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Shl second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Shl first operand not i32"))?;
                // Shift amount is masked to 5 bits (0-31) as per WebAssembly spec
                let shift = (b as u32) & 0x1F;
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(a << shift))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::I32ShrS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32ShrS second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32ShrS first operand not i32"))?;
                // Shift amount is masked to 5 bits (0-31) as per WebAssembly spec
                let shift = (b as u32) & 0x1F;
                engine
                    .exec_stack
            ...
            Instruction::I32ShrU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32ShrU second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32ShrU first operand not i32"))?;
                // Shift amount is masked to 5 bits (0-31) as per WebAssembly spec
                let shift = (b as u32) & 0x1F;
            ...
            Instruction::I32Rotl => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Rotl second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Rotl first operand not i32"))?;
            ...
            Instruction::I32Rotr => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Rotr second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Rotr first operand not i32"))?;
                // Rotate amount is masked to 5 bits (0-31) as per WebAssembly spec
                let rotate = (b as u32) & 0x1F;
            ...
            Instruction::I32DivS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("I32DivS second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("I32DivS first operand not i32"))?;
                if b == 0 {
            ...
            Instruction::I32DivU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("I32DivU second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("I32DivU first operand not i32"))?;
                if b == 0 {
                    return Err(Error::runtime_error("Division by zero"));
                }
                let result = (a as u32) / (b as u32);
            ...
            Instruction::I32Eq => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("I32Eq second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("I32Eq first operand not i32"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(if a == b { 1 } else { 0 }))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::I32Ne => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("I32Ne second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("I32Ne first operand not i32"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(if a != b { 1 } else { 0 }))
            ...
            Instruction::I32LtS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("I32LtS second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::type_error("I32LtS first operand not i32"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(if a < b { 1 } else { 0 }))
            ...
            Instruction::I64Add => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::type_error("I64Add second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::type_error("I64Add first operand not i64"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I64(a.wrapping_add(b)))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::I64Sub => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::type_error("I64Sub second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::type_error("I64Sub first operand not i64"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I64(a.wrapping_sub(b)))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::I64Mul => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Mul second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Mul first operand not i64"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I64(a.wrapping_mul(b)))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::I64DivS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64DivS second operand not i64"))?;
                ...
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64DivS first operand not i64"))?;
            ...
            Instruction::I64DivU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64DivU second operand not i64"))?;
                ...
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64DivU first operand not i64"))?;
            ...
            Instruction::I64And => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64And second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64And first operand not i64"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I64(a & b))
            ...
            Instruction::I64Or => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Or second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Or first operand not i64"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I64(a | b))
            ...
            Instruction::I64Xor => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Xor second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Xor first operand not i64"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::I64(a ^ b))
            ...
            Instruction::I64RemS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64RemS second operand not i64"))?;
                ...
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64RemS first operand not i64"))?;
            ...
            Instruction::I64RemU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64RemU second operand not i64"))?;
                ...
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64RemU first operand not i64"))?;
            ...
            Instruction::I64Shl => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Shl second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
            ...
            Instruction::I64ShrS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64ShrS second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64ShrS first operand not i64"))?;
                // Shift amount is masked to 6 bits (0-63) as per WebAssembly spec for i64
                let shift = (b as u64) & 0x3F;
                engine
                    .exec_stack
            ...
            Instruction::I64ShrU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64ShrU second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64ShrU first operand not i64"))?;
                // Shift amount is masked to 6 bits (0-63) as per WebAssembly spec for i64
                let shift = (b as u64) & 0x3F;
                // Unsigned right shift
            ...
            Instruction::I64Rotl => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Rotl second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Rotl first operand not i64"))?;
            ...
            Instruction::I64Rotr => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Rotr second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Rotr first operand not i64"))?;
                // Rotate amount is masked to 6 bits (0-63) as per WebAssembly spec for i64
                let rotate = (b as u64) & 0x3F;
            ...
            Instruction::F32Add => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Add second operand not f32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Add first operand not f32"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::F32(FloatBits32::from_bits((a + b).to_bits())))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::F32Sub => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Sub second operand not f32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Sub first operand not f32"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::F32(FloatBits32::from_bits((a - b).to_bits())))
            ...
            Instruction::F32Mul => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Mul second operand not f32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Mul first operand not f32"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::F32(FloatBits32::from_bits((a * b).to_bits())))
            ...
            Instruction::F32Div => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Div second operand not f32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Div first operand not f32"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::F32(FloatBits32::from_bits((a / b).to_bits())))
            ...
            Instruction::Select => {
                let condition_val = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .ok_or_else(|| {
                        Error::runtime_stack_underflow("Stack empty for select condition")
                    })?;
                let condition = match condition_val {
                    Value::I32(val) => val,
            ...
            Instruction::I32LtU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32LtU second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32LtU first operand not i32"))?;
            ...
            Instruction::I32GtS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32GtS second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32GtS first operand not i32"))?;
            ...
            Instruction::I32GtU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32GtU second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32GtU first operand not i32"))?;
                let result = if (a as u32) > (b as u32) { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I32LeS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32LeS second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32LeS first operand not i32"))?;
                let result = if a <= b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I32LeU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32LeU second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32LeU first operand not i32"))?;
                let result = if (a as u32) <= (b as u32) { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I32GeS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32GeS second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32GeS first operand not i32"))?;
                let result = if a >= b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I32GeU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32GeU second operand not i32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32GeU first operand not i32"))?;
                let result = if (a as u32) >= (b as u32) { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I32Eqz => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Eqz operand not i32"))?;
            ...
            Instruction::I32WrapI64 => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I32WrapI64 operand not i64"))?;
            ...
            Instruction::I64ExtendI32S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I64ExtendI32S operand not i32"))?;
            ...
            Instruction::I64ExtendI32U => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I64ExtendI32U operand not i32"))?;
            ...
            Instruction::I32TruncF32S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("I32TruncF32S operand not f32"))?;
            ...
            Instruction::I32TruncF32U => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("I32TruncF32U operand not f32"))?;
            ...
            Instruction::I64Eq => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Eq second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Eq first operand not i64"))?;
                let result = if a == b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I64Ne => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Ne second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Ne first operand not i64"))?;
                let result = if a != b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I64LtS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64LtS second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64LtS first operand not i64"))?;
                let result = if a < b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I64LtU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64LtU second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64LtU first operand not i64"))?;
            ...
            Instruction::I64GtS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64GtS second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64GtS first operand not i64"))?;
                let result = if a > b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I64GtU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64GtU second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64GtU first operand not i64"))?;
                let result = if (a as u64) > (b as u64) { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I64LeS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64LeS second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64LeS first operand not i64"))?;
                let result = if a <= b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I64LeU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64LeU second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64LeU first operand not i64"))?;
                let result = if (a as u64) <= (b as u64) { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I64GeS => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64GeS second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64GeS first operand not i64"))?;
                let result = if a >= b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I64GeU => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64GeU second operand not i64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64GeU first operand not i64"))?;
                let result = if (a as u64) >= (b as u64) { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I64Eqz => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Eqz operand not i64"))?;
                let result = if a == 0 { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Eq => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Eq second operand not f32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Eq first operand not f32"))?;
                let result = if a == b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Ne => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Ne second operand not f32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Ne first operand not f32"))?;
                let result = if a != b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Lt => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Lt second operand not f32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Lt first operand not f32"))?;
                let result = if a < b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Gt => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Gt second operand not f32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Gt first operand not f32"))?;
                let result = if a > b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Le => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Le second operand not f32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Le first operand not f32"))?;
                let result = if a <= b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Ge => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Ge second operand not f32"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Ge first operand not f32"))?;
                let result = if a >= b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Eq => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Eq second operand not f64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Eq first operand not f64"))?;
                let result = if a == b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Ne => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Ne second operand not f64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Ne first operand not f64"))?;
                let result = if a != b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Lt => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Lt second operand not f64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Lt first operand not f64"))?;
                let result = if a < b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Gt => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Gt second operand not f64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Gt first operand not f64"))?;
                let result = if a > b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Le => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Le second operand not f64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Le first operand not f64"))?;
                let result = if a <= b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Ge => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Ge second operand not f64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Ge first operand not f64"))?;
                let result = if a >= b { 1 } else { 0 };
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Abs => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Abs operand not f32"))?;
                let result = a.abs();
                engine
                    .exec_stack
                    .values
                    .push(Value::F32(FloatBits32::from_bits(result.to_bits())))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::F32Neg => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Neg operand not f32"))?;
                let result = -a;
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Ceil => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Ceil operand not f32"))?;
                let result = a.ceil();
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Floor => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Floor operand not f32"))?;
                let result = a.floor();
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Trunc => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Trunc operand not f32"))?;
                let result = {
                    #[cfg(feature = "std")]
                    {
                        a.trunc()
            ...
            Instruction::F32Nearest => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Nearest operand not f32"))?;
                let result = a.round();
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32Sqrt => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Sqrt operand not f32"))?;
                let result = a.sqrt();
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Add => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Add second operand not f64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Add first operand not f64"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::F64(FloatBits64::from_bits((a + b).to_bits())))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::F64Sub => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Sub second operand not f64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Sub first operand not f64"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::F64(FloatBits64::from_bits((a - b).to_bits())))
            ...
            Instruction::F64Mul => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Mul second operand not f64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Mul first operand not f64"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::F64(FloatBits64::from_bits((a * b).to_bits())))
            ...
            Instruction::F64Div => {
                let b = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Div second operand not f64"))?;
                let a = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Div first operand not f64"))?;
                engine
                    .exec_stack
                    .values
                    .push(Value::F64(FloatBits64::from_bits((a / b).to_bits())))
            ...
            Instruction::I32TruncF64S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("I32TruncF64S operand not f64"))?;
            ...
            Instruction::I32TruncF64U => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("I32TruncF64U operand not f64"))?;
            ...
            Instruction::I64TruncF32S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("I64TruncF32S operand not f32"))?;
            ...
            Instruction::I64TruncF32U => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("I64TruncF32U operand not f32"))?;
            ...
            Instruction::I64TruncF64S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("I64TruncF64S operand not f64"))?;
            ...
            Instruction::I64TruncF64U => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("I64TruncF64U operand not f64"))?;
            ...
            Instruction::F32ConvertI32S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("F32ConvertI32S operand not i32"))?;
                let result = a as f32;
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32ConvertI32U => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("F32ConvertI32U operand not i32"))?;
            ...
            Instruction::F32ConvertI64S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("F32ConvertI64S operand not i64"))?;
                let result = a as f32;
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F32ConvertI64U => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("F32ConvertI64U operand not i64"))?;
            ...
            Instruction::F32DemoteF64 => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F32DemoteF64 operand not f64"))?;
                let result = a as f32;
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64ConvertI32S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("F64ConvertI32S operand not i32"))?;
                let result = a as f64;
                engine
                    .exec_stack
                    .values
                    .push(Value::F64(FloatBits64::from_bits(result.to_bits())))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::F64ConvertI32U => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("F64ConvertI32U operand not i32"))?;
            ...
            Instruction::F64ConvertI64S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("F64ConvertI64S operand not i64"))?;
                let result = a as f64;
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64ConvertI64U => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("F64ConvertI64U operand not i64"))?;
            ...
            Instruction::F64PromoteF32 => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F64PromoteF32 operand not f32"))?;
                let result = a as f64;
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I32ReinterpretF32 => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("I32ReinterpretF32 operand not f32"))?;
            ...
            Instruction::I64ReinterpretF64 => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("I64ReinterpretF64 operand not f64"))?;
            ...
            Instruction::F32ReinterpretI32 => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("F32ReinterpretI32 operand not i32"))?;
            ...
            Instruction::F64ReinterpretI64 => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("F64ReinterpretI64 operand not i64"))?;
            ...
            Instruction::F64Abs => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Abs operand not f64"))?;
                let result = a.abs();
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Neg => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Neg operand not f64"))?;
                let result = -a;
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Ceil => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Ceil operand not f64"))?;
                let result = a.ceil();
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Floor => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Floor operand not f64"))?;
                let result = a.floor();
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Trunc => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Trunc operand not f64"))?;
                let result = {
                    #[cfg(feature = "std")]
                    {
                        a.trunc()
            ...
            Instruction::F64Nearest => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Nearest operand not f64"))?;
                let result = a.round();
                engine
                    .exec_stack
                    .values
            ...
            Instruction::F64Sqrt => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Sqrt operand not f64"))?;
                let result = a.sqrt();
                engine
                    .exec_stack
                    .values
            ...
            Instruction::I32Extend8S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Extend8S operand not i32"))?;
            ...
            Instruction::I32Extend16S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Extend16S operand not i32"))?;
            ...
            Instruction::I64Extend8S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Extend8S operand not i64"))?;
            ...
            Instruction::I64Extend16S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Extend16S operand not i64"))?;
            ...
            Instruction::I64Extend32S => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Extend32S operand not i64"))?;
            ...
            Instruction::I32Clz => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Clz operand not i32"))?;
                // Count leading zeros
            ...
            Instruction::I32Ctz => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Ctz operand not i32"))?;
            ...
            Instruction::I32Popcnt => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i32())
                    .ok_or_else(|| Error::validation_error("I32Popcnt operand not i32"))?;
            ...
            Instruction::I64Clz => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Clz operand not i64"))?;
                // Count leading zeros
            ...
            Instruction::I64Ctz => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Ctz operand not i64"))?;
                // Count trailing zeros
            ...
            Instruction::I64Popcnt => {
                let a = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .and_then(|v| v.as_i64())
                    .ok_or_else(|| Error::validation_error("I64Popcnt operand not i64"))?;
                // Count number of 1 bits
            ...
            Instruction::F32Min => {
                ...
                let b = b_val
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Min second operand not f32"))?;
                let a_val = engine
                    .exec_stack
                    .values
                    .pop()
                ...
                let a = a_val
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Min first operand not f32"))?;
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::F32(FloatBits32::from_float(result)))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::F32Max => {
                ...
                let b = b_val
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Max second operand not f32"))?;
                let a_val = engine
                    .exec_stack
                    .values
                    .pop()
                ...
                let a = a_val
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Max first operand not f32"))?;
            ...
            Instruction::F32Copysign => {
                ...
                let b = b_val
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Copysign second operand not f32"))?;
                let a_val = engine
                    .exec_stack
                    .values
                    .pop()
                ...
                let a = a_val
                    .and_then(|v| v.as_f32())
                    .ok_or_else(|| Error::validation_error("F32Copysign first operand not f32"))?;
            ...
            Instruction::F64Min => {
                ...
                let b = b_val
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Min second operand not f64"))?;
                let a_val = engine
                    .exec_stack
                    .values
                    .pop()
                ...
                let a = a_val
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Min first operand not f64"))?;
                // WebAssembly min: NaN if either operand is NaN, otherwise the smaller value
                let result = if a.is_nan() || b.is_nan() {
                    f64::NAN
                } else if a == 0.0 && b == 0.0 {
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::F64(FloatBits64::from_float(result)))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },
            Instruction::F64Max => {
                ...
                let b = b_val
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Max second operand not f64"))?;
                let a_val = engine
                    .exec_stack
                    .values
                    .pop()
                ...
                let a = a_val
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Max first operand not f64"))?;
                // WebAssembly max: NaN if either operand is NaN, otherwise the larger value
                let result = if a.is_nan() || b.is_nan() {
                    f64::NAN
                } else if a == 0.0 && b == 0.0 {
            ...
            Instruction::F64Copysign => {
                ...
                let b = b_val
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Copysign second operand not f64"))?;
                let a_val = engine
                    .exec_stack
                    .values
                    .pop()
                ...
                let a = a_val
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| Error::validation_error("F64Copysign first operand not f64"))?;
                // Copy sign from b to a
                let result = a.copysign(b);
                engine
                    .exec_stack
            ...
            Instruction::SelectWithType(_value_types) => {
                // SelectWithType behaves the same as Select for execution, the type information
                // is for validation
                let condition_val = engine
                    .exec_stack
                    .values
            ...
            Instruction::BrTable {
                ...
                // Select the target label: if index is in bounds, use targets[index], otherwise
                // use default_target
                let target_label = if index < targets.len() {
                    targets
                        .get(index)
                        .map_err(|e| Error::memory_out_of_bounds("Stack operation error"))?
                } else {
                    default_target
                };
            ...
            Instruction::MemoryFill(mem_idx) => {
                ...
                if offset + size > memory.size_in_bytes() {
                    return Err(Error::memory_out_of_bounds(
                        "MemoryFill operation out of bounds",
                    ));
                }

                // TODO: Fill memory with the specified value using the instruction layer
                // implementation use wrt_instructions::memory_ops::{MemoryFill,
                // MemoryOperations}; let fill_op = MemoryFill::new(mem_idx;
                // fill_op.execute(&mut memory, &Value::I32(offset as i32), &Value::I32(value as
                // i32), &Value::I32(size as i32))?;
                return Err(Error::runtime_execution_error(
                    "Memory fill not implemented",
                ));
            },
            ...
            Instruction::MemoryCopy(dst_mem_idx, src_mem_idx) => {
                ...
                // copy_op.execute(&mut memory, &Value::I32(dest as i32), &Value::I32(src as
                // i32), &Value::I32(size as i32))?;
                return Err(Error::runtime_execution_error(
                    "Memory copy not implemented",
                ));
            },
            ...
            Instruction::ReturnCallIndirect(type_idx, table_idx) => {
                ...
                let func_index = match func_index_val {
                    ...
                    _ => {
                        return Err(Error::validation_error(
                            "ReturnCallIndirect function index not i32",
                        ))
                    },
                };
                ...
                let table = self
                    ...
                    .map_err(|_| Error::validation_error("Stack operation error"))?;

                if func_index >= table.size() {
                    return Err(Error::memory_out_of_bounds(
                        "ReturnCallIndirect function index out of table bounds",
                    ));
                }
                ...
                let func_ref = table
                    .get(func_index)
                    .map_err(|e| Error::runtime_memory_access_error("Stack operation error"))?;

                let actual_func_idx = match func_ref {
                    Some(Value::FuncRef(Some(fref))) => fref.index,
                    Some(Value::FuncRef(None)) | None => {
                        return Err(Error::runtime_type_mismatch(
                            "ReturnCallIndirect null function reference",
                        ));
                    },
                    _ => {
                        return Err(Error::runtime_type_mismatch(
                            "ReturnCallIndirect invalid table element type",
                        ));
                    },
                };
                ...
                let function = module
                    .functions
                    .get(actual_func_idx as usize)
                    .map_err(|_| Error::validation_error("Stack operation error"))?;

                if function.type_idx != type_idx {
                    return Err(Error::runtime_type_mismatch(
                        "ReturnCallIndirect function type mismatch",
                    ));
                }
            ...
            Instruction::BrOnNonNull(label_idx) => {
                let ref_val = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .ok_or_else(|| {
                        Error::runtime_stack_underflow("Stack empty for br_on_non_null")
                    })?;
            ...
            Instruction::MemoryInit(data_seg_idx, mem_idx) => {
                ...
                let module = self.module_instance.module();
                let data_segment = module
                    .data
                    .get(data_seg_idx as usize)
                    .map_err(|_| Error::validation_error("Stack operation error"))?;

                // Bounds checks
                if dst_offset + size > memory.size_in_bytes() {
                    return Err(Error::memory_out_of_bounds(
                        "MemoryInit destination out of bounds",
                    ));
                }

                let data = data_segment
                    .data()
                    .map_err(|e| Error::runtime_memory_access_error("Data segment access error"))?;

                if src_offset + size > data.len() {
                    return Err(Error::memory_out_of_bounds(
                        "MemoryInit source out of bounds",
                    ));
                }
                ...
                for i in 0..size {
                    ...
                    memory
                        .write((dst_offset + i) as u32, &[*byte])
                        .map_err(|e| Error::runtime_memory_access_error("Memory write error"))?;
                }
            ...
            Instruction::RefAsNonNull => {
                let ref_val = engine
                    ...
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?
                    .ok_or_else(|| {
                        Error::runtime_stack_underflow("Stack empty for ref_as_non_null")
                    })?;
                ...
                if is_null {
                    ...
                    return Err(Error::runtime_trap_execution_error(
            ...
            Instruction::MemoryAtomicNotify { memarg } => {
                ...
                if effective_addr % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "MemoryAtomicNotify requires 4-byte alignment",
                    ));
                }

                // For now, implement as a no-op since we don't have a full threading model
                // In a full implementation, this would notify threads waiting on this memory
                // location
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(woken_count))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },

            Instruction::MemoryAtomicWait32 { memarg } => {
                let timeout_val = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?;
                let timeout = match timeout_val {
                    ...
                    _ => {
                        return Err(Error::validation_error(
                            "MemoryAtomicWait32 timeout not i64",
                        ))
                    },
                };

                let expected_val = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?;
                let expected = match expected_val {
                    ...
                    _ => {
                        return Err(Error::validation_error(
                            "MemoryAtomicWait32 expected not i32",
                        ))
                    },
                };
                ...
                if effective_addr % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "MemoryAtomicWait32 requires 4-byte alignment",
                    ));
                }
                ...
                let mut bytes = [0u8; 4];
                memory
                    .read(effective_addr, &mut bytes)
                    .map_err(|e| Error::runtime_memory_access_error("Memory read error"))?;
                ...
                let result = if current_val != expected {
                    ...
                    // In a full implementation, this would block the thread until notified or
                    // timeout For now, return "ok" (value was equal but we
                    // don't wait)
                    0i32 // "ok"
                };
            ...
            Instruction::I32AtomicLoad { memarg } => {
                ...
                if effective_addr % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicLoad requires 4-byte alignment",
                    ));
                }
            ...
            Instruction::I32AtomicStore { memarg } => {
                ...
                if effective_addr % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicStore requires 4-byte alignment",
                    ));
                }
            ...
            Instruction::I32AtomicRmwAdd { memarg } => {
                ...
                if effective_addr % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicRmwAdd requires 4-byte alignment",
                    ));
                }
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(old_value))
                    .map_err(|e| Error::runtime_stack_overflow("Stack operation error"))?;
                Ok(ControlFlow::Next)
            },

            Instruction::I32AtomicRmwCmpxchg { memarg } => {
                let replacement_val = engine
                    .exec_stack
                    .values
                    .pop()
                    .map_err(|e| Error::runtime_stack_underflow("Stack operation error"))?;
                let replacement = match replacement_val {
                    ...
                    _ => {
                        return Err(Error::validation_error(
                            "I32AtomicRmwCmpxchg replacement not i32",
                        ))
                    },
                };
                ...
                let expected = match expected_val {
                    ...
                    _ => {
                        return Err(Error::validation_error(
                            "I32AtomicRmwCmpxchg expected not i32",
                        ))
                    },
                };
                ...
                if effective_addr % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicRmwCmpxchg requires 4-byte alignment",
                    ));
                }
            ...
            Instruction::I32AtomicRmwSub { memarg } => {
                ...
                if (addr_i32 as usize + memarg.offset as usize) % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicRmwSub requires 4-byte alignment",
                    ));
                }
            ...
            Instruction::I32AtomicRmwAnd { memarg } => {
                ...
                if (addr_i32 as usize + memarg.offset as usize) % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicRmwAnd requires 4-byte alignment",
                    ));
                }
            ...
            Instruction::I32AtomicRmwOr { memarg } => {
                ...
                if (addr_i32 as usize + memarg.offset as usize) % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicRmwOr requires 4-byte alignment",
                    ));
                }
            ...
            Instruction::I32AtomicRmwXor { memarg } => {
                ...
                if (addr_i32 as usize + memarg.offset as usize) % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicRmwXor requires 4-byte alignment",
                    ));
                }
            ...
            Instruction::I32AtomicLoad16U { memarg } => {
                ...
                if (addr_i32 as usize + memarg.offset as usize) % 2 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicLoad16U requires 2-byte alignment",
                    ));
                }
            ...
            Instruction::I32AtomicRmwXchg { memarg } => {
                ...
                if (addr_i32 as usize + memarg.offset as usize) % 4 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicRmwXchg requires 4-byte alignment",
                    ));
                }
            ...
            Instruction::I32AtomicStore16 { memarg } => {
                ...
                if (addr_i32 as usize + memarg.offset as usize) % 2 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I32AtomicStore16 requires 2-byte alignment",
                    ));
                }
            ...
            Instruction::I64AtomicLoad { memarg } => {
                ...
                if (addr_i32 as usize + memarg.offset as usize) % 8 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I64AtomicLoad requires 8-byte alignment",
                    ));
                }
            ...
            Instruction::I64AtomicStore { memarg } => {
                ...
                if (addr_i32 as usize + memarg.offset as usize) % 8 != 0 {
                    return Err(Error::runtime_unaligned_memory_access(
                        "I64AtomicStore requires 8-byte alignment",
                    ));
                }
            ...
            Instruction::MemoryAtomicWait64 { memarg } => {
                ...
                let timeout_i64 = match timeout {
                    ...
                    _ => {
                        return Err(Error::validation_error(
                            "MemoryAtomicWait64 timeout not i64",
                        ))
                    },
                };
                ...
                let expected_i64 = match expected {
                    ...
                    _ => {
                        return Err(Error::validation_error(
                            "MemoryAtomicWait64 expected not i64",
                        ))
                    },
                };
            ...
            Instruction::I32AtomicRmw8AddU { memarg } => {
                ...
                let effective_addr = (addr_i32 as u32)
                    .checked_add(memarg.offset)
                    .ok_or_else(|| Error::memory_error("Address overflow"))?;
                ...
                engine
                    .exec_stack
                    .values
                    .push(Value::I32(old_value as i32))
                    .map_err(|_| Error::runtime_stack_overflow("Stack overflow"))?;
                Ok(ControlFlow::Next)
            },
            ...
            Instruction::I32AtomicRmw8CmpxchgU { memarg } => {
                ...
                let replacement_i32 = match replacement {
                    ...
                    _ => {
                        return Err(Error::validation_error(
                            "I32AtomicRmw8CmpxchgU replacement not i32",
                        ))
                    },
                };
                ...
                let expected_i32 = match expected {
                    ...
                    _ => {
                        return Err(Error::validation_error(
                            "I32AtomicRmw8CmpxchgU expected not i32",
                        ))
                    },
                };
                ...
                let addr_i32 = match addr {
                    ...
                    _ => {
                        return Err(Error::validation_error(
                            "I32AtomicRmw8CmpxchgU addr not i32",
                        ))
                    },
                };
            ...
            | Instruction::I64AtomicRmw32CmpxchgU { .. } => {
                ...
                return Err(Error::runtime_execution_error(
                    "Atomic operations not supported in MVP",
                ));
            },
            _ => {
...
impl StacklessFrame {
    fn table_init(
        ...
        let segment = module
            ...
            .map_err(|_| Error::runtime_execution_error("Invalid element segment index"))?;
        ...
        let n = len_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("table.init len not i32"))? as u32;
        let src_offset = src_offset_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("table.init src_offset not i32"))?
            as u32;
        let dst_offset = dst_offset_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("table.init dst_offset not i32"))?
            as u32;
    ...
    fn table_copy(
        ...
        let n = len_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("table.copy len not i32"))? as u32;
        let src_offset = src_offset_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("table.copy src_offset not i32"))?
            as u32;
        let dst_offset = dst_offset_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("table.copy dst_offset not i32"))?
            as u32;
    ...
    fn table_fill(&mut self, table_idx: u32, engine: &mut StacklessEngine) -> Result<()> {
        ...
        let n = n_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("table.fill count not i32"))? as u32;
    ...
    fn memory_init(
        ...
        let n: u32 = n_val
            ...
            .try_into()
            .unwrap();
        let src_offset: u32 = src_offset_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("memory.init src_offset not i32"))?
            .try_into()
            .unwrap();
        let dst_offset: u32 = dst_offset_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("memory.init dst_offset not i32"))?
            .try_into()
            .unwrap();
        ...
        let data_segment = self
            ...
            .map_err(|_| Error::runtime_execution_error("Invalid data segment index"))?;

        // Bounds checks (Wasm Spec)
        if dst_offset.checked_add(n).map_or(true, |end| end as usize > memory.size_bytes())
            || src_offset.checked_add(n).map_or(true, |end| match data_segment.data() {
                Ok(data) => end as usize > data.len(),
                Err(_) => true,
            })
        {
            return Err(Error::memory_error(
                "Memory bounds check failed for memory.init",
            ));
        }
        if n == 0 {
            return Ok();
        ...
        let data_to_write = data_segment
            .data()?
            .get((src_offset as usize)..(src_offset as usize + n as usize))
            .ok_or_else(|| Error::memory_error("memory.init source data segment OOB"))?;
    ...
    fn memory_copy(
        ...
        let src_offset: u32 = src_offset_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("memory.copy src_offset not i32"))?
            .try_into()
            .unwrap();
        let dst_offset: u32 = dst_offset_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("memory.copy dst_offset not i32"))?
            .try_into()
            .unwrap();
        ...
        if dst_offset
            .checked_add(n)
            .map_or(true, |end| end as usize > dst_memory.size_bytes())
            || src_offset
                .checked_add(n)
                .map_or(true, |end| end as usize > src_memory.size_bytes())
        {
        ...
        #[cfg(not(feature = "std"))]
        {
            ...
            // Since BoundedVec doesn't provide as_mut_slice(), we'll use a different
            // approach
            ...
            if n > 4096 {
                ...
                while copied < n {
                    ...
                    let mut temp_buffer: wrt_foundation::bounded::BoundedVec<u8, 4096, _> =
                        wrt_foundation::bounded::BoundedVec::new(provider)?;
                    ...
                    for _ in 0..to_copy {
                        temp_buffer
                            .push(0u8)
                            .map_err(|_| Error::memory_error("Failed to allocate temp buffer"))?;
                    }
    ...
    fn memory_fill(&mut self, mem_idx: u32, engine: &mut StacklessEngine) -> Result<()> {
        ...
        let dst_offset: u32 = dst_offset_val
            .and_then(|v| v.as_i32())
            .ok_or_else(|| Error::type_error("memory.fill dst_offset not i32"))?
            .try_into()
            .unwrap();
    ...
    pub fn execute_simd_operation(
        ...
        return Err(Error::runtime_execution_error(
            "SIMD operations not implemented",
        ));
    }

    /// Check if an instruction is a SIMD operation (when SIMD instructions are
    /// added to main enum)
    ///
    ...
    ///
    /// # Arguments
    /// * `_instruction` - The instruction to check (placeholder for future SIMD
    ///   instructions)
    ///
    /// # Returns
    ...
    fn extract_simd_operation(
        ...
        // This is a placeholder for when SIMD instructions are added to the main
        // Instruction enum When that happens, this method would pattern match
        // against SIMD variants and extract the SimdOp

        // Example of how it would work:
        // match instruction {
        //     Instruction::V128Load { offset, align } => Some(SimdOp::V128Load {
        // offset: *offset, align: *align }),     Instruction::I32x4Add =>
        // Some(SimdOp::I32x4Add),     Instruction::F32x4Mul =>
        // Some(SimdOp::F32x4Mul),     // ... other SIMD instructions
        //     _ => None,
        // }
...
impl Validatable for StacklessFrame {
    ...
    fn validate(&self) -> Result<()> {
        ...
        if self.pc > self.function_body()?.body.len() {
            return Err(Error::runtime_out_of_bounds(
                "Program counter exceeds function body length",
            ));
        }
