...
impl<T> Mutex<T> {
    ...
    pub fn lock(&self) -> Result<core::cell::RefMut<T>> {
        self.0.try_borrow_mut().map_err(|_| Error::poisoned_lock("Mutex poisoned"))
    }
}
...
pub struct StacklessCallbackRegistry {
    /// For simplicity in no_std, we'll use a simple approach without nested
    /// HashMaps
    #[cfg(feature = "std")]
...
impl StacklessStack {
    ...
    pub fn module(&self) -> Result<&Arc<Module>> {
        self.module
            .as_ref()
            .ok_or_else(|| Error::runtime_execution_error("No module loaded in stack"))
    }
...
impl StacklessEngine {
    ...
    /// This is the main integration point for instruction-level fuel
    /// consumption
    pub fn consume_instruction_fuel(
        &mut self,
        instruction_type: InstructionFuelType,
    ) -> Result<()> {
        let op_type = match instruction_type {
            InstructionFuelType::SimpleConstant => {
            ...
            InstructionFuelType::SimpleArithmetic => {
                wrt_foundation::operations::Type::WasmSimpleArithmetic
            },
            InstructionFuelType::ComplexArithmetic => {
                wrt_foundation::operations::Type::WasmComplexArithmetic
            },
            InstructionFuelType::FloatArithmetic => {
            ...
            InstructionFuelType::SimpleControl => {
                wrt_foundation::operations::Type::WasmSimpleControl
            },
            InstructionFuelType::ComplexControl => {
            ...
            InstructionFuelType::MemoryManagement => {
            ...
            InstructionFuelType::TypeConversion => {
                wrt_foundation::operations::Type::WasmTypeConversion
            },
            InstructionFuelType::SimdOperation => {
                wrt_foundation::operations::Type::WasmSimdOperation
            },
            InstructionFuelType::AtomicOperation => {
    ...
    fn execute_memory_load(&mut self, mem_op: MemoryLoad, memory_idx: u32) -> Result<()> {
        ...
        let effective_addr = base_addr
            .checked_add(mem_op.offset)
        ...
        match mem_op.value_type {
            ...
            _ => {
                return Err(Error::runtime_execution_error(
                    "Unsupported data type for memory store",
                ));
            },
        }
    ...
    pub fn execute(
        ...
        for arg in args {
            self.locals
                .push(arg)
                .map_err(|_| Error::runtime_stack_overflow("Local variable overflow"))?;
        }
            ...
            match &self.exec_stack.state {
                StacklessExecutionState::Running => {
                    ...
                    if let Some(current_module) = &self.current_module {
                        if let Ok(function) =
                            current_module.get_function(self.exec_stack.func_idx as usize)
                        {
    ...
    fn collect_results(&mut self) -> Result<Vec<Value>> {
        ...
        if let Some(current_module) = &self.current_module {
            if let Ok(func_type) =
                current_module.get_function_type(self.exec_stack.func_idx as usize)
            {
                ...
                for _ in 0..result_count {
                    match self.exec_stack.values.pop()? {
                        Some(value) => {
                            results.insert(0, value); // Insert at beginning to
                                                      // maintain order
                        },
                        None => {
    ...
    fn execute_instruction(&mut self, opcode: u8, code: &[u8]) -> Result<()> {
        match opcode {
            ...
            0x2E => {
                ...
                // ASIL-B: Validate alignment is power of 2 and within bounds
                if align > 1 {
                    // 2^1 = 2 bytes max for i16
                    return Err(Error::validation_error(
                        "Invalid alignment for i32.load16_s",
                    ));
                }
            ...
            0x2F => {
                ...
                if align > 1 {
                    // 2^1 = 2 bytes max for i16
                    return Err(Error::validation_error(
                        "Invalid alignment for i32.load16_u",
                    ));
                }
            ...
            0x32 => {
                ...
                if align > 1 {
                    // 2^1 = 2 bytes max for i16
                    return Err(Error::validation_error(
                        "Invalid alignment for i64.load16_s",
                    ));
                }
            ...
            0x33 => {
                ...
                if align > 1 {
                    // 2^1 = 2 bytes max for i16
                    return Err(Error::validation_error(
                        "Invalid alignment for i64.load16_u",
                    ));
                }
            ...
            0x34 => {
                ...
                if align > 2 {
                    // 2^2 = 4 bytes max for i32
                    return Err(Error::validation_error(
                        "Invalid alignment for i64.load32_s",
                    ));
                }
            ...
            0x35 => {
                ...
                if align > 2 {
                    // 2^2 = 4 bytes max for i32
                    return Err(Error::validation_error(
                        "Invalid alignment for i64.load32_u",
                    ));
                }
            ...
            0x40 => {
                ...
                let delta = match delta_value {
                    Value::I32(d) => {
                        ...
                        if d < 0 {
                            return Err(Error::runtime_invalid_parameter(
    ...
    fn execute_simd_instruction(&mut self, simd_opcode: u32) -> Result<()> {
        ...
        match simd_opcode {
            ...
            12 => {
                ...
                Err(Error::runtime_execution_error(
                    "SIMD v128.const not yet implemented",
                ))
            },
            ...
            13 => {
                ...
                Err(Error::runtime_execution_error(
                    "SIMD i8x16.shuffle not yet implemented",
                ))
            },
            14 => {
                // i8x16.swizzle - Swizzle lanes
                Err(Error::runtime_execution_error(
                    "SIMD i8x16.swizzle not yet implemented",
                ))
            },
            ...
            15 => {
                ...
                Err(Error::runtime_execution_error(
                    "SIMD i8x16.splat not yet implemented",
                ))
            },
            16 => {
                // i16x8.splat - Splat i32 to 8x16-bit vector
                Err(Error::runtime_execution_error(
                    "SIMD i16x8.splat not yet implemented",
                ))
            },
            17 => {
                // i32x4.splat - Splat i32 to 4x32-bit vector
                Err(Error::runtime_execution_error(
                    "SIMD i32x4.splat not yet implemented",
                ))
            },
            18 => {
                // i64x2.splat - Splat i64 to 2x64-bit vector
                Err(Error::runtime_execution_error(
                    "SIMD i64x2.splat not yet implemented",
                ))
            },
            19 => {
                // f32x4.splat - Splat f32 to 4x32-bit vector
                Err(Error::runtime_execution_error(
                    "SIMD f32x4.splat not yet implemented",
                ))
            },
            20 => {
                // f64x2.splat - Splat f64 to 2x64-bit vector
                Err(Error::runtime_execution_error(
                    "SIMD f64x2.splat not yet implemented",
                ))
            },
            ...
            158 => {
                ...
                Err(Error::runtime_execution_error(
                    "SIMD i32x4.add not yet implemented",
                ))
            },
            159 => {
                // i32x4.sub - Subtract 4x32-bit vectors
                Err(Error::runtime_execution_error(
                    "SIMD i32x4.sub not yet implemented",
                ))
            },
            160 => {
                // i32x4.mul - Multiply 4x32-bit vectors
                Err(Error::runtime_execution_error(
                    "SIMD i32x4.mul not yet implemented",
                ))
            },
            ...
            174 => {
                ...
                Err(Error::runtime_execution_error(
                    "SIMD i64x2.add not yet implemented",
                ))
            },
            175 => {
                // i64x2.sub - Subtract 2x64-bit vectors
                Err(Error::runtime_execution_error(
                    "SIMD i64x2.sub not yet implemented",
                ))
            },
            176 => {
                // i64x2.mul - Multiply 2x64-bit vectors
                Err(Error::runtime_execution_error(
                    "SIMD i64x2.mul not yet implemented",
                ))
            },
    ...
    fn execute_atomic_instruction(&mut self, atomic_opcode: u32) -> Result<()> {
        ...
        match atomic_opcode {
            ...
            0 => {
                ...
                Err(Error::runtime_execution_error(
                    "Atomic memory.atomic.notify not yet implemented",
                ))
            },
            1 => {
                // memory.atomic.wait32 - Wait on 32-bit value
                Err(Error::runtime_execution_error(
                    "Atomic memory.atomic.wait32 not yet implemented",
                ))
            },
            2 => {
                // memory.atomic.wait64 - Wait on 64-bit value
                Err(Error::runtime_execution_error(
                    "Atomic memory.atomic.wait64 not yet implemented",
                ))
            },
            3 => {
                // atomic.fence - Memory fence
                Err(Error::runtime_execution_error(
                    "Atomic atomic.fence not yet implemented",
                ))
            },
            ...
            23 => {
                ...
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.store not yet implemented",
                ))
            },
            24 => {
                // i64.atomic.store - Atomic store 64-bit integer
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.store not yet implemented",
                ))
            },
            25 => {
                // i32.atomic.store8 - Atomic store 8-bit from 32-bit
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.store8 not yet implemented",
                ))
            },
            26 => {
                // i32.atomic.store16 - Atomic store 16-bit from 32-bit
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.store16 not yet implemented",
                ))
            },
            27 => {
                // i64.atomic.store8 - Atomic store 8-bit from 64-bit
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.store8 not yet implemented",
                ))
            },
            28 => {
                // i64.atomic.store16 - Atomic store 16-bit from 64-bit
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.store16 not yet implemented",
                ))
            },
            29 => {
                // i64.atomic.store32 - Atomic store 32-bit from 64-bit
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.store32 not yet implemented",
                ))
            },
            ...
            30 => {
                ...
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.rmw.add not yet implemented",
                ))
            },
            31 => {
                // i64.atomic.rmw.add - Atomic read-modify-write add 64-bit
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.rmw.add not yet implemented",
                ))
            },
            32 => {
                // i32.atomic.rmw8.add_u - Atomic RMW add 8-bit to 32-bit
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.rmw8.add_u not yet implemented",
                ))
            },
            33 => {
                // i32.atomic.rmw16.add_u - Atomic RMW add 16-bit to 32-bit
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.rmw16.add_u not yet implemented",
                ))
            },
            ...
            46 => {
                ...
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.rmw.sub not yet implemented",
                ))
            },
            47 => {
                // i64.atomic.rmw.sub - Atomic read-modify-write subtract 64-bit
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.rmw.sub not yet implemented",
                ))
            },
            ...
            62 => {
                ...
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.rmw.and not yet implemented",
                ))
            },
            63 => {
                // i64.atomic.rmw.and - Atomic read-modify-write AND 64-bit
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.rmw.and not yet implemented",
                ))
            },
            78 => {
                // i32.atomic.rmw.or - Atomic read-modify-write OR 32-bit
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.rmw.or not yet implemented",
                ))
            },
            79 => {
                // i64.atomic.rmw.or - Atomic read-modify-write OR 64-bit
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.rmw.or not yet implemented",
                ))
            },
            94 => {
                // i32.atomic.rmw.xor - Atomic read-modify-write XOR 32-bit
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.rmw.xor not yet implemented",
                ))
            },
            95 => {
                // i64.atomic.rmw.xor - Atomic read-modify-write XOR 64-bit
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.rmw.xor not yet implemented",
                ))
            },
            ...
            110 => {
                ...
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.rmw.xchg not yet implemented",
                ))
            },
            111 => {
                // i64.atomic.rmw.xchg - Atomic read-modify-write exchange 64-bit
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.rmw.xchg not yet implemented",
                ))
            },
            ...
            126 => {
                ...
                Err(Error::runtime_execution_error(
                    "Atomic i32.atomic.rmw.cmpxchg not yet implemented",
                ))
            },
            127 => {
                // i64.atomic.rmw.cmpxchg - Atomic compare-and-swap 64-bit
                Err(Error::runtime_execution_error(
                    "Atomic i64.atomic.rmw.cmpxchg not yet implemented",
                ))
            },
    ...
    fn read_block_type(&mut self, code: &[u8]) -> Result<wrt_foundation::BlockType> {
        ...
        if self.exec_stack.pc >= code.len() {
            return Err(Error::runtime_execution_error(
                "Unexpected end of bytecode while reading block type",
            ));
        }
        ...
        match byte {
            ...
            0x7F => Ok(wrt_foundation::BlockType::Value(Some(
                wrt_foundation::ValueType::I32,
            ))),
            0x7E => Ok(wrt_foundation::BlockType::Value(Some(
                wrt_foundation::ValueType::I64,
            ))),
            0x7D => Ok(wrt_foundation::BlockType::Value(Some(
                wrt_foundation::ValueType::F32,
            ))),
            0x7C => Ok(wrt_foundation::BlockType::Value(Some(
                wrt_foundation::ValueType::F64,
            ))),
            _ => {
        ...
        if let Ok(instruction) = body.instructions.get(pc) {
            ...
            match instruction {
                ...
                wrt_foundation::types::Instruction::I32Add => {
                    use wrt_instructions::arithmetic_ops::{
                        ArithmeticContext,
                        ArithmeticOp,
                    };
                    ArithmeticOp::I32Add.execute(self)
                ...
                wrt_foundation::types::Instruction::LocalGet(index) => {
                    use wrt_instructions::variable_ops::{
                        VariableContext,
                        VariableOp,
                    };
                ...
                wrt_foundation::types::Instruction::Block { block_type_idx } => {
                    ...
                    let label = Label {
                        ...
                        arity: 0, /* Simplified - would need to decode block_type_idx to get
                                   * actual arity */
                                    ...
                                    for _ in 0..local.count {
                                        let default_value = match local.value_type {
                                            ...
                                            wrt_foundation::types::ValueType::F32 => {
                                                Value::F32(wrt_foundation::FloatBits32(0))
                                            },
                                            wrt_foundation::types::ValueType::F64 => {
                                                Value::F64(wrt_foundation::FloatBits64(0))
                                            },
                    ...
                    if let Some(module_instance) = &self.current_module {
                        ...
                        let actual_func_idx = match func_ref {
                            ...
                            Some(Value::FuncRef(None)) => {
                                return Err(Error::runtime_null_reference(
                                    "Null function reference",
                                ))
                            },
                            None => {
                                return Err(Error::runtime_null_reference("Table entry is empty "))
                            },
                        ...
                        let module = module_instance.module();
                        let expected_type =
                            module.get_function_type(type_idx).ok_or_else(|| {
                                Error::type_error("Expected function type not found ")
                            })?;
                        let actual_func =
                            module.get_function(actual_func_idx).ok_or_else(|| {
                                Error::runtime_function_not_found("Function not found")
                            })?;
                        let actual_type = module
                            .get_function_type(actual_func.type_idx)
                            .ok_or_else(|| Error::type_error("Actual function type not found "))?;
                ...
                wrt_foundation::types::Instruction::I32Eq => {
                    use wrt_instructions::comparison_ops::{
                        ComparisonContext,
                        ComparisonOp,
                    };
                    ComparisonOp::I32Eq.execute(self)
...
impl StacklessEngine {
    ...
    fn branch_to_label(&mut self, label_depth: u32) -> Result<()> {
        ...
        let target_label = self
            ...
            .labels
            .get(target_idx)
            .map_err(|_| Error::runtime_execution_error("Label not found"))?;
...
impl VariableContext for StacklessEngine {
    fn get_local(&self, index: u32) -> Result<Value> {
        self.locals
            .get(index as usize)
            .map_err(|_| Error::runtime_out_of_bounds("Local variable index out of bounds"))
    }
...
impl wrt_instructions::memory_ops::MemoryContext for StacklessEngine {
    ...
    fn get_memory(&mut self, memory_idx: u32) -> Result<&mut dyn MemoryOperations> {
        ...
        // Since we can't return a mutable reference from an Arc<Memory>, we'll need to
        // handle this differently
        Err(Error::memory_not_found(
...
impl ControlContext for StacklessEngine {
    ...
    fn push_control_value(&mut self, value: Value) -> Result<()> {
        self.exec_stack
            .values
            .push(value)
            .map_err(|_| Error::runtime_stack_overflow("Operand stack overflow"))?;
        Ok(())
    }
    ...
    fn enter_block(&mut self, block_type: Block) -> Result<()> {
        ...
        self.exec_stack
            .labels
            .push(label)
            .map_err(|_| Error::runtime_stack_overflow("Label stack overflow"))?;
        Ok(())
    }
    ...
    fn exit_block(&mut self) -> Result<Block> {
        ...
        let label = self
            ...
            .labels
            .remove(last_idx)
            .map_err(|_| Error::runtime_stack_underflow("No block to exit"))?;
            ...
            for _ in 0..arity {
                match self.exec_stack.values.pop()? {
                    Some(value) => {
                        ...
                        values
                            .insert(0, value)
                            .map_err(|_| Error::runtime_stack_overflow("Branch values overflow"))?;
                    },
                    None => {
    ...
    fn return_function(&mut self) -> Result<()> {
        ...
        if let Some(current_module) = &self.current_module {
                ...
                current_module.get_function_type(self.exec_stack.func_idx as usize)
            {
                ...
                for _ in 0..return_arity {
                    match self.exec_stack.values.pop()? {
                        Some(value) => {
                            // Insert at beginning to maintain order (since we're popping in
                            // reverse)
        ...
        self.exec_stack.state = StacklessExecutionState::Returning { values };
        Ok(())
    }
    ...
    fn execute_call_indirect(
        ...
        if let Some(module_instance) = &self.current_module {
            ...
            let expected_type = module
                .get_function_type(type_idx as u32)
                .ok_or_else(|| Error::type_error("Expected function type not found "))?;
...
impl FunctionOperations for StacklessEngine {
    ...
    fn get_function_type(&self, func_idx: u32) -> Result<u32> {
        ...
        if let Some(current_module) = self.get_current_module() {
            ...
            Err(Error::runtime_error(
                "No current module available for function lookup ",
            ))
        }
    }
    ...
    fn get_table_function(&self, table_idx: u32, elem_idx: u32) -> Result<u32> {
        ...
        if let Some(_current_module) = self.get_current_module() {
            ...
            Err(Error::runtime_error(
                "No current module available for table lookup ",
            ))
        }
    }
