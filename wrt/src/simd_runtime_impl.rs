...
impl SimdProvider for AssilCompliantSimdProvider {
    fn execute_with_provider(&self, op: &SimdOp, inputs: &[Value]) -> Result<Value> {
        ...
        match op {
            // Load Operations
            V128Load { offset: _, align: _ } => execute_v128_load(inputs),
            V128Load8x8S { offset: _, align: _ } => execute_v128_load_8x8_s(inputs),
            V128Load8x8U { offset: _, align: _ } => execute_v128_load_8x8_u(inputs),
            V128Load16x4S { offset: _, align: _ } => execute_v128_load_16x4_s(inputs),
            V128Load16x4U { offset: _, align: _ } => execute_v128_load_16x4_u(inputs),
            V128Load32x2S { offset: _, align: _ } => execute_v128_load_32x2_s(inputs),
            V128Load32x2U { offset: _, align: _ } => execute_v128_load_32x2_u(inputs),
            V128Load8Splat { offset: _, align: _ } => execute_v128_load_8_splat(inputs),
            V128Load16Splat { offset: _, align: _ } => execute_v128_load_16_splat(inputs),
            V128Load32Splat { offset: _, align: _ } => execute_v128_load_32_splat(inputs),
            V128Load64Splat { offset: _, align: _ } => execute_v128_load_64_splat(inputs),
            V128Store { offset: _, align: _ } => execute_v128_store(inputs),
            ...
            I16x8ExtMulHighI8x16S => simd_additional_ops::execute_i16x8_ext_mul_high_i8x16_s(inputs),
            I16x8ExtMulLowI8x16U => simd_additional_ops::execute_i16x8_ext_mul_low_i8x16_u(inputs),
            I16x8ExtMulHighI8x16U => simd_additional_ops::execute_i16x8_ext_mul_high_i8x16_u(inputs),
            I32x4ExtMulLowI16x8S => simd_additional_ops::execute_i32x4_ext_mul_low_i16x8_s(inputs),
            I32x4ExtMulHighI16x8S => simd_additional_ops::execute_i32x4_ext_mul_high_i16x8_s(inputs),
            I32x4ExtMulLowI16x8U => simd_additional_ops::execute_i32x4_ext_mul_low_i16x8_u(inputs),
            I32x4ExtMulHighI16x8U => simd_additional_ops::execute_i32x4_ext_mul_high_i16x8_u(inputs),
            I64x2ExtMulLowI32x4S => simd_additional_ops::execute_i64x2_ext_mul_low_i32x4_s(inputs),
            I64x2ExtMulHighI32x4S => simd_additional_ops::execute_i64x2_ext_mul_high_i32x4_s(inputs),
            I64x2ExtMulLowI32x4U => simd_additional_ops::execute_i64x2_ext_mul_low_i32x4_u(inputs),
            I64x2ExtMulHighI32x4U => simd_additional_ops::execute_i64x2_ext_mul_high_i32x4_u(inputs),
            
            // Pairwise Addition
            I16x8ExtAddPairwiseI8x16S => simd_additional_ops::execute_i16x8_ext_add_pairwise_i8x16_s(inputs),
            I16x8ExtAddPairwiseI8x16U => simd_additional_ops::execute_i16x8_ext_add_pairwise_i8x16_u(inputs),
            I32x4ExtAddPairwiseI16x8S => simd_additional_ops::execute_i32x4_ext_add_pairwise_i16x8_s(inputs),
            I32x4ExtAddPairwiseI16x8U => simd_additional_ops::execute_i32x4_ext_add_pairwise_i16x8_u(inputs),
...
fn execute_i64x2_add(inputs: &[Value]) -> Result<Value> {
    ...
    for i in 0..2 {
        ...
        let a_val = u64::from_le_bytes([a_bytes[0], a_bytes[1], a_bytes[2], a_bytes[3], 
                                        a_bytes[4], a_bytes[5], a_bytes[6], a_bytes[7]]);
        let b_val = u64::from_le_bytes([b_bytes[0], b_bytes[1], b_bytes[2], b_bytes[3], 
...
fn execute_i64x2_neg(inputs: &[Value]) -> Result<Value> {
    ...
    for i in 0..2 {
        let a_bytes = &a[i*8..i*8+8];
        let a_val = u64::from_le_bytes([a_bytes[0], a_bytes[1], a_bytes[2], a_bytes[3], 
                                        a_bytes[4], a_bytes[5], a_bytes[6], a_bytes[7]]) as i64;
...
fn execute_f64x2_add(inputs: &[Value]) -> Result<Value> {
    ...
    for i in 0..2 {
        let a_bytes = &a[i*8..i*8+8];
        let b_bytes = &b[i*8..i*8+8];
        let a_val = f64::from_le_bytes([a_bytes[0], a_bytes[1], a_bytes[2], a_bytes[3], 
                                        a_bytes[4], a_bytes[5], a_bytes[6], a_bytes[7]]);
        let b_val = f64::from_le_bytes([b_bytes[0], b_bytes[1], b_bytes[2], b_bytes[3], 
                                        b_bytes[4], b_bytes[5], b_bytes[6], b_bytes[7]]);
        let res_val = a_val + b_val;
        let res_bytes = res_val.to_le_bytes();
        result[i*8..i*8+8].copy_from_slice(&res_bytes);
...
fn execute_i64x2_all_true(inputs: &[Value]) -> Result<Value> {
    ...
    let all_true = (0..2).all(|i| {
        ...
        let val = u64::from_le_bytes([bytes[0], bytes[1], bytes[2], bytes[3], 
...
fn execute_i32x4_extract_lane(inputs: &[Value], lane: u8) -> Result<Value> {
    ...
    let lane_idx = lane as usize;
    let val = u32::from_le_bytes([a[lane_idx*4], a[lane_idx*4+1], a[lane_idx*4+2], a[lane_idx*4+3]]);
    Ok(Value::I32(val as i32))
}
...
fn execute_i64x2_extract_lane(inputs: &[Value], lane: u8) -> Result<Value> {
    ...
    let val = u64::from_le_bytes([val_bytes[0], val_bytes[1], val_bytes[2], val_bytes[3], 
...
fn execute_f32x4_extract_lane(inputs: &[Value], lane: u8) -> Result<Value> {
    ...
    let lane_idx = lane as usize;
    let val = f32::from_le_bytes([a[lane_idx*4], a[lane_idx*4+1], a[lane_idx*4+2], a[lane_idx*4+3]]);
    Ok(Value::F32(val))
}
...
fn execute_f64x2_extract_lane(inputs: &[Value], lane: u8) -> Result<Value> {
    ...
    let lane_idx = lane as usize;
    let val_bytes = &a[lane_idx*8..lane_idx*8+8];
    let val = f64::from_le_bytes([val_bytes[0], val_bytes[1], val_bytes[2], val_bytes[3], 
                                  val_bytes[4], val_bytes[5], val_bytes[6], val_bytes[7]]);
    Ok(Value::F64(val))
}
...
fn execute_i32x4_trunc_sat_f64x2_s_zero(inputs: &[Value]) -> Result<Value> {
    ...
    for i in 0..2 {
        ...
        let f_val = f64::from_le_bytes([
            ...
            f_bytes[4], f_bytes[5], f_bytes[6], f_bytes[7]
        ]);
...
fn execute_i8x16_narrow_i16x8_s(inputs: &[Value]) -> Result<Value> {
    ...
    for i in 0..8 {
        ...
        let i8_val = if i16_val > 127 { 127i8 } else if i16_val < -128 { -128i8 } else { i16_val as i8 };
...
fn execute_i8x16_narrow_i16x8_u(inputs: &[Value]) -> Result<Value> {
    ...
    for i in 0..8 {
        let i16_bytes = &a[i*2..i*2+2];
        let i16_val = i16::from_le_bytes([i16_bytes[0], i16_bytes[1]]);
        let u8_val = if i16_val > 255 { 255u8 } else if i16_val < 0 { 0u8 } else { i16_val as u8 };
        result[i] = u8_val;
    }
...
fn execute_i16x8_narrow_i32x4_s(inputs: &[Value]) -> Result<Value> {
    ...
    for i in 0..4 {
        ...
        let i16_val = if i32_val > 32767 { 32767i16 } else if i32_val < -32768 { -32768i16 } else { i32_val as i16 };
...
fn execute_i16x8_narrow_i32x4_u(inputs: &[Value]) -> Result<Value> {
    ...
    for i in 0..4 {
        let i32_bytes = &a[i*4..i*4+4];
        let i32_val = i32::from_le_bytes([i32_bytes[0], i32_bytes[1], i32_bytes[2], i32_bytes[3]]);
        let u16_val = if i32_val > 65535 { 65535u16 } else if i32_val < 0 { 0u16 } else { i32_val as u16 };
