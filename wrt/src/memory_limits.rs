...
mod validation {
    ...
    fn validate_resource_budget() {
        ...
        assert!(budget < 600 * 1024, "Resource budget exceeds 600KB: {}KB", budget / 1024);
    }
    
    #[test]
    fn validate_execution_budget() {
        let budget = 
            execution::MODULE_SIZE_LIMIT +
            execution::FUNCTIONS_PER_MODULE_LIMIT * BYTES_PER_FUNCTION +
            execution::STACK_DEPTH_LIMIT * 64; // Stack frame size
            
        assert!(budget < 1536 * 1024, "Execution budget exceeds 1.5MB: {}KB", budget / 1024);
    }
    ...
    fn validate_total_budget() {
        ...
        assert!(total <= TOTAL_BUDGET, 
            "Total budget exceeds 2MB: {}KB", total / 1024);
    }
}
